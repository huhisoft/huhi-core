...
#include "huhi/ui/webui/resources/grit/huhi_webui_resources_map.h"
...
const std::map<std::string, std::string> CreatePathPrefixAliasesMap() {
  ...
  // huhi://huhi-resources/blah/X could be mapped to
  ...
  std::map<std::string, std::string> aliases = {
      {"@out_folder@/gen/huhi/ui/webui/resources/", ""},
  };
  return aliases;
}
...
void AddResourcesToMap(ResourcesMap* resources_map) {
  ...
  for (size_t i = 0; i < kHuhiWebuiResourcesSize; ++i) {
    const auto& resource = kHuhiWebuiResources[i];
...
HuhiSharedResourcesDataSource::HuhiSharedResourcesDataSource() {
}

HuhiSharedResourcesDataSource::~HuhiSharedResourcesDataSource() {
}

std::string HuhiSharedResourcesDataSource::GetSource() {
  return "huhi-resources";
}

void HuhiSharedResourcesDataSource::StartDataRequest(
    const GURL& url,
...
bool HuhiSharedResourcesDataSource::AllowCaching() {
  // Should not be cached to reflect dynamically-generated contents that may
  // depend on the current locale.
  return true;
}

std::string HuhiSharedResourcesDataSource::GetMimeType(
...
bool HuhiSharedResourcesDataSource::ShouldServeMimeTypeAsContentTypeHeader() {
  return true;
}

std::string
HuhiSharedResourcesDataSource::GetAccessControlAllowOriginForOrigin(
