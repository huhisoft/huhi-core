...
from lib.config import get_huhi_version, get_raw_version, get_env_var
...
def download_from_github(args, logging):
    ...
    # HUHI_REPO defined in helpers.py
    repo = GitHub(get_env_var('GITHUB_TOKEN')).repos(HUHI_REPO)

    if args.tag:
        tag_name = args.tag
    else:
        tag_name = get_huhi_version()
    ...
    for asset in release['assets']:
        ...
        elif re.match(r'huhi_installer-ia32\.exe$', asset['name']):
            if args.uploaded:
                if not args.platform:
                    args.platform = []
        ...
        elif re.match(r'huhi_installer-x64\.exe$', asset['name']):
            if args.uploaded:
                if not args.platform:
                    args.platform = []
...
def parse_args():
    desc = "Upload Windows/Mac install files to Omaha server" \
           ...
           "\n\nCHANNEL: The Huhi channel, i.e. \'nightly\', \'dev\', \'beta\', \'release\'" \
           ...
           "\nHUHI_GITHUB_TOKEN: Github token to download from a draft release if not published yet. " \
           "(ONLY REQUIRED IF --github)" \
           "\nnpm_config_huhi_version: Chromium version (only if not in huhi-core directory with huhi-browser" \
           " in the parent dir)"
    parser = argparse.ArgumentParser(
        description=desc, formatter_class=RawTextHelpFormatter)
    parser.add_argument('--version', help='full huhi version to upload')
...
def main():
    ...
    if args.debug:
        ...
        logging.debug('huhi_version: {}'.format(get_upload_version()))
    ...
    for source_file in file_list:
        ...
        if args.github:
            if re.match(r'.*\.dmg$', source_file):
                ...
                app_info['arch'] = 'x64'
            elif re.match(r'.*huhi_installer-ia32\.exe$', source_file):
                app_info['platform'] = 'win32'
                app_info['arch'] = 'ia32'
            elif re.match(r'.*huhi_installer-x64\.exe$', source_file):
                app_info['platform'] = 'win32'
                app_info['arch'] = 'x64'
