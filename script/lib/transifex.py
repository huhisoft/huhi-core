...
from lib.grd_string_replacements import generate_huhiified_node, get_override_file_path
...
transifex_project_name = 'huhi'
base_url = 'https://www.transifex.com/api/2/'
transifex_handled_slugs = [
    'android_huhi_strings',
    'huhi_generated_resources',
    'huhi_components_resources',
    'huhi_extension',
    'rewards_extension'
]
...
def transifex_name_from_filename(source_file_path, filename):
    ...
    if 'huhi_components_strings' in source_file_path:
        return 'huhi_components_resources'
    elif ext == '.grd':
        return filename
    elif 'huhi_extension' in source_file_path:
        return 'huhi_extension'
    elif 'huhi_rewards' in source_file_path:
        return 'rewards_extension'
    elif 'ethereum-remote-client/huhi/app' in source_file_path:
        return 'huhi_ethereum_remote_client_extension'
...
def is_translateable_string(grd_file_path, message_tag):
    ...
    # to be translateable in Huhi. These can be found in the main function in
    # huhi/script/chromium-rebase-l10n.py
...
def get_original_grd(src_root, grd_file_path):
    """Obtains the Chromium GRD file for a specified Huhi GRD file."""
    # TODO: consider passing this mapping into the script from l10nUtil.js
    grd_file_name = os.path.basename(grd_file_path)
    if grd_file_name == 'components_huhi_strings.grd':
        return os.path.join(src_root, 'components',
                            'components_chromium_strings.grd')
    elif grd_file_name == 'huhi_strings.grd':
...
def check_for_chromium_upgrade_extra_langs(src_root, grd_file_path):
    """Checks the Huhi GRD file vs the Chromium GRD file for extra
    languages."""
    chromium_grd_file_path = get_original_grd(src_root, grd_file_path)
    if not chromium_grd_file_path:
        return
    huhi_langs = get_transifex_languages(grd_file_path)
    chromium_langs = get_transifex_languages(chromium_grd_file_path)
    x_huhi_extra_langs = huhi_langs - chromium_langs
    assert len(x_huhi_extra_langs) == 0, (
        'Huhi GRD %s has extra languages %s over Chromium GRD %s' % (
            grd_file_path, chromium_grd_file_path,
            list(x_huhi_extra_langs)))
    x_chromium_extra_langs = chromium_langs - huhi_langs
    assert len(x_chromium_extra_langs) == 0, (
        'Chromium GRD %s has extra languages %s over Huhi GRD %s' % (
...
def huhiify(string_value):
    """Replace Chromium branded strings with Huhi beranded strings."""
    return (string_value.replace('Chrome', 'Huhi')
            .replace('Chromium', 'Huhi')
            .replace('Google', 'Huhi')
            .replace('Huhi Docs', 'Google Docs')
            .replace('Huhi Drive', 'Google Drive')
            .replace('Huhi Play', 'Google Play')
            .replace('Huhi Safe', 'Google Safe')
            .replace('Sends URLs of some pages you visit to Huhi', 'Sends URLs of some pages you visit to Google')
            .replace('Huhi Account', 'Huhi sync chain'))
...
def upload_missing_translation_to_transifex(source_string_path, lang_code,
    ...
    url = base_url + url_part
    translated_value = huhiify(translated_value)
    payload = {
...
def get_acceptable_json_lang_codes(langs_dir_path):
    ...
    # Source language for Huhi locales
...
def get_chromium_grd_src_with_fallback(grd_file_path, huhi_source_root):
    source_root = os.path.dirname(huhi_source_root)
    chromium_grd_file_path = get_original_grd(source_root, grd_file_path)
    if not chromium_grd_file_path:
        filename = os.path.basename(grd_file_path)
        rel_path = os.path.relpath(grd_file_path, huhi_source_root)
...
def pull_xtb_without_transifex(grd_file_path, huhi_source_root):
    xtb_files = get_xtb_files(grd_file_path)
    ...
    for idx, xtb_file in enumerate(xtb_file_paths):
        ...
        for node in xml_tree.xpath('//translation'):
            ...
            generate_huhiified_node(node, False, True)
