...
default_replacements = [
    (r'Huhi Web Store', r'Web Store'),
    (r'Automatically send usage statistics and crash reports to Huhi',
     r'Automatically send crash reports to Huhi'),
    (r'Automatically sends usage statistics and crash reports to Huhi',
     r'Automatically sends crash reports to Huhi'),
...
fixup_replacements = [
    (r'Huhi Cloud Print', r'Google Cloud Print'),
    (r'Huhi Docs', r'Google Docs'),
    (r'Huhi Drive', r'Google Drive'),
    (r'Huhi OS', r'Chrome OS'),
    (r'Huhi Safe Browsing', r'Google Safe Browsing'),
    ...
    (r'Sends URLs of some pages you visit to Huhi', r'Sends URLs of some pages you visit to Google'),
    (r'Google Google', r'Google'),
    (r'Huhi Account', r'Huhi sync chain'),
]
...
def huhiify_grd_text(text, is_main_text, branding_replacements_only):
    """Replaces text string to Huhi wording"""
...
def generate_huhiified_node(elem, is_comment, branding_replacements_only):
    """Replaces a node and attributes to Huhi wording"""
    if elem.text:
        elem.text = huhiify_grd_text(elem.text, not is_comment, branding_replacements_only)

    if elem.tail:
        elem.tail = huhiify_grd_text(elem.tail, not is_comment, branding_replacements_only)

    if 'desc' in elem.keys():
        elem.attrib['desc'] = huhiify_grd_text(elem.attrib['desc'], False, branding_replacements_only)
    if 'meaning' in elem.keys():
        elem.attrib['meaning'] = huhiify_grd_text(elem.attrib['meaning'], False, branding_replacements_only)
    for child in elem:
        generate_huhiified_node(child, is_comment, branding_replacements_only)
...
def update_huhiified_grd_tree_override(source_xml_tree, branding_replacements_only):
    """Takes in a grd(p) tree and replaces all messages and comments with Huhi wording"""
    for elem in source_xml_tree.xpath('//message'):
        generate_huhiified_node(elem, False, branding_replacements_only)
    for elem in source_xml_tree.xpath('//comment()'):
        generate_huhiified_node(elem, True, branding_replacements_only)


def write_huhiified_grd_override(source_string_path):
    """Takes in a grd file and replaces all messages and comments with Huhi wording"""
    source_xml_tree = etree.parse(source_string_path)
    update_huhiified_grd_tree_override(source_xml_tree, False)
    write_xml_file_from_tree(source_string_path, source_xml_tree)


def get_override_file_path(source_string_path):
    """Obtain src/huhi source string override path for local grd strings with replacements"""
