...
from lib.config import get_huhi_version, get_chrome_version, get_raw_version
...
def get_app_info(appinfo, args):
    ...
    changelog_url = 'https://github.com/huhisoft/huhi-browser/blob/master/CHANGELOG_DESKTOP.md'

    if args.version:
        version_values = args.version.split('.')
        chrome_major = version_values[0]
        huhi_version = '.'.join(version_values[1:3])
        version = args.version
    else:
        chrome_major = get_chrome_version().split('.')[0]
        huhi_version = get_upload_version()
        version = chrome_major + '.' + huhi_version
...
def get_upload_version():
    '''
    Returns the version of huhi-browser
    '''
...
def sign_update_sparkle(dmg, dsaprivpem):
    ...
    https://github.com/huhisoftware/Sparkle/blob/master/bin/sign_update
