...
def main():
    ...
    if os.environ.get('HUHI_CORE_DIR'):
        huhi_core_dir = os.environ.get('HUHI_CORE_DIR')
    else:
        logging.error("Error: Required environment variable \'HUHI_CORE_DIR\' not set! Exiting...")
        exit(1)

    if args.debug:
        logging.basicConfig(stream=sys.stderr, level=logging.DEBUG)
        logging.debug('huhi_version: {}'.format(get_raw_version()))
        ...
        logging.debug('huhi_core_dir: {}'.format(huhi_core_dir))
    ...
    for item in ['upload_to_aptly', 'upload_to_rpm_repo']:
        ...
        if re.match(r'.*rpm.*', item):
            bucket = 'huhi-browser-rpm-staging-'
        else:
            bucket = 'huhi-browser-apt-staging-'

        upload_script = os.path.join(huhi_core_dir, 'script', item)
...
def download_linux_pkgs_from_github(args, logging):
    ...
    repo = GitHub(args.github_token).repos(HUHI_REPO)
    tag_name = args.tag
    release = {}
...
def parse_args():
    ...
    parser.add_argument('-c', '--channel', help='The Huhi channel, i.e. \'nightly\', \'dev\', \'beta\', \'release\'',
                        required=True)
