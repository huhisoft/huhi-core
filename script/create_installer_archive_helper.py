...
def SignAndCopyPreSignedBinaries(skip_signing, output_dir, staging_dir, current_version):
    if not skip_signing:
        ...
        """Copies already signed three binaries - huhi.exe and chrome.dll
        ...
        shutil.copy(os.path.join(src_dir, 'huhi.exe'), chrome_dir)
        shutil.copy(os.path.join(src_dir, 'chrome.dll'), version_dir)


def HuhiCopyAllFilesToStagingDir(config, staging_dir, g_archive_inputs):
    current_dir = os.path.realpath(os.path.dirname(os.path.realpath(__file__)))

    huhi_extension_locales_src_dir_path = os.path.realpath(
        os.path.join(current_dir, os.pardir, 'components',
                     'huhi_extension', 'extension', 'huhi_extension', '_locales'))
    CopyExtensionLocalization('huhi_extension', huhi_extension_locales_src_dir_path,
                              config, staging_dir, g_archive_inputs)

    huhi_rewards_locales_src_dir_path = os.path.realpath(
        os.path.join(current_dir, os.pardir, 'components',
                     'huhi_rewards', 'resources', 'extension', 'huhi_rewards', '_locales'))
    CopyExtensionLocalization('huhi_rewards', huhi_rewards_locales_src_dir_path,
                              config, staging_dir, g_archive_inputs)
...
def CopyExtensionLocalization(extension_name, locales_src_dir_path, config, staging_dir, g_archive_inputs):
    ...
    locales_dest_path = os.path.join(locales_dest_path, config.get('GENERAL', 'huhi_resources.pak'),
