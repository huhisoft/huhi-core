...
from lib.config import (PLATFORM, get_env_var, product_name,
                        ...
                        get_huhi_version, get_platform_key, get_raw_version)
...
def main():
    ...
    # HUHI_REPO is defined in lib/helpers.py for now
    repo = GitHub(get_env_var('GITHUB_TOKEN')).repos(HUHI_REPO)

    tag = get_huhi_version()
    ...
    DIST_NAME = get_zip_name(project_name(), get_huhi_version(), args.target_arch)
    SYMBOLS_NAME = get_zip_name(project_name(), get_huhi_version(), args.target_arch, 'symbols')
    DSYM_NAME = get_zip_name(project_name(), get_huhi_version(), args.target_arch, 'dsym')
    PDB_NAME = get_zip_name(project_name(), get_huhi_version(), args.target_arch, 'pdb')

    print('[INFO] Uploading release {}'.format(release['tag_name']))
    # Upload Huhi with GitHub Releases API.
    if args.target_os != 'android':
        upload_huhi(repo, release, os.path.join(d_dir, DIST_NAME), force=args.force)
        upload_huhi(repo, release, os.path.join(d_dir, SYMBOLS_NAME), force=args.force)
    else:
        o_dir = o_dir + '/apks'
    # if PLATFORM == 'darwin':
    #     upload_huhi(repo, release, os.path.join(d_dir, DSYM_NAME))
    # elif PLATFORM == 'win32':
    #     upload_huhi(repo, release, os.path.join(d_dir, PDB_NAME))

    pkgs = get_huhi_packages(o_dir, release_channel(), get_raw_version(),
                              args.target_os, args.target_arch, args.target_apk_base)

    for pkg in pkgs:
        upload_huhi(repo, release, os.path.join(o_dir, pkg), force=args.force)

    # mksnapshot = get_zip_name('mksnapshot', get_huhi_version(), args.target_arch)
    # upload_huhi(repo, release, os.path.join(d_dir, mksnapshot))
...
def get_huhi_packages(dir, channel, version, target_os, target_arch='x64', target_apk_base=''):
    pkgs = []
    ...
    for file in os.listdir(dir):
        if os.path.isfile(os.path.join(dir, file)):
            ...
            if PLATFORM == 'darwin':
                ...
                file_dmg = 'Huhi-Browser' + channel_capitalized_dashed + '.dmg'
                file_pkg = 'Huhi-Browser' + channel_capitalized_dashed + '.pkg'
                if re.match(r'Huhi Browser' + channel_capitalized_spaced + r'.*\.dmg$', file):
                    filecopy(file_path, file_dmg)
                    pkgs.append(file_dmg)
                elif file == file_dmg:
                    pkgs.append(file_dmg)
                elif re.match(r'Huhi Browser' + channel_capitalized_spaced + r'.*\.pkg$', file):
                ...
                if re.match(r'huhi-browser' + channel_dashed + '_' + version + r'.*\.deb$', file):
                    pkgs.append(file)
                elif re.match(r'huhi-browser' + channel_dashed + '-' + version + r'.*\.rpm$', file):
                    pkgs.append(file)
                elif target_os == 'android':
                    if target_arch == 'arm':
                        if not target_apk_base:
                            if re.match(r'Huhi.*arm.apk$', file):
                                pkgs.append(file)
                        else:
                            if target_apk_base == 'classic':
                                ...
                                if file == 'HuhiModernarm.apk':
                                    pkgs.append(file)
                            elif target_apk_base == 'mono':
                                if file == 'HuhiMonoarm.apk':
                                    pkgs.append(file)
                    elif target_arch == 'arm64':
                        if file == 'HuhiMonoarm64.apk':
                            pkgs.append(file)
                    elif target_arch == 'x86':
                        if not target_apk_base:
                            if re.match(r'Huhi.*x86.apk$', file):
                                pkgs.append(file)
                        else:
                            if target_apk_base == 'classic':
                                if file == 'Huhix86.apk':
                                ...
                                if file == 'HuhiModernx86.apk':
                                    pkgs.append(file)
                            elif target_apk_base == 'mono':
                                if file == 'HuhiMonox86.apk':
                                    pkgs.append(file)
                    elif target_arch == 'x64':
                        if file == 'HuhiMonox64.apk':
                            pkgs.append(file)
                    else:
                        if re.match(r'Huhi.*.apk$', file):
                            pkgs.append(file)
            elif PLATFORM == 'win32':
                ...
                file_stub = 'HuhiBrowser' + channel_arch_extension
                file_stub_silent = 'HuhiBrowserSilent' + channel_arch_extension
                file_stub_untagged = 'HuhiBrowserUntagged' + channel_arch_extension
                file_stn = 'HuhiBrowserStandalone' + channel_arch_extension
                file_stn_silent = 'HuhiBrowserStandaloneSilent' + channel_arch_extension
                file_stn_untagged = 'HuhiBrowserStandaloneUntagged' + channel_arch_extension
                file_installer = 'huhi_installer-' + target_arch + '.exe'

                if re.match(r'HuhiBrowser' + channel_capitalized + r'Setup' + arch + r'_.*\.exe', file):
                    filecopy(file_path, file_stub)
                    pkgs.append(file_stub)
                elif re.match(r'HuhiBrowserSilent' + channel_capitalized + r'Setup' + arch + r'_.*\.exe', file):
                    filecopy(file_path, file_stub_silent)
                    pkgs.append(file_stub_silent)
                elif re.match(r'HuhiBrowserUntagged' + channel_capitalized + r'Setup' + arch + r'_.*\.exe', file):
                    filecopy(file_path, file_stub_untagged)
                    pkgs.append(file_stub_untagged)
                elif re.match(r'HuhiBrowserStandalone' + channel_capitalized + r'Setup' + arch + r'_.*\.exe', file):
                    filecopy(file_path, file_stn)
                    pkgs.append(file_stn)
                elif re.match(r'HuhiBrowserStandaloneSilent' + channel_capitalized + r'Setup' + arch +
                              r'_.*\.exe', file):
                    filecopy(file_path, file_stn_silent)
                    pkgs.append(file_stn_silent)
                elif re.match(r'HuhiBrowserStandaloneUntagged' + channel_capitalized + r'Setup' + arch +
                              r'_.*\.exe', file):
                    filecopy(file_path, file_stn_untagged)
                    pkgs.append(file_stn_untagged)
                elif re.match(r'huhi_installer.exe', file):
...
def create_release_draft(repo, tag):
    ...
    nightly_winstallers = (
        '`HuhiBrowserNightlySetup.exe` and `HuhiBrowserNightlySetup32.exe`')
    dev_winstallers = (
        '`HuhiBrowserDevSetup.exe` and `HuhiBrowserDevSetup32.exe`')
    beta_winstallers = (
        '`HuhiBrowserBetaSetup.exe` and `HuhiBrowserBetaSetup32.exe`')
    release_winstallers = (
        '`HuhiBrowserSetup.exe` and `HuhiBrowserSetup32.exe`')

    nightly_dev_beta_warning = '''*This is not the released version of Huhi.
...
version of Huhi on the desktop. These builds show our work in progress and \
...
# Mac installation
Install Huhi-Browser{channel_capitalized_dashed}.dmg on your system.

# Linux install instructions
http://huhi-browser.readthedocs.io/en/latest/installing-huhi.html#linux
...
def upload_huhi(github, release, file_path, filename=None, force=False):
    ...
    try:
        for asset in release['assets']:
            if asset['name'] == filename:
                ...
                github.repos(HUHI_REPO).releases.assets(asset['id']).delete()
    ...
    if 'armv7l' in filename:
        ...
        upload_huhi(github, release, arm_file_path)
...
def auth_token():
    ...
    message = ('Error: Please set the $HUHI_GITHUB_TOKEN '
