...
from lib.grd_string_replacements import (write_xml_file_from_tree, write_huhiified_grd_override,
                                         update_huhiified_grd_tree_override, get_override_file_path)
...
def generate_overrides_and_replace_strings(source_string_path):
    ...
    update_huhiified_grd_tree_override(original_xml_tree_with_branding_fixes, True)
    write_huhiified_grd_override(source_string_path)
...
def main():
    args = parse_args()
    # This file path is a string path inside huhi/ but just recently copied
    ...
    if basename == 'huhi_strings':
        ...
        elem1.text = 'Inbound rule for Huhi Beta to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_DESCRIPTION_CANARY"]')[0]
        elem1.text = 'Inbound rule for Huhi Nightly to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//message[@name="IDS_INBOUND_MDNS_RULE_DESCRIPTION_DEV"]')[0]
        elem1.text = 'Inbound rule for Huhi Dev to allow mDNS traffic.'
        elem1.attrib.pop('desc')
        elem1.attrib.pop('translateable')
        elem1 = xml_tree.xpath(
            '//part[@file="settings_chromium_strings.grdp"]')[0]
        elem1.set('file', 'settings_huhi_strings.grdp')
