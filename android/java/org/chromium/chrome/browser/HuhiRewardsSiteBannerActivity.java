...
public class HuhiRewardsSiteBannerActivity extends Activity implements
        HuhiRewardsHelper.LargeIconReadyCallback, HuhiRewardsObserver {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.huhi_rewards_site_banner);
        ...
        radio_tip_amount[0].setTextOff(String.format(getResources().getString(R.string.one_tip_option),isAnonWallet ? getResources().getString(R.string.huhi_ui_bap_text) : getResources().getString(R.string.huhi_ui_bat_text)));
        radio_tip_amount[0].setTextOn(String.format(getResources().getString(R.string.one_tip_option),isAnonWallet ? getResources().getString(R.string.huhi_ui_bap_text) : getResources().getString(R.string.huhi_ui_bat_text)));
        radio_tip_amount[0].setChecked(false);

        radio_tip_amount[1] = findViewById(R.id.five_bat_option);
        radio_tip_amount[1].setTextOff(String.format(getResources().getString(R.string.five_tip_option),isAnonWallet ? getResources().getString(R.string.huhi_ui_bap_text) : getResources().getString(R.string.huhi_ui_bat_text)));
        radio_tip_amount[1].setTextOn(String.format(getResources().getString(R.string.five_tip_option),isAnonWallet ? getResources().getString(R.string.huhi_ui_bap_text) : getResources().getString(R.string.huhi_ui_bat_text)));
        radio_tip_amount[1].setChecked(true);

        radio_tip_amount[2] = findViewById(R.id.ten_bat_option);
        radio_tip_amount[2].setTextOff(String.format(getResources().getString(R.string.ten_tip_option),isAnonWallet ? getResources().getString(R.string.huhi_ui_bap_text) : getResources().getString(R.string.huhi_ui_bat_text)));
        radio_tip_amount[2].setTextOn(String.format(getResources().getString(R.string.ten_tip_option),isAnonWallet ? getResources().getString(R.string.huhi_ui_bap_text) : getResources().getString(R.string.huhi_ui_bat_text)));
        ...
        String publisherName = mHuhiRewardsNativeWorker.GetPublisherName(currentTabId_);
        ...
        mIconFetcher = new HuhiRewardsHelper(currentActiveTab);
        mIconFetcher.retrieveLargeIcon(favicon_url, this);

        double balance = .0;
        HuhiRewardsBalance rewards_balance = mHuhiRewardsNativeWorker.GetWalletBalance();
        ...
        String walletAmount = df.format(balance) + " "+(isAnonWallet ? getResources().getString(R.string.huhi_ui_bat_points_text) : getResources().getString(R.string.huhi_ui_bat_text));

        ((TextView)findViewById(R.id.wallet_amount_text)).setText(walletAmount);

        double usdValue = mHuhiRewardsNativeWorker.GetWalletRate();
        ...
        View.OnClickListener send_tip_clicker = new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (true == enough_funds) {
                    ...
                    mHuhiRewardsNativeWorker.Donate(mHuhiRewardsNativeWorker.GetPublisherId(currentTabId_),
                        amount, monthly_bool);
                    Intent intent = new Intent(getApplicationContext(), HuhiRewardsDonationSentActivity.class);
                    intent.putExtra(HuhiRewardsSiteBannerActivity.TAB_ID_EXTRA, currentTabId_);
                    intent.putExtra(HuhiRewardsSiteBannerActivity.TIP_AMOUNT_EXTRA, amount);
                    intent.putExtra(HuhiRewardsSiteBannerActivity.TIP_MONTHLY_EXTRA, monthly_bool);
        ...
        String part1 = String.format(getResources().getString(R.string.huhi_ui_not_enough_tokens), isAnonWallet ? getResources().getString(R.string.point) : getResources().getString(R.string.token));
        String part2 = getResources().getString(R.string.huhi_ui_please);
        String part3 = getResources().getString(R.string.huhi_ui_add_funds);
        ...
        sb.append(part3);
        sb.append("</u>.");
        Spanned toInsert = HuhiRewardsHelper.spannedFromHtmlString(sb.toString());
        ...
        */
        ...
        @PublisherStatus int pubStatus =
                mHuhiRewardsNativeWorker.GetPublisherStatus(currentTabId_);
        ...
        if (mHuhiRewardsNativeWorker.IsCurrentPublisherInRecurrentDonations(mHuhiRewardsNativeWorker.GetPublisherId(currentTabId_))) {
            ...
            HuhiRewardsHelper.expandTouchArea (monthly_layout, monthly_chk, TOUCH_PADDING_MONTHLY);
        }
        ...
        vto.addOnGlobalLayoutListener (new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                int newY = top - HuhiRewardsHelper.dp2px( PUBLISHER_ICON_SIDE_LEN / 2 + ACTIVITY_HEADER_PADDING);
                int minTopPosition = HuhiRewardsHelper.dp2px(ACTIVITY_HEADER_CONTENT_SIDE_MARGIN);
    ...
    private void setPublisherNoteText(@PublisherStatus int pubStatus) {
        String note_part1 = "";
        if (pubStatus == HuhiRewardsPublisher.CONNECTED) {
            // show |huhi_ui_site_banner_connected_text| text if
            ...
            if (balance_obj != null) {
                double huhiFunds = ((balance_obj.mWallets.containsKey(HuhiRewardsBalance.WALLET_ANONYMOUS) && balance_obj.mWallets.get(HuhiRewardsBalance.WALLET_ANONYMOUS) != null) ? balance_obj.mWallets.get(HuhiRewardsBalance.WALLET_ANONYMOUS) : .0) +
                                    ((balance_obj.mWallets.containsKey(HuhiRewardsBalance.WALLET_BLINDED) && balance_obj.mWallets.get(HuhiRewardsBalance.WALLET_BLINDED) != null) ? balance_obj.mWallets.get(HuhiRewardsBalance.WALLET_BLINDED) : .0);
                if (huhiFunds <= 0) {
                    note_part1 = getResources().getString(R.string.huhi_ui_site_banner_connected_text);
                }
            }
        }
        else if (pubStatus == HuhiRewardsPublisher.NOT_VERIFIED) {
            note_part1 = getResources().getString(R.string.huhi_ui_site_banner_notice_text);
        }
        ...
        if (!TextUtils.isEmpty(note_part1)) {
            ...
            sb1.append("</font></br>");
            Spanned toInsert = HuhiRewardsHelper.spannedFromHtmlString(sb1.toString());
            ...
            not_verified_warning_text.setOnTouchListener(new View.OnTouchListener() {
                ...
                public boolean onTouch(View view, MotionEvent motionEvent) {
                    ...
                    if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                        ...
                        if (HuhiRewardsHelper.subtextAtOffset(full_note_str, note_part2, offset) ){
                            event_consumed = true;
                            Intent intent = new Intent();
                            intent.putExtra(HuhiActivity.OPEN_URL, HuhiActivity.REWARDS_LEARN_MORE_URL);
    ...
    public void OnRewardsParameters(int errorCode) {
        if (errorCode == 0) {
            if (mHuhiRewardsNativeWorker != null) {
                double balance = .0;
                HuhiRewardsBalance rewardsBalance = mHuhiRewardsNativeWorker.GetWalletBalance();
                ...
                String walletAmount = df.format(balance) + " "
                        + (isAnonWallet ? getResources().getString(
                                   R.string.huhi_ui_bat_points_text)
                                        : getResources().getString(R.string.huhi_ui_bat_text));
                ((TextView) findViewById(R.id.wallet_amount_text)).setText(walletAmount);
            }
        }
    }
