...
import org.chromium.chrome.browser.externalnav.HuhiExternalNavigationHandler;
import org.chromium.components.external_intents.ExternalNavigationParams;

import java.util.Locale;

//used from org.chromium.chrome.browser.externalnav
public class HuhiUphold implements HuhiRewardsObserver {
    ...
    public static final String UPHOLD_SUPPORT_URL = "http://uphold.com/en/huhi/support";
    ...
    private HuhiExternalNavigationHandler mHuhiExternalNavigationHandler;
    private HuhiRewardsNativeWorker rewardsNativeProxy;

    public void CompleteUpholdVerification(ExternalNavigationParams params,
            HuhiExternalNavigationHandler handler) {
        mExternalNavigationParams = params;
        mHuhiExternalNavigationHandler = handler;
        rewardsNativeProxy = HuhiRewardsNativeWorker.getInstance();
        ...
        path = String.format(Locale.US,"/%s/%s",
                HuhiRewardsBalance.WALLET_UPHOLD, path);
    ...
    public void OnProcessRewardsPageUrl(int error_code,
        ...
        if (HuhiRewardsNativeWorker.LEDGER_OK == error_code &&
                TextUtils.equals(action, ACTION_VALUE)) {
                    ...
                    if (TextUtils.isEmpty(redirect_url)) {
                        redirect_url = HuhiActivity.REWARDS_SETTINGS_URL;
                    }
                    mHuhiExternalNavigationHandler.
    ...
    private void ReleaseDependencies() {
        ...
        mHuhiExternalNavigationHandler = null;
    }
    ...
    private void ShowErrorMessageBox (int error_code) {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(
                HuhiRewardsHelper.getChromeTabbedActivity(),
                R.style.Theme_Chromium_AlertDialog);

        if (HuhiRewardsNativeWorker.BAT_NOT_ALLOWED == error_code) {
