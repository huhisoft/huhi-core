...
public class HuhiExternalNavigationHandler extends ExternalNavigationHandler {
    private HuhiUphold mHuhiUphold;

    public HuhiExternalNavigationHandler(ExternalNavigationDelegate delegate) {
        super(delegate);
    }
    ...
    private boolean isUpholdOverride(ExternalNavigationParams params) {
        if (!params.getUrl().startsWith(HuhiUphold.UPHOLD_REDIRECT_URL)) return false;
        return true;
    }

    private void CompleteUpholdVerification(ExternalNavigationParams params) {
        mHuhiUphold = new HuhiUphold();
        mHuhiUphold.CompleteUpholdVerification(params, this);
    }
