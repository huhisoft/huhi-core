...
public class NTPUtil {
    ...
    public static void updateOrientedUI(Context context, ViewGroup view) {
        ...
        if (ConfigurationUtils.isLandscape(context) && UserPrefs.get(Profile.getLastUsedRegularProfile()).getBoolean(HuhiPref.NEW_TAB_PAGE_SHOW_BACKGROUND_IMAGE)) {
            // In landscape
    ...
    public static int checkForNonDistruptiveBanner(NTPImage ntpImage, SponsoredTab sponsoredTab) {
        HuhiRewardsNativeWorker mHuhiRewardsNativeWorker = HuhiRewardsNativeWorker.getInstance();

        if (sponsoredTab.shouldShowBanner()) {
            if (UserPrefs.get(Profile.getLastUsedRegularProfile()).getBoolean(HuhiPref.ENABLED)) {
                ...
                } else if (HuhiAdsNativeHelper.nativeIsLocaleValid(Profile.getLastUsedRegularProfile())) {
                    if (ntpImage instanceof Wallpaper) {
                ...
                if (ntpImage instanceof Wallpaper && !mHuhiRewardsNativeWorker.IsCreateWalletInProcess()) {
                    return SponsoredImageUtil.BR_INVALID_OPTION;
                }
            }
        }
    ...
    private static SpannableString getBannerText(ChromeActivity chromeActivity, int ntpType,
        ...
        Spanned learnMoreSpanned = HuhiRewardsHelper.spannedFromHtmlString(bannerText);
        SpannableString learnMoreTextSS = new SpannableString(learnMoreSpanned.toString());

        ForegroundColorSpan brOffForegroundSpan = new ForegroundColorSpan(chromeActivity.getResources().getColor(R.color.huhi_theme_color));
    ...
    public static boolean isReferralEnabled() {
        ...
        boolean isReferralEnabled = UserPrefs.get(Profile.getLastUsedRegularProfile()).getInteger(HuhiPref.NEW_TAB_PAGE_SUPER_REFERRAL_THEMES_OPTION) == 1 ? true : false;
    ...
    public static void openNewTab(boolean isIncognito, String url) {
        ChromeTabbedActivity chromeTabbedActivity = HuhiRewardsHelper.getChromeTabbedActivity();
