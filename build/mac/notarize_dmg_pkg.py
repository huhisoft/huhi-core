...
# src/out/Release/Huhi_Browser_CHANNEL_Packaging), the signing directory is
# relative to that
...
from signing_helper import GetHuhiSigningConfig
...
def create_config(config_args, development, mac_provisioning_profile):
    ...
    config_class = GetHuhiSigningConfig(config_class, mac_provisioning_profile)
    return config_class(*config_args)


def NotarizeHuhiDmgPkg(paths, config, dmg, pkg, outdir, signed):
    """
    Notarize Huhi .dmg and .pkg files.
    """
    uuids_to_path_map = {}
    for dist in config.distributions:
        ...
        for result in notarize.wait_for_results(
            ...
            huhi_path = uuids_to_path_map[result]
            notarize.staple(huhi_path)
...
def main():
    ...
    rc = NotarizeHuhiDmgPkg(paths, config, args.dmg, args.pkg, args.outdir, args.signed)
    return rc
