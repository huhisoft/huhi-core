...
def _OverrideVersionKey(plist, huhi_version):
  """ `minor.build` version string is used for update.
  When we begin to use the Major version component, Huhi version string will
  ...
  version_values = huhi_version.split('.')
...
def Main(argv):
  ...
  parser.add_option('--huhi_channel', dest='huhi_channel', action='store',
      type='string', default=None, help='Channel (beta, dev, nightly)')
  parser.add_option('--huhi_product_dir_name', dest='huhi_product_dir_name',
