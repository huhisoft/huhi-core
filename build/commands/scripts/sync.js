...
const installDepotTools = (options = config.defaultOptions) => {
  options.cwd = config.huhiCoreDir
...
async function RunCommand () {
  ...
  let huhiCoreRef = program.args[0]
  if (!huhiCoreRef) {
    huhiCoreRef = program.init ? config.getProjectVersion('huhi-core') : null
  }

  if (huhiCoreRef || program.init || program.force) {
    // we're doing a reset of huhi-core so try to stash any changes
    Log.progress('Stashing any local changes')
    util.runGit(config.huhiCoreDir, ['stash'], true)
  }

  if (huhiCoreRef) {
    Log.progress(`Resetting huhi core to "${huhiCoreRef}"...`)
    // try to checkout to the right ref if possible
    util.runGit(config.huhiCoreDir, ['reset', '--hard', 'HEAD'], true)
    let checkoutResult = util.runGit(config.huhiCoreDir, ['checkout', huhiCoreRef], true)
    if (checkoutResult === null && program.create) {
      checkoutResult = util.runGit(config.huhiCoreDir, ['checkout', '-b', huhiCoreRef], true)
    }
    // Handle checkout failure
    if (checkoutResult === null) {
      Log.error('Could not checkout: ' + huhiCoreRef)
    }
    // Checkout was successful
    const huhiCoreSha = util.runGit(config.huhiCoreDir, ['rev-parse', 'HEAD'])
    Log.progress(`...huhi core is now at commit ID ${huhiCoreSha}`)
  }

  Log.progress('Running gclient sync...')
  const result = util.gclientSync(program.init || program.force, program.init, huhiCoreRef)
  const postSyncHuhiCoreRef = util.getGitReadableLocalRef(config.huhiCoreDir)
  Log.status(`Huhi Core is now at ${postSyncHuhiCoreRef || '[unknown]'}`)
...
Log.progress('Huhi Browser Sync starting')
RunCommand()
.then(() => {
  Log.progress('Huhi Browser Sync complete')
})
.catch((err) => {
  Log.error('Huhi Browser Sync ERROR:')
  console.error(err)
  process.exit(1)
})
