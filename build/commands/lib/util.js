...
const autoGeneratedHuhiToChromiumMapping = Object.assign({}, require('./l10nUtil').autoGeneratedHuhiToChromiumMapping)
...
async function applyPatches() {
  ...
  const coreRepoPath = config.huhiCoreDir
...
const util = {
  ...
  updateBranding: () => {
    ...
    const huhiComponentsDir = path.join(config.huhiCoreDir, 'components')
    const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
    const huhiAppDir = path.join(config.huhiCoreDir, 'app')
    const chromeBrowserResourcesDir = path.join(config.srcDir, 'chrome', 'browser', 'resources')
    const huhiBrowserResourcesDir = path.join(config.huhiCoreDir, 'browser', 'resources')
    const huhiAppVectorIconsDir = path.join(config.huhiCoreDir, 'vector_icons', 'chrome', 'app')
    const chromeAndroidJavaStringsTranslationsDir = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'strings', 'translations')
    const huhiAndroidJavaStringsTranslationsDir = path.join(config.huhiCoreDir, 'browser', 'ui', 'android', 'strings', 'translations')

    let fileMap = new Set();
    // The following 3 entries we map to the same name, not the chromium equivalent name for copying back
    autoGeneratedHuhiToChromiumMapping[path.join(huhiAppDir, 'huhi_strings.grd')] = path.join(chromeAppDir, 'huhi_strings.grd')
    autoGeneratedHuhiToChromiumMapping[path.join(huhiAppDir, 'settings_huhi_strings.grdp')] = path.join(chromeAppDir, 'settings_huhi_strings.grdp')
    autoGeneratedHuhiToChromiumMapping[path.join(huhiComponentsDir, 'components_huhi_strings.grd')] = path.join(chromeComponentsDir, 'components_huhi_strings.grd')

    Object.entries(autoGeneratedHuhiToChromiumMapping).forEach(mapping => fileMap.add(mapping))

    // Copy xtb files for:
    // huhi/app/resources/chromium_strings*.xtb
    // huhi/app/resources/generated_resoruces*.xtb
    // huhi/components/strings/components_chromium_strings*.xtb
    // huhi/browser/ui/android/strings/translations/android_chrome_strings*.xtb
    fileMap.add([path.join(huhiAppDir, 'resources'), path.join(chromeAppDir, 'resources')])
    fileMap.add([path.join(huhiComponentsDir, 'strings'), path.join(chromeComponentsDir, 'strings')])
    fileMap.add([huhiAndroidJavaStringsTranslationsDir, chromeAndroidJavaStringsTranslationsDir])
    // By overwriting, we don't need to modify some grd files.
    fileMap.add([path.join(huhiAppDir, 'theme', 'huhi'), path.join(chromeAppDir, 'theme', 'huhi')])
    fileMap.add([path.join(huhiAppDir, 'theme', 'huhi'), path.join(chromeAppDir, 'theme', 'chromium')])
    fileMap.add([path.join(huhiAppDir, 'theme', 'default_100_percent', 'huhi'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'huhi')])
    fileMap.add([path.join(huhiAppDir, 'theme', 'default_200_percent', 'huhi'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'huhi')])
    fileMap.add([path.join(huhiAppDir, 'theme', 'default_100_percent', 'huhi'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(huhiAppDir, 'theme', 'default_200_percent', 'huhi'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(huhiAppDir, 'theme', 'default_100_percent', 'common'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'common')])
    fileMap.add([path.join(huhiAppDir, 'theme', 'default_200_percent', 'common'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'common')])
    fileMap.add([path.join(huhiComponentsDir, 'resources', 'default_100_percent'), path.join(chromeComponentsDir, 'resources', 'default_100_percent')])
    fileMap.add([path.join(huhiComponentsDir, 'resources', 'default_100_percent', 'huhi'), path.join(chromeComponentsDir, 'resources', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(huhiComponentsDir, 'resources', 'default_200_percent'), path.join(chromeComponentsDir, 'resources', 'default_200_percent')])
    fileMap.add([path.join(huhiComponentsDir, 'resources', 'default_200_percent', 'huhi'), path.join(chromeComponentsDir, 'resources', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(huhiAppVectorIconsDir, 'vector_icons', 'huhi'), path.join(chromeAppDir, 'vector_icons', 'huhi')])
    // Copy chrome-logo-faded.png for replacing chrome logo of welcome page with huhi's on Win8.
    fileMap.add([path.join(huhiBrowserResourcesDir, 'chrome-logo-faded.png'), path.join(chromeBrowserResourcesDir, 'chrome-logo-faded.png')])
    fileMap.add([path.join(huhiBrowserResourcesDir, 'downloads', 'images', 'incognito_marker.svg'), path.join(chromeBrowserResourcesDir, 'downloads', 'images', 'incognito_marker.svg')])
    fileMap.add([path.join(huhiBrowserResourcesDir, 'settings', 'images'), path.join(chromeBrowserResourcesDir, 'settings', 'images')])
    // Copy to make our ${branding_path_component}_behaviors.cc
    fileMap.add([path.join(config.huhiCoreDir, 'chromium_src', 'chrome', 'installer', 'setup', 'huhi_behaviors.cc'),
                 path.join(config.srcDir, 'chrome', 'installer', 'setup', 'huhi_behaviors.cc')])
    ...
    if (process.platform === 'darwin') {
      ...
      // Each channel's app icons are stored in huhi/app/theme/$channel/app.icns.
      // With this copying, we don't need to modify chrome/BUILD.gn for this.
      const iconSource = path.join(huhiAppDir, 'theme', 'huhi', 'mac', config.channel, 'app.icns')
      const iconDest = path.join(chromeAppDir, 'theme', 'huhi', 'mac', 'app.icns')
      ...
      const brandingSource = path.join(huhiAppDir, 'theme', 'huhi', branding_file_name)
      const brandingDest = path.join(chromeAppDir, 'theme', 'huhi', 'BRANDING')
    ...
    if (config.targetOS === 'android') {
      ...
      if (config.channel === 'development') {
        androidIconSet = 'res_huhi_default'
      } else if (config.channel === '') {
        androidIconSet = 'res_huhi'
      } else if (config.channel === 'beta') {
        androidIconSet = 'res_huhi_beta'
      } else if (config.channel === 'dev') {
        androidIconSet = 'res_huhi_dev'
      } else if (config.channel === 'nightly') {
        androidIconSet = 'res_huhi_nightly'
      }
  ...
  updateOmahaMidlFiles: () => {
    ...
    const srcDir = path.join(config.huhiCoreDir, 'win_build_output', 'midl', 'google_update')
  ...
  // Latest version and download url are inserted to cdm header file and huhi-core refers it.
...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  generateWidevineSigFiles: () => {
    ...
    util.run('python', [sig_generator, '--input_file=' + path.join(src_dir, 'huhi.exe'),
        '--flags=1',
        '--certificate=' + cert,
        '--private_key=' + key,
        '--output_file=' + path.join(config.outputDir, 'huhi.exe.sig'),
  ...
  copyRedirectCC: () => {
    ...
    // from there to shorten the command line in huhi/script/redirect-cc.cmd
    console.log('Copying redirect-cc.py...')
    const src = path.join(config.huhiCoreDir, 'script', 'redirect-cc.py')
  ...
  generateXcodeWorkspace: (options = config.defaultOptions) => {
    ...
    const args = util.buildArgsToString(config.buildArgs())
    const genScript = path.join(config.huhiCoreDir, 'vendor', 'gn-project-generators', 'xcode.py')
  ...
  lint: (options = {}) => {
    ...
    cmd_options.cwd = config.huhiCoreDir
    cmd_options = mergeWithDefault(cmd_options)
    util.run('vpython', [path.join(config.huhiCoreDir, 'build', 'commands', 'scripts', 'lint.py'),
  ...
  gclientSync: (forceReset = false, cleanup = false, huhiCoreRef = null, options = {}) => {
    ...
    if (forceReset || huhiCoreRef) {
      if (!huhiCoreRef) {
        // Use current branch (sync will then pull latest) or current exact hash
        // if we're not in a branch.
        huhiCoreRef = util.runGit(config.huhiCoreDir, ['rev-parse', '--abbrev-ref', 'HEAD'])
        if (huhiCoreRef === 'HEAD') {
          // get the rev hash if we're not in a branch
          huhiCoreRef = util.runGit(config.huhiCoreDir, ['rev-parse', 'HEAD'])
        }
      }

      args = args.concat(['--revision', 'src/huhi@' + huhiCoreRef])
      reset = true
    }
    ...
    if (forceReset) {
      ...
      if (cleanup) {
        // temporarily ignored until we can figure out how not to delete src/huhi in the process
        // args = args.concat(['-D'])
      }
    }
