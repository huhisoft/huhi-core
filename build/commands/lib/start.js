...
const start = (passthroughArgs, buildConfig = config.defaultBuildConfig, options) => {
  ...
  if (options.no_sandbox) {
    huhiArgs.push('--no-sandbox')
  }
  if (options.disable_huhi_extension) {
    huhiArgs.push('--disable-huhi-extension')
  }
  if (options.disable_huhi_rewards_extension) {
    huhiArgs.push('--disable-huhi-rewards-extension')
  }
  if (options.disable_pdfjs_extension) {
    huhiArgs.push('--disable-pdfjs-extension')
  }
  if (options.disable_webtorrent_extension) {
    huhiArgs.push('--disable-webtorrent-extension')
  }
  if (options.ui_mode) {
    huhiArgs.push(`--ui-mode=${options.ui_mode}`)
  }
  if (!options.enable_huhi_update) {
    // This only has meaning with MacOS and official build.
    huhiArgs.push('--disable-huhi-update')
  }
  if (options.disable_doh) {
    huhiArgs.push('--disable-doh')
  }
  if (options.enable_smart_tracking_protection) {
    huhiArgs.push('--enable-smart-tracking-protection')
  }
  if (options.single_process) {
    huhiArgs.push('--single-process')
  }
  if (options.show_component_extensions) {
    huhiArgs.push('--show-component-extension-options')
  }
  if (options.rewards) {
    huhiArgs.push(`--rewards=${options.rewards}`)
  }
  if (options.huhi_ads_testing) {
    huhiArgs.push('--huhi-ads-testing')
  }
  if (options.huhi_ads_debug) {
    huhiArgs.push('--huhi-ads-debug')
  }
  if (options.huhi_ads_production) {
    huhiArgs.push('--huhi-ads-production')
  }
  if (options.huhi_ads_staging) {
    huhiArgs.push('--huhi-ads-staging')
  }
  huhiArgs = huhiArgs.concat(passthroughArgs)

  let user_data_dir
  if (options.user_data_dir_name) {
    if (process.platform === 'darwin') {
      user_data_dir = path.join(process.env.HOME, 'Library', 'Application\\ Support', 'HuhiSoftware', options.user_data_dir_name)
    } else if (process.platform === 'win32') {
      user_data_dir = path.join(process.env.LocalAppData, 'HuhiSoftware', options.user_data_dir_name)
    } else {
      user_data_dir = path.join(process.env.HOME, '.config', 'HuhiSoftware', options.user_data_dir_name)
    }
    huhiArgs.push('--user-data-dir=' + user_data_dir);
  }
  const networkLogFile = path.resolve(path.join(config.rootDir, 'network_log.json'))
  if (options.network_log) {
    huhiArgs.push(`--log-net-log=${networkLogFile}`)
    huhiArgs.push(`--net-log-capture-mode=Everything`)
    if (user_data_dir) {
  ...
  if (options.network_log) {
    console.log('Network audit started. Logging requests for the next 2min or until you quit Huhi...')
  }

  let outputPath = options.output_path
  if (!outputPath) {
    outputPath = path.join(config.outputDir, 'huhi')
  ...
  util.run(outputPath, huhiArgs, cmdOptions)
