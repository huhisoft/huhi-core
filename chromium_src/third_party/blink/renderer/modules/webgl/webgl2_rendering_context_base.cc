...
ScriptValue FarbleGLIntParameter(WebGL2RenderingContextBase* owner,
  ...
  if (value > 0) {
    std::mt19937_64 prng =
        huhi::HuhiSessionCache::From(*ExecutionContext::From(script_state))
...
#define HUHI_WEBGL2_RENDERING_CONTEXT_BASE_GETPARAMETER                      \
  if (ExecutionContext* context = ExecutionContext::From(script_state)) {     \
    if (WebContentSettingsClient* settings =                                  \
      ...
      if (settings->GetHuhiFarblingLevel() == HuhiFarblingLevel::MAXIMUM) { \
        switch (pname) {                                                      \
      ...
      } else if (settings->GetHuhiFarblingLevel() ==                         \
                 HuhiFarblingLevel::BALANCED) {                              \
        switch (pname) {                                                      \
          case GL_MAX_VERTEX_UNIFORM_COMPONENTS:                              \
...
#undef HUHI_WEBGL2_RENDERING_CONTEXT_BASE_GETPARAMETER
