...
#include "third_party/blink/renderer/modules/storage/huhi_dom_window_storage.h"
...
// static
const char HuhiDOMWindowStorage::kSupplementName[] = "HuhiDOMWindowStorage";

// static
HuhiDOMWindowStorage& HuhiDOMWindowStorage::From(LocalDOMWindow& window) {
  HuhiDOMWindowStorage* supplement =
      Supplement<LocalDOMWindow>::From<HuhiDOMWindowStorage>(window);
  if (!supplement) {
    supplement = MakeGarbageCollected<HuhiDOMWindowStorage>(window);
...
// static
StorageArea* HuhiDOMWindowStorage::sessionStorage(
    LocalDOMWindow& window,
    ExceptionState& exception_state) {
...
// static
StorageArea* HuhiDOMWindowStorage::localStorage(
    LocalDOMWindow& window,
    ExceptionState& exception_state) {
  return From(window).localStorage(exception_state);
}

HuhiDOMWindowStorage::HuhiDOMWindowStorage(LocalDOMWindow& window)
...
StorageArea* HuhiDOMWindowStorage::sessionStorage(
  ...
  if (!base::FeatureList::IsEnabled(features::kHuhiEphemeralStorage))
    return storage;
...
StorageArea* HuhiDOMWindowStorage::ephemeralSessionStorage() {
...
StorageArea* HuhiDOMWindowStorage::ephemeralLocalStorage() {
...
void HuhiDOMWindowStorage::Trace(Visitor* visitor) const {
