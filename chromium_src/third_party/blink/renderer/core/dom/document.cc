...
HuhiSessionCache::HuhiSessionCache(Document& document)
...
HuhiSessionCache& HuhiSessionCache::From(Document& document) {
  HuhiSessionCache* cache =
      Supplement<Document>::From<HuhiSessionCache>(document);
  if (!cache) {
    cache = MakeGarbageCollected<HuhiSessionCache>(document);
