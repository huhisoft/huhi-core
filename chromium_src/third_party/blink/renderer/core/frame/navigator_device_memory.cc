...
float FarbleDeviceMemory(blink::ExecutionContext* context) {
  ...
  // If Huhi Shields are down or anti-fingerprinting is off for this site,
  // return the true value.
  if (!settings || settings->GetHuhiFarblingLevel() == HuhiFarblingLevel::OFF)
    return true_value;
...
float NavigatorDeviceMemory::deviceMemory(ScriptState* script_state) const {
  ...
  return huhi::FarbleDeviceMemory(context);
}
