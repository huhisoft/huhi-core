...
const char kHuhiSessionToken[] = "huhi_session_token";
const char HuhiSessionCache::kSupplementName[] = "HuhiSessionCache";
...
HuhiSessionCache::HuhiSessionCache(ExecutionContext& context)
  ...
  DCHECK(cmd_line->HasSwitch(kHuhiSessionToken));
  base::StringToUint64(cmd_line->GetSwitchValueASCII(kHuhiSessionToken),
                       &session_key_);
...
HuhiSessionCache& HuhiSessionCache::From(ExecutionContext& context) {
  HuhiSessionCache* cache =
      Supplement<ExecutionContext>::From<HuhiSessionCache>(context);
  if (!cache) {
    cache = MakeGarbageCollected<HuhiSessionCache>(context);
    ProvideTo(context, cache);
  }
  return *cache;
}

AudioFarblingCallback HuhiSessionCache::GetAudioFarblingCallback(
...
scoped_refptr<blink::StaticBitmapImage> HuhiSessionCache::PerturbPixels(
    blink::WebContentSettingsClient* settings,
...
HuhiSessionCache::PerturbPixelsInternal(
    scoped_refptr<blink::StaticBitmapImage> image_bitmap) {
  DCHECK(image_bitmap);
...
WTF::String HuhiSessionCache::GenerateRandomString(std::string seed,
                                                    wtf_size_t length) {
  uint8_t key[32];
  crypto::HMAC h(crypto::HMAC::SHA256);
...
WTF::String HuhiSessionCache::FarbledUserAgent(WTF::String real_user_agent) {
...
std::mt19937_64 HuhiSessionCache::MakePseudoRandomGenerator() {
  uint64_t seed = *reinterpret_cast<uint64_t*>(domain_key_);
