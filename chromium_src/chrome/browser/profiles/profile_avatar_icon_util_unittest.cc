...
TEST(ProfileUtilTest, HuhiAvatarIconChoices) {
  // Test that the avatar icon choices presented to the user
  // are huhi's and include the placeholder so that
  ...
  const size_t expected_selectable_avatar_count =
      profiles::kHuhiDefaultAvatarIconsCount + 1;
  const size_t actual_selectable_avatar_count = avatars->GetSize();

  // Avatars are Huhi's, not Chromium's
...
TEST(ProfileUtilTest, RandomIconNeverFirstIcon) {
  // Test that for Huhi, a call to get a random
  ...
  // the placeholder index (which for huhi is always the first item).
