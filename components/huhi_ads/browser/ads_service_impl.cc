...
#include "huhi/components/huhi_ads/browser/ad_notification.h"
...
#include "huhi/components/services/bat_ads/public/cpp/ads_client_mojo_bridge.h"
...
#if defined(OS_ANDROID)
#include "huhi/browser/notifications/huhi_notification_platform_bridge_helper_android.h"
...
using huhi_rewards::RewardsNotificationService;
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("ads_service_impl", R"(
      semantics {
        sender: "Huhi Ads Service"
        description:
          "This service is used to communicate with Huhi servers "
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable this feature by visiting huhi://rewards."
...
void AdsServiceImpl::ResetState() {
  ...
  profile_->GetPrefs()->ClearPrefsWithPrefixSilently("huhi.huhi_ads");
...
#if defined(OS_ANDROID)
  if (GetBooleanPref(huhi_rewards::prefs::kUseRewardsStagingServer)) {
    environment = ads::Environment::STAGING;
  }
#else
...
void AdsServiceImpl::SetBuildChannel() {
  ...
  build_channel->name = huhi::GetChannelName();
...
void AdsServiceImpl::OpenNewTabWithUrl(
  ...
  if (g_huhi_browser_process->IsShuttingDown()) {
    return;
  }
...
void AdsServiceImpl::RegisterUserModelComponentsForLocale(
    const std::string& locale) {
  g_huhi_browser_process->user_model_file_service()->
...
void AdsServiceImpl::MigratePrefs() {
  is_upgrading_from_pre_huhi_ads_build_ = IsUpgradingFromPreHuhiAdsBuild();
  if (is_upgrading_from_pre_huhi_ads_build_) {
    VLOG(1) << "Migrating ads preferences from pre Huhi Ads build";

    // Force migration of preferences from version 1 if
    // |is_upgrading_from_pre_huhi_ads_build_| is set to |true| to fix
    // "https://github.com/huhisoft/huhi-browser/issues/5434"
...
void AdsServiceImpl::MigratePrefsVersion2To3() {
  ...
  const auto country_code = huhi_l10n::GetCountryCode(locale);

  // Disable ads if upgrading from a pre huhi ads build due to a bug where ads
  // were always enabled
  DisableAdsIfUpgradingFromPreHuhiAdsBuild();
...
void AdsServiceImpl::MigratePrefsVersion6To7() {
  ...
  if (is_a_legacy_country_code) {
    // Do not disable Huhi Ads for legacy country codes introduced before
    // version 1.3.x
    return;
  }
  ...
  if (last_schema_version >= 4) {
    // Do not disable Huhi Ads if |kSupportedCountryCodesLastSchemaVersion|
...
bool AdsServiceImpl::IsUpgradingFromPreHuhiAdsBuild() {
  // Huhi ads was hidden in 0.62.x however due to a bug |prefs::kEnabled| was
  // set to |true| causing "https://github.com/huhisoft/huhi-browser/issues/5434"

  // |prefs::kIdleThreshold| was not serialized in 0.62.x

  // |prefs::kVersion| was introduced in 0.63.x

  // We can detect if we are upgrading from a pre Huhi ads build by checking
...
void AdsServiceImpl::DisableAdsIfUpgradingFromPreHuhiAdsBuild() {
  if (!is_upgrading_from_pre_huhi_ads_build_) {
    return;
  }
...
std::string AdsServiceImpl::GetLocale() const {
  return huhi_l10n::LocaleHelper::GetInstance()->GetLocale();
}
...
void AdsServiceImpl::CloseNotification(
    const std::string& uuid) {
#if defined(OS_ANDROID)
  const std::string huhi_ads_url_prefix = kHuhiAdsUrlPrefix;
  const GURL service_worker_scope =
      GURL(huhi_ads_url_prefix.substr(0, huhi_ads_url_prefix.size() - 1));
  HuhiNotificationPlatformBridgeHelperAndroid::MaybeRegenerateNotification(
      uuid, service_worker_scope);
#endif
  display_service_->Close(NotificationHandler::Type::HUHI_ADS, uuid);
}
...
void AdsServiceImpl::LoadUserModelForId(
  ...
  const base::Optional<base::FilePath> path =
      g_huhi_browser_process->user_model_file_service()->GetPathForId(id);
