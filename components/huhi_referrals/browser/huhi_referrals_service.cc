...
std::string BuildReferralEndpoint(const std::string& path) {
  ...
  env->GetVar("HUHI_REFERRALS_SERVER", &referral_server);
  if (referral_server.empty())
    referral_server = kHuhiReferralsServer;
  if (env->HasVar("HUHI_REFERRALS_LOCAL"))
    proto = "http";
...
HuhiReferralsService::HuhiReferralsService(PrefService* pref_service,
...
HuhiReferralsService::~HuhiReferralsService() {
}

void HuhiReferralsService::Start() {
  if (initialized_)
    return;
  ...
  finalization_checks_timer_->Start(
      ...
      base::TimeDelta::FromSeconds(
          huhi_base::random::Geometric(kFinalizationChecksFrequency)),
      this, &HuhiReferralsService::OnFinalizationChecksTimerFired);
  ...
  fetch_referral_headers_timer_->Start(
      FROM_HERE,
      base::TimeDelta::FromSeconds(
        huhi_base::random::Geometric(kFetchReferralHeadersFrequency)),
      this, &HuhiReferralsService::OnFetchReferralHeadersTimerFired);
  ...
  if (!checked_for_promo_code_file && !has_initialized && download_id.empty())
    task_runner_->PostTaskAndReply(
        FROM_HERE,
        base::Bind(&HuhiReferralsService::ReadPromoCode,
                   base::Unretained(this)),
        base::Bind(&HuhiReferralsService::OnReadPromoCodeComplete,
                   weak_factory_.GetWeakPtr()));

  initialized_ = true;
}

void HuhiReferralsService::Stop() {
...
void HuhiReferralsService::SetReferralInitializedCallbackForTest(
...
// static
bool HuhiReferralsService::IsDefaultReferralCode(const std::string& code) {
  return code == kDefaultPromoCode;
}

// static
bool HuhiReferralsService::GetMatchingReferralHeaders(
...
void HuhiReferralsService::OnFinalizationChecksTimerFired() {
  PerformFinalizationChecks();
}

void HuhiReferralsService::OnFetchReferralHeadersTimerFired() {
  FetchReferralHeaders();
}

void HuhiReferralsService::OnReferralHeadersLoadComplete(
...
void HuhiReferralsService::OnReferralInitLoadComplete(
    std::unique_ptr<std::string> response_body) {
  int response_code = -1;
  ...
  if (referral_init_loader_->NetError() != net::OK || response_code < 200 ||
    ...
    initialization_timer_->Start(
                    FROM_HERE,
                    base::TimeDelta::FromSeconds(
                      huhi_base::random::Geometric(
                              kReportInitializationFrequency)),
                    this, &HuhiReferralsService::InitReferral);
  ...
  task_runner_->PostTask(FROM_HERE,
                         base::Bind(&HuhiReferralsService::DeletePromoCodeFile,
                                    base::Unretained(this)));
}

void HuhiReferralsService::OnReferralFinalizationCheckLoadComplete(
    std::unique_ptr<std::string> response_body) {
  int response_code = -1;
...
void HuhiReferralsService::OnReadPromoCodeComplete() {
...
void HuhiReferralsService::GetFirstRunTime() {
#if defined(OS_ANDROID)
  ...
  task_runner_->PostTask(
      FROM_HERE, base::Bind(&HuhiReferralsService::GetFirstRunTimeDesktop,
                            base::Unretained(this)));
#endif
}

void HuhiReferralsService::GetFirstRunTimeDesktop() {
#if !defined(OS_ANDROID)
...
void HuhiReferralsService::PerformFinalizationChecks() {
  // Delete the promo code preference, if appropriate.
  base::PostTask(
      FROM_HERE, {content::BrowserThread::UI},
      base::BindOnce(&HuhiReferralsService::MaybeDeletePromoCodePref,
                     base::Unretained(this)));

  // Check for referral finalization, if appropriate.
  base::PostTask(
      FROM_HERE, {content::BrowserThread::UI},
      base::BindOnce(&HuhiReferralsService::MaybeCheckForReferralFinalization,
                     base::Unretained(this)));
}

base::FilePath HuhiReferralsService::GetPromoCodeFileName() const {
  base::FilePath user_data_dir;
  base::PathService::Get(chrome::DIR_USER_DATA, &user_data_dir);
  return user_data_dir.AppendASCII("promoCode");
}

void HuhiReferralsService::ReadPromoCode() {
...
void HuhiReferralsService::DeletePromoCodeFile() const {
  base::FilePath promo_code_file = GetPromoCodeFileName();
...
void HuhiReferralsService::MaybeCheckForReferralFinalization() {
  ...
  env->GetVar("HUHI_REFERRALS_CHECK_TIME", &check_time_str);
...
void HuhiReferralsService::MaybeDeletePromoCodePref() const {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  uint64_t delete_time = 90 * 24 * 60 * 60;
  std::unique_ptr<base::Environment> env(base::Environment::Create());
  std::string delete_time_str;
  env->GetVar("HUHI_REFERRALS_DELETE_TIME", &delete_time_str);
...
std::string HuhiReferralsService::BuildReferralInitPayload() const {
...
std::string HuhiReferralsService::BuildReferralFinalizationCheckPayload()
    const {
  base::Value root(base::Value::Type::DICTIONARY);
  root.SetKey("api_key", base::Value(api_key_));
...
void HuhiReferralsService::FetchReferralHeaders() {
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation(
        "huhi_referral_headers_fetcher", R"(
        semantics {
          sender:
            "Huhi Referrals Service"
          description:
            "Fetches referral headers from Huhi."
          trigger:
            "An update timer indicates that it's time to fetch referral headers."
          data: "Huhi referral headers."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->url =
      GURL(BuildReferralEndpoint(kHuhiReferralsHeadersPath));
  ...
  referral_headers_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(&HuhiReferralsService::OnReferralHeadersLoadComplete,
                     base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

void HuhiReferralsService::InitReferral() {
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("huhi_referral_initializer", R"(
        semantics {
          sender:
            "Huhi Referrals Service"
          description:
            "Validates the current referral offer with Huhi, potentially "
            "unlocking special features and/or services."
          trigger:
            "On startup, sends the current referral code to Huhi."
          data: "Huhi referral metadata."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->url = GURL(BuildReferralEndpoint(kHuhiReferralsInitPath));
  resource_request->load_flags =
      net::LOAD_DO_NOT_SEND_COOKIES | net::LOAD_DO_NOT_SAVE_COOKIES |
      net::LOAD_BYPASS_CACHE | net::LOAD_DISABLE_CACHE |
      net::LOAD_DO_NOT_SEND_AUTH_DATA;
  ...
  referral_init_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(&HuhiReferralsService::OnReferralInitLoadComplete,
                     base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

#if defined(OS_ANDROID)
void HuhiReferralsService::GetSafetynetStatusResult(
...
void HuhiReferralsService::CheckForReferralFinalization() {
#if defined(OS_ANDROID)
  if (pref_service_->GetString(kSafetynetStatus).empty()) {
    // Get safetynet status before finalization
    safetynet_check::ClientAttestationCallback attest_callback =
        base::BindOnce(&HuhiReferralsService::GetSafetynetStatusResult,
                       weak_factory_.GetWeakPtr());
...
#endif
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("huhi_referral_finalization_checker",
        R"(
        semantics {
          sender:
            "Huhi Referrals Service"
          description:
            "Fetches referral finalization data from Huhi."
          trigger:
            ""
          data: "Huhi referral finalization status."
          destination: WEBSITE
        }
        policy {
          cookies_allowed: NO
  ...
  resource_request->method = "PUT";
  resource_request->url =
      GURL(BuildReferralEndpoint(kHuhiReferralsActivityPath));
  resource_request->load_flags =
      net::LOAD_DO_NOT_SEND_COOKIES | net::LOAD_DO_NOT_SAVE_COOKIES |
      net::LOAD_BYPASS_CACHE | net::LOAD_DISABLE_CACHE |
      net::LOAD_DO_NOT_SEND_AUTH_DATA;
  ...
  referral_finalization_check_loader_->DownloadToString(
      loader_factory,
      base::BindOnce(
          &HuhiReferralsService::OnReferralFinalizationCheckLoadComplete,
          base::Unretained(this)),
      kMaxReferralServerResponseSizeBytes);
}

std::string HuhiReferralsService::FormatExtraHeaders(
...
void RegisterPrefsForHuhiReferralsService(PrefRegistrySimple* registry) {
