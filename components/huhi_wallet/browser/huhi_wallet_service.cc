...
#include "huhi/components/huhi_wallet/browser/huhi_wallet_delegate.h"
...
HuhiWalletService::HuhiWalletService(content::BrowserContext* context,
  ...
  pref_change_registrar_->Add(kHuhiWalletWeb3Provider,
...
// See https://github.com/huhisoft/huhi-browser/wiki/Huhi-Ethereum-Remote-Client-Wallet-Seed-Information#note-on-salts
// static
std::string
...
bool HuhiWalletService::LoadFromPrefs(PrefService* prefs,
  ...
  if (!base::Base64Decode(
          prefs->GetString(kHuhiWalletAES256GCMSivNonce),
          nonce)) {
    return false;
  }
  if (!base::Base64Decode(
          prefs->GetString(kHuhiWalletEncryptedSeed),
          cipher_seed)) {
    return false;
  }
  return true;
...
// static
void HuhiWalletService::SaveToPrefs(
    PrefService* prefs,
  ...
  prefs->SetString(kHuhiWalletEncryptedSeed,
      base64_cipher_seed);
}
...
bool HuhiWalletService::LoadRootSeedInfo(std::vector<uint8_t> key,
  ...
  if (HuhiWalletService::LoadFromPrefs(
          prefs, &cipher_seed, &nonce)) {
    // Decrypt the existing seed.
    if (!HuhiWalletService::OpenSeed(
...
// https://github.com/huhisoft/huhi-browser/wiki/Huhi-Ethereum-Remote-Client-Wallet-Seed-Information
std::string HuhiWalletService::GetWalletSeed(
    std::vector<uint8_t> key) {
  std::string seed;
  if (!LoadRootSeedInfo(key, &seed)) {
    return "";
  }
  return HuhiWalletService::GetEthereumRemoteClientSeedFromRootSeed(
      seed);
}
...
// For more information, see:
// https://github.com/huhisoft/huhi-browser/wiki/Huhi-Ethereum-Remote-Client-Wallet-Seed-Information
std::string HuhiWalletService::GetBitGoSeed(
    std::vector<uint8_t> key) {
  std::string seed;
  if (!LoadRootSeedInfo(key, &seed)) {
    return "";
