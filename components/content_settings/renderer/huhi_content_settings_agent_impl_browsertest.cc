...
class HuhiContentSettingsAgentImplBrowserTest : public InProcessBrowserTest {
 public:
  HuhiContentSettingsAgentImplBrowserTest() :
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_.RegisterRequestMonitor(base::BindRepeating(
        &HuhiContentSettingsAgentImplBrowserTest::SaveReferrer,
  ...
  void BlockReferrers() {
    content_settings()->SetContentSettingCustomScope(
        ...
        ContentSettingsType::PLUGINS, huhi_shields::kReferrers,
  ...
  void Block3PCookies() {
    huhi_shields::SetCookieControlType(content_settings(),
  ...
  void ShieldsDown() {
    huhi_shields::SetHuhiShieldsEnabled(content_settings(),
                                          false,
                                          top_level_page_url());
  }
  ...
  void BlockThirdPartyFingerprinting() {
    huhi_shields::SetFingerprintingControlType(content_settings(),
                                                ControlType::BLOCK_THIRD_PARTY,
                                                top_level_page_url());
  }
...
class HuhiContentSettingsAgentImplV2BrowserTest
    : public HuhiContentSettingsAgentImplBrowserTest {
 public:
  void SetUp() override { HuhiContentSettingsAgentImplBrowserTest::SetUp(); }

 private:
  base::test::ScopedFeatureList scoped_feature_list_;
};

IN_PROC_BROWSER_TEST_F(HuhiContentSettingsAgentImplV2BrowserTest,
                       WebGLReadPixels) {
...
IN_PROC_BROWSER_TEST_F(HuhiContentSettingsAgentImplV2BrowserTest,
                       FarbleGetImageData) {
  // Farbling should be default when kHuhiFingerprintingV2 is enabled
  ...
  // settings and kHuhiFingerprintingV2 is enabled
  BlockFingerprinting();
  NavigateToPageWithIframe();
  hash = -1;
  EXPECT_TRUE(
  ...
  // content settings and kHuhiFingerprintingV2 is enabled
  SetFingerprintingDefault();
  NavigateToPageWithIframe();
  hash = -1;
  EXPECT_TRUE(
...
IN_PROC_BROWSER_TEST_F(HuhiContentSettingsAgentImplBrowserTest, BlockCookies) {
  BlockCookies();
...
IN_PROC_BROWSER_TEST_F(HuhiContentSettingsAgentImplBrowserTest, AllowCookies) {
  AllowCookies();
...
IN_PROC_BROWSER_TEST_F(HuhiContentSettingsAgentImplBrowserTest,
                       ChromiumCookieBlockOverridesHuhiAllowCookiesTopLevel) {
  AllowCookies();
  HostContentSettingsMap* content_settings =
...
IN_PROC_BROWSER_TEST_F(HuhiContentSettingsAgentImplBrowserTest,
                       ChromiumCookieBlockOverridesHuhiAllowCookiesIframe) {
  AllowCookies();
  HostContentSettingsMap* content_settings =
      HostContentSettingsMapFactory::GetForProfile(browser()->profile());
  content_settings->SetContentSettingCustomScope(
...
IN_PROC_BROWSER_TEST_F(HuhiContentSettingsAgentImplBrowserTest, BlockScripts) {
  BlockScripts();
...
IN_PROC_BROWSER_TEST_F(HuhiContentSettingsAgentImplBrowserTest, AllowScripts) {
  AllowScripts();
