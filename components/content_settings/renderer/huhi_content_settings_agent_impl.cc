...
#include "huhi/components/content_settings/renderer/huhi_content_settings_agent_impl.h"
...
bool IsHuhiShieldsDown(const blink::WebFrame* frame,
...
bool HuhiContentSettingsAgentImpl::OnMessageReceived(
  ...
  bool handled = true;
  IPC_BEGIN_MESSAGE_MAP(HuhiContentSettingsAgentImpl, message)
    IPC_MESSAGE_HANDLER(HuhiFrameMsg_AllowScriptsOnce, OnAllowScriptsOnce)
...
void HuhiContentSettingsAgentImpl::OnAllowScriptsOnce(
    const std::vector<std::string>& origins) {
  preloaded_temporarily_allowed_scripts_ = std::move(origins);
}

void HuhiContentSettingsAgentImpl::DidCommitProvisionalLoad(
...
bool HuhiContentSettingsAgentImpl::IsScriptTemporilyAllowed(
...
bool HuhiContentSettingsAgentImpl::AllowScriptFromSource(
    bool enabled_per_settings,
...
void HuhiContentSettingsAgentImpl::DidBlockFingerprinting(
    const base::string16& details) {
  Send(new HuhiViewHostMsg_FingerprintingBlocked(routing_id(), details));
}

bool HuhiContentSettingsAgentImpl::IsHuhiShieldsDown(
    const blink::WebFrame* frame,
    const GURL& secondary_url) {
  return !content_setting_rules_ ||
         ::content_settings::IsHuhiShieldsDown(
             frame, secondary_url, content_setting_rules_->huhi_shields_rules);
}

bool HuhiContentSettingsAgentImpl::AllowFingerprinting(
    bool enabled_per_settings) {
...
HuhiFarblingLevel HuhiContentSettingsAgentImpl::GetHuhiFarblingLevel() {
  blink::WebLocalFrame* frame = render_frame()->GetWebFrame();
