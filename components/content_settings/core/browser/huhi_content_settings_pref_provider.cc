...
Rule CloneRule(const Rule& rule, bool reverse_patterns = false) {
  // huhi plugin rules incorrectly use first party url as primary
  ...
  if (primary_pattern ==
    ...
    DCHECK(reverse_patterns);  // we should only hit this for huhi plugin rules
...
class HuhiShieldsRuleIterator : public RuleIterator {
 public:
  explicit HuhiShieldsRuleIterator(std::vector<Rule> rules)
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(HuhiShieldsRuleIterator);
};
...
HuhiPrefProvider::HuhiPrefProvider(PrefService* prefs,
                                     bool off_the_record,
    ...
    : PrefProvider(prefs, off_the_record, store_last_modified, restore_session),
      initialized_(false),
      weak_factory_(this) {
  huhi_pref_change_registrar_.Init(prefs_);
  huhi_pref_change_registrar_.Add(
      kGoogleLoginControlType,
      base::BindRepeating(&HuhiPrefProvider::OnCookiePrefsChanged,
                          base::Unretained(this)));

  WebsiteSettingsRegistry* website_settings =
      WebsiteSettingsRegistry::GetInstance();
  // Makes HuhiPrefProvder handle plugin type.
  for (const WebsiteSettingsInfo* info : *website_settings) {
    if (info->type() == ContentSettingsType::PLUGINS) {
      content_settings_prefs_.insert(std::make_pair(
          info->type(),
          std::make_unique<ContentSettingsPref>(
              info->type(), prefs_, &huhi_pref_change_registrar_,
...
HuhiPrefProvider::~HuhiPrefProvider() {}

void HuhiPrefProvider::ShutdownOnUIThread() {
  RemoveObserver(this);
  huhi_pref_change_registrar_.RemoveAll();
  PrefProvider::ShutdownOnUIThread();
}

// static
void HuhiPrefProvider::RegisterProfilePrefs(
    user_prefs::PrefRegistrySyncable* registry) {
  PrefProvider::RegisterProfilePrefs(registry);
  // Register shields settings migration pref.
  registry->RegisterIntegerPref(kHuhiShieldsSettingsVersion, 1);
}

void HuhiPrefProvider::MigrateShieldsSettings(bool incognito) {
...
void HuhiPrefProvider::MigrateShieldsSettingsV1ToV2() {
  // Check if migration is needed.
  if (prefs_->GetInteger(kHuhiShieldsSettingsVersion) != 1)
    return;
  ...
  prefs_->SetInteger(kHuhiShieldsSettingsVersion, 2);
}

void HuhiPrefProvider::MigrateShieldsSettingsV1ToV2ForOneType(
...
bool HuhiPrefProvider::SetWebsiteSetting(
  ...
  // handle changes to huhi cookie settings from chromium cookie settings UI
  if (content_type == ContentSettingsType::COOKIES) {
    auto* value = in_value.get();
    auto match = std::find_if(
        huhi_cookie_rules_[off_the_record_].begin(),
        huhi_cookie_rules_[off_the_record_].end(),
    ...
    if (match != huhi_cookie_rules_[off_the_record_].end()) {
      ...
      // convert to legacy firstParty format for huhi plugin settings
...
std::unique_ptr<RuleIterator> HuhiPrefProvider::GetRuleIterator(
      ContentSettingsType content_type,
      const ResourceIdentifier& resource_identifier,
  ...
  if (content_type == ContentSettingsType::COOKIES) {
    ...
    return std::make_unique<HuhiShieldsRuleIterator>(std::move(rules));
  }
...
void HuhiPrefProvider::UpdateCookieRules(ContentSettingsType content_type,
                                          bool incognito) {
  auto& rules = cookie_rules_[incognito];
  auto old_rules = std::move(huhi_cookie_rules_[incognito]);

  rules.clear();
  huhi_cookie_rules_[incognito].clear();
  ...
  if (prefs_->GetBoolean(kGoogleLoginControlType)) {
    ...
    huhi_cookie_rules_[incognito].emplace_back(CloneRule(google_auth_rule));
    ...
    huhi_cookie_rules_[incognito].emplace_back(CloneRule(firebase_rule));
  }
  ...
  auto huhi_shields_iterator = PrefProvider::GetRuleIterator(
  ...
  while (huhi_shields_iterator && huhi_shields_iterator->HasNext()) {
    shield_rules.emplace_back(CloneRule(huhi_shields_iterator->Next()));
  }

  huhi_shields_iterator.reset();

  // add huhi cookies after checking shield status
  auto huhi_cookies_iterator = PrefProvider::GetRuleIterator(
      ContentSettingsType::PLUGINS,
      huhi_shields::kCookies,
      incognito);

  // Matching cookie rules against shield rules.
  while (huhi_cookies_iterator && huhi_cookies_iterator->HasNext()) {
    auto rule = huhi_cookies_iterator->Next();
    if (IsActive(rule, shield_rules)) {
      rules.emplace_back(CloneRule(rule, true));
      huhi_cookie_rules_[incognito].emplace_back(CloneRule(rule, true));
    }
  }
      ...
      rules.emplace_back(
          Rule(ContentSettingsPattern::Wildcard(),
               ...
               base::Value::FromUniquePtrValue(
                   ContentSettingToValue(CONTENT_SETTING_ALLOW)),
               base::Time(), SessionModel::Durable));
      huhi_cookie_rules_[incognito].emplace_back(
          Rule(ContentSettingsPattern::Wildcard(),
               shield_rule.primary_pattern,
               base::Value::FromUniquePtrValue(
                   ContentSettingToValue(CONTENT_SETTING_ALLOW)),
  ...
  std::vector<Rule> huhi_cookie_updates;
  for (const auto& new_rule : huhi_cookie_rules_[incognito]) {
    auto match = std::find_if(
        old_rules.begin(),
        old_rules.end(),
    ...
    if (match == old_rules.end()) {
      huhi_cookie_updates.emplace_back(CloneRule(new_rule));
    }
  }
  ...
  for (const auto& old_rule : old_rules) {
    auto match = std::find_if(
        huhi_cookie_rules_[incognito].begin(),
        huhi_cookie_rules_[incognito].end(),
    ...
    if (match == huhi_cookie_rules_[incognito].end()) {
      huhi_cookie_updates.emplace_back(
          Rule(old_rule.primary_pattern, old_rule.secondary_pattern,
               base::Value(), old_rule.expiration, old_rule.session_model));
    }
  }

  // Notify huhi cookie changes as ContentSettingsType::COOKIES
  if (initialized_ && content_type == ContentSettingsType::PLUGINS) {
    ...
    base::PostTask(
        ...
        base::BindOnce(&HuhiPrefProvider::NotifyChanges,
                       weak_factory_.GetWeakPtr(),
                       std::move(huhi_cookie_updates), incognito));
  }
}

void HuhiPrefProvider::NotifyChanges(const std::vector<Rule>& rules,
                                      bool incognito) {
...
void HuhiPrefProvider::OnCookiePrefsChanged(
    const std::string& pref) {
  OnCookieSettingsChanged(ContentSettingsType::PLUGINS);
}

void HuhiPrefProvider::OnCookieSettingsChanged(
    ContentSettingsType content_type) {
  UpdateCookieRules(content_type, true);
  UpdateCookieRules(content_type, false);
}

void HuhiPrefProvider::OnContentSettingChanged(
    const ContentSettingsPattern& primary_pattern,
    const ContentSettingsPattern& secondary_pattern,
    ContentSettingsType content_type,
  ...
  if (content_type == ContentSettingsType::COOKIES ||
      (content_type == ContentSettingsType::PLUGINS &&
          (resource_identifier == huhi_shields::kCookies ||
           resource_identifier == huhi_shields::kHuhiShields))) {
