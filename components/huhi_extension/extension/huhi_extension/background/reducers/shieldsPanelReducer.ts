  ...
  switch (action.type) {
    ...
    case shieldsPanelTypes.SHIELDS_PANEL_DATA_UPDATED: {
      ...
      if (chrome.test && shieldsPanelState.getActiveTabData(state)) {
        chrome.test.sendMessage('huhi-extension-shields-data-ready')
      }
      break
    }
    case shieldsPanelTypes.SHIELDS_TOGGLED: {
      ...
      setAllowHuhiShields(tabData.origin, action.setting)
        .then(() => {
      ...
      state = shieldsPanelState
        .updateTabShieldsData(state, tabId, { huhiShields: action.setting })
      break
    }
    ...
    case shieldsPanelTypes.SHIELDS_READY: {
      onShieldsPanelShown().catch(() => {
        console.error('error calling `chrome.huhiShields.onShieldsPanelShown()`')
      })
      break
    }
    ...
    case shieldsPanelTypes.CONTENT_SCRIPTS_LOADED: {
      ...
      Promise.all([chrome.huhiShields.shouldDoCosmeticFilteringAsync(action.url), chrome.huhiShields.isFirstPartyCosmeticFilteringEnabledAsync(action.url)])
