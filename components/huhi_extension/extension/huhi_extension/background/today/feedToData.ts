...
function convertFeedItem (item: HuhiToday.FeedItem): HuhiToday.FeedItem {
...
export default async function getHuhiTodayData (
  feedContent: HuhiToday.FeedItem[],
  enabledPublishers: HuhiToday.Publishers
): Promise<HuhiToday.Feed | undefined> {
  // No sponsor content yet, wait until we have content spec
  // const sponsors: (HuhiToday.Article)[] = []
  const deals: (HuhiToday.Deal)[] = []
  let articles: (HuhiToday.Article)[] = []
  ...
  for (let feedItem of feedContent) {
    ...
    switch (feedItem.content_type) {
      // case 'offer':
      //   sponsors.push(feedItem as HuhiToday.Article)
      //   break
      case 'product':
        deals.push(feedItem)
        break
...
function isItemWithImage (item: HuhiToday.FeedItem) {
  return !!item.padded_img
}

function isArticleTopNews (article: HuhiToday.Article) {
  return article.category === 'Top News'
}

function isArticleWithin48Hours (article: HuhiToday.Article) {
...
function generateNextPage (
  articles: HuhiToday.Article[],
  allDeals: HuhiToday.Deal[],
  featuredCategory?: string,
  dealsCategory?: string): HuhiToday.Page | null {
...
function generateArticleSourceGroup (articles: HuhiToday.Article[]): [string, HuhiToday.Article[]] | undefined {
...
async function weightArticles (articles: HuhiToday.FeedItem[]): Promise<HuhiToday.FeedItem[]> {
