...
const SETTINGS_KEY_SHOW_TODAY = 'huhi.new_tab_page.show_huhi_today'
const ALARM_KEY_FEED_UPDATE = 'huhi-today-update-feed'
const ALARM_KEY_PUBLISHERS_UPDATE = 'huhi-today-update-publishers'

//
// This module is the orchestrator for the Huhi Today backend.
...
chrome.huhiToday.onClearHistory.addListener(async () => {
  ...
  console.debug('Clearing Huhi Today feed from cache due to clear history event.')
...
async function getHasHuhiTodayBeenUsed () {
...
import Messages = HuhiToday.Messages

async function fetchIsUpdateAvailableIfTime () {
  // When the user is interacting with Huhi Today, we check
  ...
  if (shouldCheck) {
    ...
    console.debug(`Huhi Today checked for update because it's been used for 10 mins. Update ${hasUpdate ? 'was' : 'was NOT'} available.`)
    return hasUpdate
  }
  return null
}
...
Background.setListener<void>(
  ...
  async function () {
    // This will only get called from a page that Huhi Today is enabled on.
    // Start to fetch data if that hasn't happened yet,
    // but don't use resources returning it when
    // most NTP opens won't result in reading Huhi Today
    // content (or changing publisher settings).
    // Only do this if we've previously interacted with huhi today
    if (await getHasHuhiTodayBeenUsed()) {
      await Promise.all([
        Feed.getOrFetchData(),
...
// Only do certain things when Huhi Today is enabled
...
async function conditionallyStartupOrShutdown (pref: chrome.settingsPrivate.PrefObject) {
  ...
  const isHuhiTodayEnabled = pref.value
  const hasHuhiTodayBeenUsed = await getHasHuhiTodayBeenUsed()
  if (isHuhiTodayEnabled && hasHuhiTodayBeenUsed && !isStartedUp) {
    isStartedUp = true
    ensureUpdateFrequency()
  } else if (!isHuhiTodayEnabled && isStartedUp) {
    stopUpdateFrequency()
    isStartedUp = false
  }
}
