...
import { isPublisherContentAllowed } from '../../../../../common/huhiToday'
...
type RemoteData = HuhiToday.FeedItem[]
...
const feedUrl = URLS.huhiTodayFeed
let memoryTodayData: HuhiToday.Feed | undefined
...
function setStorageData (feed: HuhiToday.Feed) {
...
export async function checkForRemoteUpdate () {
  ...
  setLastUpdateCheckTime(Date.now())
  console.debug(`Huhi Today checked for new update. New Etag is "${newEtag}". Last Etag was "${existingEtag}".`)
