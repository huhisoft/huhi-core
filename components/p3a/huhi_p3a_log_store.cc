...
void RecordP3A(uint64_t answers_count) {
  ...
  UMA_HISTOGRAM_EXACT_LINEAR("Huhi.P3A.SentAnswersCount", answer, 3);
}

}  // namespace

HuhiP3ALogStore::HuhiP3ALogStore(Delegate* delegate,
...
HuhiP3ALogStore::~HuhiP3ALogStore() = default;

void HuhiP3ALogStore::RegisterPrefs(PrefRegistrySimple* registry) {
  registry->RegisterDictionaryPref(kPrefName);
}

void HuhiP3ALogStore::UpdateValue(const std::string& histogram_name,
                                   uint64_t value) {
...
void HuhiP3ALogStore::RemoveValueIfExists(const std::string& histogram_name) {
...
void HuhiP3ALogStore::ResetUploadStamps() {
...
bool HuhiP3ALogStore::has_unsent_logs() const {
  return !unsent_entries_.empty();
}

bool HuhiP3ALogStore::has_staged_log() const {
  return !staged_entry_key_.empty();
}

const std::string& HuhiP3ALogStore::staged_log() const {
...
std::string HuhiP3ALogStore::staged_log_type() const {
  DCHECK(!staged_entry_key_.empty());
  auto iter = log_.find(staged_entry_key_);
  DCHECK(iter != log_.end());

  if (base::StartsWith(iter->first, "Huhi.P2A",
...
const std::string& HuhiP3ALogStore::staged_log_hash() const {
  NOTREACHED();
  return staged_log_hash_;
}

const std::string& HuhiP3ALogStore::staged_log_signature() const {
  NOTREACHED();
  return staged_log_signature_;
}

void HuhiP3ALogStore::StageNextLog() {
  // Stage the next item.
  ...
  VLOG(2) << "HuhiP3ALogStore::StageNextLog: staged " << staged_entry_key_;
}

void HuhiP3ALogStore::DiscardStagedLog() {
  if (!has_staged_log()) {
    return;
  }
...
void HuhiP3ALogStore::MarkStagedLogAsSent() {}

void HuhiP3ALogStore::TrimAndPersistUnsentLogs() {
  NOTREACHED();
}

void HuhiP3ALogStore::LoadPersistedUnsentLogs() {
  DCHECK(log_.empty());
