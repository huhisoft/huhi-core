...
class HuhiSyncAuthManagerTest : public testing::Test {
 protected:
  ...
  HuhiSyncAuthManagerTest() : identity_env_(&test_url_loader_factory_) {}

  ~HuhiSyncAuthManagerTest() override {}

  std::unique_ptr<HuhiSyncAuthManager> CreateAuthManager(
      const AccountStateChangedCallback& account_state_changed,
      const CredentialsChangedCallback& credentials_changed) {
    return std::make_unique<HuhiSyncAuthManager>(
        identity_env_.identity_manager(), account_state_changed,
        credentials_changed);
  }

  void SetNetworkTime() {
    huhi_sync::NetworkTimeHelper::GetInstance()->SetNetworkTimeForTest(
...
TEST_F(HuhiSyncAuthManagerTest, IgnoresEventsIfNotRegistered) {
...
TEST_F(HuhiSyncAuthManagerTest, GetAccessToken) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
  ...
  const std::string kHuhiServerKeyHeaderString =
    std::string(kHuhiServicesKeyHeader) + ": " + HUHI_SERVICES_KEY;

  ASSERT_EQ(auth_manager->GetCredentials().access_token,
            std::string(kAccessToken) + "\r\n" + kHuhiServerKeyHeaderString);
...
TEST_F(HuhiSyncAuthManagerTest, Reset) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
...
TEST_F(HuhiSyncAuthManagerTest, MalformedSyncCode) {
  base::MockCallback<AccountStateChangedCallback> account_state_changed;
  base::MockCallback<CredentialsChangedCallback> credentials_changed;
  EXPECT_CALL(account_state_changed, Run()).Times(0);
  EXPECT_CALL(credentials_changed, Run()).Times(0);
  ...
  auth_manager->DeriveSigningKeys("huhi5566");
  EXPECT_TRUE(
      auth_manager->GetActiveAccountInfo().account_info.account_id.empty());
}
