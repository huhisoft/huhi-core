...
namespace {
std::string AppendHuhiServiceKeyHeaderString() {
  return std::string("\r\n") + kHuhiServicesKeyHeader + ": " +
         HUHI_SERVICES_KEY;
}
}  // namespace

HuhiSyncAuthManager::HuhiSyncAuthManager(
...
void HuhiSyncAuthManager::DeriveSigningKeys(const std::string& seed) {
  VLOG(1) << __func__;
  if (seed.empty())
    return;
...
void HuhiSyncAuthManager::RequestAccessToken() {
  VLOG(1) << __func__;
  huhi_sync::NetworkTimeHelper::GetInstance()->GetNetworkTime(
      base::BindOnce(&HuhiSyncAuthManager::OnNetworkTimeFetched,
                     weak_ptr_factory_.GetWeakPtr()));
}

SyncAccountInfo HuhiSyncAuthManager::DetermineAccountToUse() const {
...
std::string HuhiSyncAuthManager::GenerateAccessToken(
  ...
  huhi_sync::crypto::Sign(timestamp_bytes, private_key_, &signature);
  DCHECK(huhi_sync::crypto::Verify(timestamp_bytes, signature, public_key_));
  ...
  return encoded_access_token + AppendHuhiServiceKeyHeaderString();
}

void HuhiSyncAuthManager::OnNetworkTimeFetched(const base::Time& time) {
