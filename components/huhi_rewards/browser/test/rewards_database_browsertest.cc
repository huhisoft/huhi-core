...
// npm run test -- huhi_browser_tests --filter=RewardsDatabaseBrowserTest.*
...
IN_PROC_BROWSER_TEST_F(RewardsDatabaseBrowserTest, Migration_4_ActivityInfo) {
  {
    ...
    while (info_sql.Step()) {
      auto info = ledger::type::PublisherInfo::New();
    ...
    EXPECT_EQ(list.at(0)->id, "huhisoft.com");
...
IN_PROC_BROWSER_TEST_F(RewardsDatabaseBrowserTest, Migration_5_ActivityInfo) {
  {
    ...
    EXPECT_EQ(list.at(1)->id, "huhisoft.com");
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_8_PendingContribution) {
  {
    ...
    pending_contribution->type = ledger::type::RewardsType::ONE_TIME_TIP;
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_11_ContributionInfo) {
  {
    ...
    EXPECT_EQ(tip_sql.ColumnInt(2),
        static_cast<int>(ledger::type::RewardsType::ONE_TIME_TIP));;
    ...
    EXPECT_EQ(ac_sql.ColumnInt(2),
        static_cast<int>(ledger::type::RewardsType::AUTO_CONTRIBUTE));;
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_12_ContributionInfo) {
  {
    ...
    sql::Statement info_sql(db_.GetUniqueStatement(query.c_str()));
    while (info_sql.Step()) {
      auto info = ledger::type::PendingContributionInfo::New();
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_14_UnblindedToken) {
  {
    ...
    while (tokens_sql.Step()) {
      auto info = ledger::type::UnblindedToken::New();
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_18_CredsBatch) {
  {
    ...
    ledger::type::CredsBatch creds_database;
    std::string query =
    ...
    while (sql.Step()) {
      ...
      creds_database.trigger_type =
          static_cast<ledger::type::CredsBatchType>(sql.ColumnInt(2));
      ...
      creds_database.status =
          static_cast<ledger::type::CredsBatchStatus>(sql.ColumnInt(8));
    }

    ledger::type::CredsBatch creds_expected;
    creds_expected.trigger_id = "36baa4c3-f92d-4121-b6d9-db44cb273a02";
    creds_expected.trigger_type = ledger::type::CredsBatchType::PROMOTION;
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_21_ContributionInfoPublishers) {
  {
    ...
    sql::Statement info_sql(db_.GetUniqueStatement(query.c_str()));
    while (info_sql.Step()) {
      auto contribution_publisher = ledger::type::ContributionPublisher::New();
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_23_ContributionQueue) {
  {
    ...
    ledger::type::ContributionQueue contribution_queue;
    const std::string query =
    ...
    while (sql.Step()) {
      ...
      contribution_queue.type =
          static_cast<ledger::type::RewardsType>(sql.ColumnInt(1));
    ...
    EXPECT_EQ(contribution_queue.type, ledger::type::RewardsType::ONE_TIME_TIP);
    ...
    ledger::type::ContributionQueuePublisher contribution_queue_publisher;
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_27_UnblindedTokens) {
  {
    ...
    ledger::type::UnblindedToken unblinded_token;
...
IN_PROC_BROWSER_TEST_F(
    ...
    Migration_26_UnblindedTokens) {
  {
    ...
    ledger::type::UnblindedTokenPtr token;
    std::vector<std::string> actual_values;
    while (sql.Step()) {
      if (!token) {
        token = ledger::type::UnblindedToken::New();
        ...
        token->redeem_type =
            static_cast<ledger::type::RewardsType>(sql.ColumnInt(8));
      }
    ...
    EXPECT_EQ(token->redeem_type, ledger::type::RewardsType::ONE_TIME_TIP);
  }
}
