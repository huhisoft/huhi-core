...
#include "huhi/base/containers/utils.h"
...
#include "huhi/components/huhi_rewards/browser/android_util.h"
#include "huhi/components/huhi_rewards/browser/file_util.h"
#include "huhi/components/huhi_rewards/browser/logging.h"
#include "huhi/components/huhi_rewards/browser/logging_util.h"
#include "huhi/components/huhi_rewards/browser/rewards_notification_service.h"
#include "huhi/components/huhi_rewards/browser/rewards_notification_service_impl.h"
...
#include "huhi/components/huhi_rewards/browser/static_values.h"
#include "huhi/components/huhi_rewards/browser/switches.h"
#include "huhi/components/huhi_rewards/common/pref_names.h"
#include "huhi/components/services/bat_ledger/public/cpp/ledger_client_mojo_bridge.h"
...
const char pref_prefix[] = "huhi.rewards";
...
GetNetworkTrafficAnnotationTagForFaviconFetch() {
  return net::DefineNetworkTrafficAnnotation(
      "huhi_rewards_favicon_fetcher", R"(
      semantics {
        sender:
          "Huhi Rewards Media Fetcher"
        description:
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTagForURLLoad() {
  return net::DefineNetworkTrafficAnnotation("rewards_service_impl", R"(
      semantics {
        ...
        trigger:
          "User-initiated for direct tipping or on a set interval while Huhi "
      ...
      policy {
        ...
        setting:
          ...
          "bar or by visiting huhi://rewards/."
        policy_exception_justification:
          "Not implemented."
      })");
}
...
RewardsServiceImpl::RewardsServiceImpl(Profile* profile)
  ...
  content::URLDataSource::Add(profile_,
                              std::make_unique<HuhiRewardsSource>(profile_));
...
void RewardsServiceImpl::OnAttestPromotion(
  ...
  if (promotion->type == ledger::type::PromotionType::ADS) {
    auto* ads_service = huhi_ads::AdsServiceFactory::GetForProfile(profile_);
    if (ads_service) {
...
void RewardsServiceImpl::PrepareLedgerEnvForTesting() {
  ...
  // this is needed because we are using huhiledger_request_util::buildURL
...
void RewardsServiceImpl::ProcessRewardsPageUrl(
  ...
  const auto url = GURL("huhi:/" + path + query);
  for (net::QueryIterator it(url); !it.IsAtEnd(); it.Advance()) {
...
void RewardsServiceImpl::GetHuhiWallet(GetHuhiWalletCallback callback) {
  if (!Connected()) {
    std::move(callback).Run(nullptr);
    return;
  }

  bat_ledger_->GetHuhiWallet(
    base::BindOnce(&RewardsServiceImpl::OnGetHuhiWallet,
        AsWeakPtr(),
        std::move(callback)));
}

void RewardsServiceImpl::OnGetHuhiWallet(
    GetHuhiWalletCallback callback,
