...
const tipTwitterMedia = (mediaMetaData: RewardsTip.MediaMetaData) => {
  ...
  chrome.tabs.query({
    ...
    chrome.huhiRewards.tipTwitterUser(tabId, mediaMetaData)
  })
}
...
const tipGitHubMedia = (mediaMetaData: RewardsTip.MediaMetaData) => {
  ...
  chrome.tabs.query({
    ...
    chrome.huhiRewards.tipGitHubUser(tabId, mediaMetaData)
  })
}
...
const tipRedditMedia = (mediaMetaData: RewardsTip.MediaMetaData) => {
  ...
  chrome.tabs.query({
    ...
    chrome.huhiRewards.tipRedditUser(tabId, mediaMetaData)
  })
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  switch (action) {
    ...
    case 'rewardsEnabled': {
      ...
      chrome.huhiRewards.getRewardsMainEnabled(function (enabled: boolean) {
    ...
    case 'inlineTippingPlatformEnabled': {
      ...
      chrome.huhiRewards.getInlineTippingPlatformEnabled(msg.key, function (enabled: boolean) {
        sendResponse({ enabled })
      })
      // Must return true for asynchronous calls to sendResponse
      return true
