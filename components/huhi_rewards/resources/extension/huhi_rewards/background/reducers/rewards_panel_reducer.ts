...
export const rewardsPanelReducer: Reducer<RewardsExtension.State | undefined> = (state: RewardsExtension.State, action: any) => {
  ...
  switch (action.type) {
    case types.TOGGLE_ENABLE_MAIN: {
      ...
      state[key] = enable
      chrome.huhiRewards.saveSetting(key, enable ? '1' : '0')
      break
    }
    case types.CREATE_WALLET:
      chrome.huhiRewards.createWallet()
      state = { ...state }
    ...
    case types.ON_TAB_RETRIEVED: {
      ...
      if (!publisher || (publisher.tabUrl !== tab.url || !validKey)) {
        ...
        chrome.huhiRewards.getPublisherData(
          tab.windowId,
          tab.url,
          tab.favIconUrl || '',
    ...
    case types.ON_NOTIFICATION_ADDED: {
      ...
      // (https://github.com/huhisoft/huhi-browser/issues/4344)
    ...
    case types.INCLUDE_IN_AUTO_CONTRIBUTION: {
      ...
      chrome.huhiRewards.includeInAutoContribution(publisherKey, exclude)
      break
    }
    ...
    case types.ON_SETTING_SAVE: {
      ...
      if (key) {
        state[key] = value
        chrome.huhiRewards.saveSetting(key, value)
      }
      break
    }
    case types.REMOVE_RECURRING_TIP: {
      ...
      chrome.huhiRewards.removeRecurringTip(publisherKey)
      break
    }
    case types.SAVE_RECURRING_TIP: {
      ...
      chrome.huhiRewards.saveRecurringTip(publisherKey, newAmount)
      break
    }
