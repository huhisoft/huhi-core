...
import { WalletAddIcon, BatColorIcon } from 'huhi-ui/components/icons'
...
export class Panel extends React.Component<Props, State> {
  ...
  onFetchCaptcha = () => {
    ...
    if (notification && notification.id) {
      ...
      if (promoId) {
        chrome.huhiRewards.claimPromotion(promoId, (properties: RewardsExtension.Captcha) => {
  ...
  onSolution = (promotionId: string, x: number, y: number) => {
    ...
    chrome.huhiRewards.attestPromotion(promotionId, data, (result: number, promotion?: RewardsExtension.Promotion) => {
      // if wrong position try again
      if (result === 6) {
        chrome.huhiRewards.claimPromotion(promotionId, (properties: RewardsExtension.Captcha) => {
          this.actions.onClaimPromotion(properties)
        })
      }
  ...
  onDisconnectClick = () => {
    chrome.huhiRewards.disconnectWallet('uphold')
  }
