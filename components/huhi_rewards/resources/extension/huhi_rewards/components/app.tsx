...
import { BatColorIcon, WalletAddIcon } from 'huhi-ui/components/icons'
...
export class RewardsPanel extends React.Component<Props, State> {
  ...
  componentDidMount () {
    ...
    chrome.huhiRewards.onlyAnonWallet((only: boolean) => {
      this.setState({
        onlyAnonWallet: !!only
      })
    })

    chrome.huhiRewards.getRewardsMainEnabled(((enabled: boolean) => {
  ...
  openPrivacyPolicy () {
    chrome.tabs.create({
      url: 'https://huhisoft.com/privacy#rewards'
    })
  }
