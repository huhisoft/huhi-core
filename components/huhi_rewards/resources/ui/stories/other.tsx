...
import { BatColorIcon, SettingsIcon, UpholdColorIcon } from 'huhi-ui/components/icons'
import GrantClaim from '../components/grantClaim'

const favicon = require('./img/huhi-favicon.png')
...
storiesOf('Rewards/Other/Desktop', module)
    ...
    return (
      <div style={{ width: '595px' }}>
        <Box
          ...
          description={
            text('Description', `Earn tokens by seeing ads on Huhi. Ads are matched from machine learning and the data temporarily present in your browser without tracking your information or sending it outside.`)
          }
          onToggle={onToggle}
  ...
  .add('List - Token', () => {
    return (
      <div style={{ width: '400px' }}>
        <ListToken
          title={text('Title', 'Huhi Contribute')}
  ...
  .add('Tooltip', () => {
    const huhiAdsText = <span>Huhi Ads Settings</span>
...
storiesOf('Rewards/Other/Mobile', module)
  ...
  .add('Box', withState({ checked: true, toggle: true }, (store) => {
    ...
    return (
      <div style={{ width: '100%' }}>
        <BoxMobile
          title={text('Title', 'Huhi Auto-Contribute')}
          toggle={boolean('Show toggle', store.state.toggle)}
          checked={boolean('Toggle checked', store.state.checked)}
          type={select<any>('Type', { contribute: 'contribute', donation: 'donation', ads: 'ads' }, 'contribute')}
          description={
