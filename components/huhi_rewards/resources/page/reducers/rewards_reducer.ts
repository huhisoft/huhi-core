...
const rewardsReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  ...
  switch (action.type) {
    ...
    case types.GET_TIP_TABLE: {
      chrome.send('huhi_rewards.getRecurringTips')
      chrome.send('huhi_rewards.getOneTimeTips')
      break
    }
    case types.GET_CONTRIBUTE_LIST: {
      chrome.send('huhi_rewards.getContributionList')
      break
    }
    case types.GET_ADS_DATA: {
      chrome.send('huhi_rewards.getAdsData')
      break
    }
    ...
    case types.GET_ADS_HISTORY: {
      chrome.send('huhi_rewards.getAdsHistory')
      break
    }
    ...
    case types.TOGGLE_AD_THUMB_UP: {
      chrome.send('huhi_rewards.toggleAdThumbUp',
    ...
    case types.TOGGLE_AD_THUMB_DOWN: {
      chrome.send('huhi_rewards.toggleAdThumbDown',
                  [action.payload.uuid, action.payload.creativeSetId, action.payload.likeAction])
      break
    }
    ...
    case types.TOGGLE_AD_OPT_IN_ACTION: {
      chrome.send('huhi_rewards.toggleAdOptInAction',
    ...
    case types.TOGGLE_AD_OPT_OUT_ACTION: {
      chrome.send('huhi_rewards.toggleAdOptOutAction',
                  [action.payload.category, action.payload.optAction])
      break
    }
    ...
    case types.TOGGLE_SAVE_AD: {
      chrome.send('huhi_rewards.toggleSaveAd',
    ...
    case types.TOGGLE_FLAG_AD: {
      chrome.send('huhi_rewards.toggleFlagAd',
    ...
    case types.ON_ADS_SETTING_SAVE: {
      ...
      if (key) {
        state[key] = value
        chrome.send('huhi_rewards.saveAdsSetting', [key, value.toString()])
    ...
    case types.ON_INLINE_TIP_SETTINGS_CHANGE: {
      ...
      inlineTip[key] = value
      chrome.send('huhi_rewards.setInlineTippingPlatformEnabled', [key, value.toString()])
    ...
    case types.PROCESS_REWARDS_PAGE_URL: {
      ...
      chrome.send('huhi_rewards.processRewardsPageUrl', [path, query])
    ...
    case types.ON_PROCESS_REWARDS_PAGE_URL: {
      ...
      chrome.send('huhi_rewards.getExternalWallet', [data.walletType])
      ...
      if (data.walletType === 'uphold') {
        chrome.send('huhi_rewards.fetchBalance')
    ...
    case types.DISCONNECT_WALLET: {
      chrome.send('huhi_rewards.disconnectWallet', [action.payload.walletType])
      break
    }
    case types.ONLY_ANON_WALLET: {
      chrome.send('huhi_rewards.onlyAnonWallet')
      break
    }
    ...
    case types.GET_COUNTRY_CODE: {
      chrome.send('huhi_rewards.getCountryCode')
      break
    }
    ...
    case types.COMPLETE_RESET: {
      chrome.send('huhi_rewards.completeReset')
      break
    }
