...
const walletReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  ...
  switch (action.type) {
    ...
    case types.RECOVER_WALLET: {
      ...
      chrome.send('huhi_rewards.recoverWallet', [key])
      break
    }
    case types.ON_RECOVER_WALLET_DATA: {
      ...
      if (result === 0) {
        chrome.send('huhi_rewards.fetchPromotions')
    ...
    case types.GET_PENDING_CONTRIBUTIONS: {
      chrome.send('huhi_rewards.getPendingContributions')
      break
    }
    ...
    case types.REMOVE_PENDING_CONTRIBUTION: {
      chrome.send('huhi_rewards.removePendingContribution', [
        action.payload.id
      ])
      break
    }
    case types.REMOVE_ALL_PENDING_CONTRIBUTION: {
      chrome.send('huhi_rewards.removeAllPendingContribution')
      break
    }
    ...
    case types.ON_BALANCE: {
      ...
      if (status === 0) { // on ledger::type::Result::LEDGER_OK
        ...
        chrome.send('huhi_rewards.getExternalWallet', ['uphold'])
    ...
    case types.GET_EXTERNAL_WALLET: {
      chrome.send('huhi_rewards.getExternalWallet', [action.payload.type])
      break
    }
    ...
    case types.GET_MONTHLY_REPORT: {
      ...
      chrome.send('huhi_rewards.getMonthlyReport', [month, year])
      break
    }
    ...
    case types.GET_MONTHLY_REPORT_IDS: {
      chrome.send('huhi_rewards.getMonthlyReportIds')
      break
    }
