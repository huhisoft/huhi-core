...
import { WalletAddIcon, WalletWithdrawIcon } from 'huhi-ui/components/icons'
...
class PageWallet extends React.Component<Props, State> {
  ...
  onModalBackupOnCopy = async (backupKey: string) => {
    ...
    try {
      ...
      chrome.send('huhi_rewards.setBackupCompleted')
    } catch (e) {
  ...
  onModalBackupOnSaveFile = (backupKey: string) => {
    ...
    const backupFileText = 'huhi_wallet_recovery.txt'
  ...
  getProcessorString = (processor: Rewards.Processor) => {
    ...
    switch (processor) {
      ...
      case 1: { // Rewards.Processor.HUHI_TOKENS
        text = getLocale('processorHuhiTokens')
        break
      }
      ...
      case 3: { // Rewards.Processor.HUHI_USER_FUNDS
        text = getLocale('processorHuhiUserFunds')
        break
      }
    }
