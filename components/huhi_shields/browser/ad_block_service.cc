...
std::string GetTagFromPrefName(const std::string& pref_name) {
  if (pref_name == kFBEmbedControlType) {
    return huhi_shields::kFacebookEmbeds;
  }
  if (pref_name == kTwitterEmbedControlType) {
    return huhi_shields::kTwitterEmbeds;
  }
  if (pref_name == kLinkedInEmbedControlType) {
    return huhi_shields::kLinkedInEmbeds;
  }
  return "";
}
...
AdBlockRegionalServiceManager* AdBlockService::regional_service_manager() {
  if (!regional_service_manager_)
    regional_service_manager_ =
        huhi_shields::AdBlockRegionalServiceManagerFactory(
            component_delegate_);
...
AdBlockService::custom_filters_service() {
  if (!custom_filters_service_)
    custom_filters_service_ =
        huhi_shields::AdBlockCustomFiltersServiceFactory(
            component_delegate_);
...
void AdBlockService::OnComponentReady(const std::string& component_id,
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&huhi_component_updater::GetDATFileAsString,
                     resources_file_path),
...
void AdBlockPrefService::OnPreferenceChanged(const std::string& pref_name) {
  ...
  g_huhi_browser_process->ad_block_service()->EnableTag(tag, enabled);
  g_huhi_browser_process->ad_block_regional_service_manager()->EnableTag(
      tag, enabled);
  g_huhi_browser_process->ad_block_custom_filters_service()->EnableTag(
      tag, enabled);
}
