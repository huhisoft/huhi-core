...
bool TrackingProtectionService::ShouldStoreState(HostContentSettingsMap* map,
  ...
  if (!huhi_shields::GetHuhiShieldsEnabled(map, starting_site))
    return true;


  if (huhi_shields::GetCookieControlType(map, starting_site) !=
      ControlType::BLOCK)
    return true;
...
#else  // !BUILDFLAG(HUHI_STP_ENABLED)
bool TrackingProtectionService::ShouldStoreState(HostContentSettingsMap* map,
                                                 int render_process_id,
                                                 int render_frame_id,
                                                 const GURL& origin_url) const {
...
#endif  // BUILDFLAG(HUHI_STP_ENABLED)
