...
void RecordShieldsToggled(PrefService* local_state) {
  ::huhi_shields::MaybeRecordShieldsUsageP3A(::huhi_shields::kShutOffShields,
                                              local_state);
}

void RecordShieldsSettingChanged(PrefService* local_state) {
  ::huhi_shields::MaybeRecordShieldsUsageP3A(
      ::huhi_shields::kChangedPerSiteShields, local_state);
}
...
void SetHuhiShieldsEnabled(HostContentSettingsMap* map,
                            bool enable,
                            const GURL& url,
  ...
  map->SetContentSettingCustomScope(
      ...
      ContentSettingsType::PLUGINS, kHuhiShields,
      // this is 'allow_huhi_shields' so 'enable' == 'allow'
      enable ? CONTENT_SETTING_ALLOW : CONTENT_SETTING_BLOCK);

  RecordShieldsToggled(local_state);
}

void ResetHuhiShieldsEnabled(HostContentSettingsMap* map, const GURL& url) {
  if (url.is_valid() && !url.SchemeIsHTTPOrHTTPS())
    return;
...
bool GetHuhiShieldsEnabled(HostContentSettingsMap* map, const GURL& url) {
  if (url.is_valid() && !url.SchemeIsHTTPOrHTTPS())
    return false;

  ContentSetting setting = map->GetContentSetting(
      url, GURL(), ContentSettingsType::PLUGINS, kHuhiShields);

  // see EnableHuhiShields - allow and default == true
...
bool ShouldDoCosmeticFiltering(HostContentSettingsMap* map, const GURL& url) {
  return base::FeatureList::IsEnabled(features::kHuhiAdblockCosmeticFiltering)
      && GetHuhiShieldsEnabled(map, url)
...
ControlType GetFingerprintingControlType(HostContentSettingsMap* map,
  ...
  ContentSetting fp_setting =
      GetHuhiFPContentSettingFromRules(fingerprinting_rules, url);
...
void DispatchBlockedEvent(const GURL& request_url,
  ...
  HuhiShieldsWebContentsObserver::DispatchBlockedEvent(
      block_type, request_url.spec(),
      render_process_id, render_frame_id, frame_tree_node_id);

#if BUILDFLAG(ENABLE_HUHI_PERF_PREDICTOR)
  huhi_perf_predictor::PerfPredictorTabHelper::DispatchBlockedEvent(
...
bool ShouldCleanReferrerForTopLevelNavigation(const std::string& method,
  ...
  // See https://github.com/huhisoft/huhi-browser/issues/8696
