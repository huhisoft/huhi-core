...
import { CaratRightIcon } from 'huhi-ui/components/icons'
...
function CategoryList (props: CategoryListProps) {
  ...
  return (
    <>
      <SettingsSectionTitle>{getLocale('huhiTodaySourcesTitle')}</SettingsSectionTitle>
...
type CategoryProps = {
  category: string
  publishers: HuhiToday.Publisher[]
  onBack: () => any
...
const categoryNameAll = getLocale('huhiTodayCategoryNameAll')
...
export default function Sources (props: SourcesProps) {
  const publishersByCategory = React.useMemo<Map<string, HuhiToday.Publisher[]>>(() => {
    const result = new Map<string, HuhiToday.Publisher[]>()
