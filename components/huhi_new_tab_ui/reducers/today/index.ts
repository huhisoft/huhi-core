...
export type HuhiTodayState = {
  ...
  articleScrollTo?: HuhiToday.FeedItem
}
...
const defaultState: HuhiTodayState = {
  isFetching: true,
...
if (history.state && history.state.todayArticle) {
  ...
  defaultState.articleScrollTo = history.state.todayArticle as HuhiToday.FeedItem
...
// if it gets removed and rendered again (e.g. if huhi today is toggled off and on).
// Reset to defaultState when Today is turned off or refreshed.

const reducer = createReducer<HuhiTodayState>({}, defaultState)
