...
const StyledPage = styled<PageProps, 'div'>('div')`
  ...
  /* Fix the main NTP content so, when Huhi Today is in-view,
  ...
  .${CLASSNAME_PAGE_STUCK} & {
    position: fixed;
    bottom: 0;
    /* Blur out the content when Huhi Today is interacted
...
export const Page: React.FunctionComponent<PageProps> = (props) => {
  ...
  // extra scroll area (Huhi Today) then we "sticky" the Page at
...
export const GridItemNavigationHuhiToday = styled<{}, 'div'>('div')`
  position: absolute;
  bottom: 20px;
  left: 50%;
...
function getPageBackground (p: HasImageProps) {
  ...
  // and Page is the element with blur (when huhi today is active)
...
export const IconButton = styled<IconButtonProps, 'button'>('button')`
  ...
  &:focus-visible {
    outline: 2px solid ${p => p.theme.color.brandHuhiInteracting};
  }
`
