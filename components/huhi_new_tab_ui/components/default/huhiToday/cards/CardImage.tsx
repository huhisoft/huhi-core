...
function useGetUnpaddedImage (paddedUrl: string, onLoaded?: () => any) {
  ...
  React.useEffect(() => {
    // Storybook method
    // @ts-ignore
    if (window.huhiStorybookUnpadUrl) {
      // @ts-ignore
      window.huhiStorybookUnpadUrl(paddedUrl)
      .then(onReceiveUnpaddedUrl)
      return
    }
    Background.send<HuhiToday.Messages.GetImageDataResponse,
        HuhiToday.Messages.GetImageDataPayload>(
