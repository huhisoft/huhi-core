...
const huhiExtensionId = 'mnojpmjdmbbfmejpflffifhffcmidifd'
...
export function send<U= void, T= void> (messageType: string, payload?: T): Promise<U> {
  ...
  console.debug(`Sending data to huhi extension for ${messageType}`, { messageType, payload })
  return new Promise(function (resolve) {
    chrome.runtime.sendMessage(huhiExtensionId, { messageType, payload }, function (responseData: U) {
      console.debug(`got response from huhi extension for "${messageType}"`, responseData)
