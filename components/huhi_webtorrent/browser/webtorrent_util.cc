...
bool IsWebtorrentEnabled(content::BrowserContext* browser_context) {
  ...
  return registry->enabled_extensions().Contains(huhi_webtorrent_extension_id);
}
...
bool IsWebtorrentURL(const GURL& url) {
  if (url.SchemeIs(extensions::kExtensionScheme) &&
      ...
      (url.ExtractFileName() == huhi_webtorrent_extension_filename ||
       url.ExtractFileName() == huhi_webtorrent_extension_filename2)) {
    return true;
  }
