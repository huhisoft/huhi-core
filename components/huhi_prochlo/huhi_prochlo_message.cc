...
#include "huhi/components/huhi_prochlo/prochlo_data.h"
...
bool MakeProchlomation(uint64_t metric,
  ...
  // TODO(iefremov): - create patch for adding `huhi_p3a`
  // to src/base/trace_event/builtin_categories.h
  // TRACE_EVENT0("huhi_p3a", "MakeProchlomation");

  HuhiProchloCrypto crypto;
...
void InitProchloMessage(uint64_t metric_hash,
                        ...
                        huhi_pyxis::PyxisMessage* pyxis_message) {
  DCHECK(pyxis_message);
  huhi_pyxis::PyxisValue* value = pyxis_message->add_pyxis_values();
...
void GenerateProchloMessage(uint64_t metric_hash,
  ...
  // TODO(iefremov): - create patch for adding `huhi_p3a`
  // to src/base/trace_event/builtin_categories.h
  // TRACE_EVENT0("huhi_p3a", "GenerateProchloMessage");
  ShufflerItem item;
...
void GenerateP3AMessage(uint64_t metric_hash,
  ...
  // TODO(iefremov): - create patch for adding `huhi_p3a`
  // to src/base/trace_event/builtin_categories.h
  // TRACE_EVENT0("huhi_p3a", "GenerateP3AMessage");
  uint8_t data[kProchlomationDataLength] = {0};
