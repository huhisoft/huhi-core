...
function addHuhiBehaviors(moduleName, component) {
...
function addHuhiProperties(moduleName, component) {
...
const allHuhiTemplateModificationsMap = {}

function addHuhiTemplateModifications(moduleName, component, modifyFn) {
...
const styleOverridePrefix = 'huhi-override-style-'

function addHuhiStyleOverride(moduleName, component) {
  ...
  if (debug)
    console.log(`Huhi Style Override added for ${moduleName}`, styleElement)
}
...
export function RegisterPolymerComponentProperties(propertiesMap) {
  ...
  for (const componentName in propertiesMap) {
    if (componentPropertyModifications[componentName]) {
        addHuhiProperties(componentName, componentPropertyModifications[componentName])
    }
  }
}
...
export function RegisterPolymerTemplateModifications(modificationsMap) {
  ...
  for (const componentName in modificationsMap) {
    ...
    addHuhiTemplateModifications(componentName, existingComponent, modifyFn)
  }
  Object.assign(allHuhiTemplateModificationsMap, awaitingComponentModifications)
}
...
export async function RegisterStyleOverride(componentName, styleTemplate) {
  ...
  const existingComponent = window.customElements.get(componentName)
  if (existingComponent) {
    addHuhiStyleOverride(componentName, existingComponent)
  } else {
...
export function OverrideIronIcons(iconSetName, overridingIconSetName, iconOverrides) {
  ...
  for (const chromiumIconName in iconOverrides) {
    ...
    if (!chromiumIcon) {
      console.error(`[huhi overrides] Could not find chromium icon '${chromiumIconName}' in iconset '${iconSetName}' for replacement!`)
      continue
    }
    const huhiIconName = iconOverrides[chromiumIconName]
    const huhiIcon = overrideIconSet.querySelector(`#${huhiIconName}`)
    if (!huhiIcon) {
      console.error(`[huhi overrides] Could not find huhi icon '${huhiIconName}' in iconset '${overridingIconSetName}' for replacement!`)
      continue
    }
    // replace
    ...
    while (huhiIcon.firstChild) {
      chromiumIcon.appendChild(huhiIcon.firstChild)
    }
  }
...
function PerformHuhiModifications(name, component) {
  if (debug) {
    console.debug(`Polymer component registering: ${name}`, component)
  }
  addHuhiBehaviors(name, component)
  addHuhiProperties(name, component)
  const templateModifyFn = allHuhiTemplateModificationsMap[name]
  if (templateModifyFn) {
    addHuhiTemplateModifications(name, component, templateModifyFn)
    delete allHuhiTemplateModificationsMap[name]
  }
  if (moduleNamesWithStyleOverrides.includes(name)) {
    addHuhiStyleOverride(name, component)
  }
}
...
Polymer.Class = function (info, mixin) {
  ...
  PerformHuhiModifications(name, info)
  return oldClass(info, mixin)
}
