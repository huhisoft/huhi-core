...
PostClaimHuhi::PostClaimHuhi(LedgerImpl* ledger) : ledger_(ledger) {
  DCHECK(ledger_);
}

PostClaimHuhi::~PostClaimHuhi() = default;

std::string PostClaimHuhi::GetUrl() {
...
std::string PostClaimHuhi::GeneratePayload(
...
type::Result PostClaimHuhi::CheckStatusCode(const int status_code) {
...
void PostClaimHuhi::Request(
    const std::string& destination_payment_id,
    PostClaimHuhiCallback callback) {
  auto url_callback = std::bind(&PostClaimHuhi::OnRequest,
      this,
      _1,
      callback);
