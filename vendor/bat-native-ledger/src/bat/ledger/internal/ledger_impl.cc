...
LedgerImpl::LedgerImpl(ledger::LedgerClient* client) :
    ...
    media_(std::make_unique<huhiledger_media::Media>(this)),
    contribution_(
...
huhiledger_media::Media* LedgerImpl::media() const {
  return media_.get();
}
...
void LedgerImpl::OnPostData(
  ...
  if (type == TWITCH_MEDIA_TYPE) {
    ...
    huhiledger_media::GetTwitchParts(post_data, &twitchParts);
  ...
  if (type == VIMEO_MEDIA_TYPE) {
    ...
    huhiledger_media::GetVimeoParts(post_data, &parts);
