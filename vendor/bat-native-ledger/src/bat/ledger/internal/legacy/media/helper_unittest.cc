...
// npm run test -- huhi_unit_tests --filter=MediaHelperTest.*
...
TEST(MediaHelperTest, GetMediaKey) {
  // provider is missing
  std::string result = huhiledger_media::GetMediaKey("key", "");
  ASSERT_EQ(result, "");

  // key is missing
  result = huhiledger_media::GetMediaKey("", "youtube");
  ASSERT_EQ(result, "");

  // all ok
  result = huhiledger_media::GetMediaKey("key", "youtube");
...
TEST(MediaHelperTest, GetTwitchParts) {
  ...
  // string is empty
  huhiledger_media::GetTwitchParts("", &twitch_parts);
  ASSERT_EQ(twitch_parts.size(), 0u);

  // random string
  huhiledger_media::GetTwitchParts("this is random string", &twitch_parts);
  ASSERT_EQ(twitch_parts.size(), 0u);
  ...
  // all ok
  huhiledger_media::GetTwitchParts(post_data, &twitch_parts);
...
//  // string empty
  std::string result = huhiledger_media::ExtractData("", "/", "!");
  ASSERT_EQ(result, "");

  // missing start
  result = huhiledger_media::ExtractData("st/find/me!", "", "!");
