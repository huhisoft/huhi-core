...
// npm run test -- huhi_unit_tests --filter=BatHelperTest.*
...
TEST(BatHelperTest, HasSameDomainAndPath) {
  ...
  // regular url
  bool result = huhiledger_bat_helper::HasSameDomainAndPath(
      url, url_portion, path);
  ASSERT_EQ(result, true);

  // empty url with portion
  url = std::string();
  result = huhiledger_bat_helper::HasSameDomainAndPath(
      url, url_portion, path);
