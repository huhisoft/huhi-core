...
std::string Wallet::GetWalletPassphrase(type::HuhiWalletPtr wallet) {
  if (!wallet) {
...
type::HuhiWalletPtr Wallet::GetWallet() {
  const std::string wallet_string =
      ledger_->ledger_client()->GetEncryptedStringState(state::kWalletHuhi);
  ...
  if (!value || !value->is_dict()) {
    BLOG(0, "Parsing of huhi wallet failed");
    return nullptr;
  }
  ...
  auto wallet = ledger::type::HuhiWallet::New();
...
bool Wallet::SetWallet(type::HuhiWalletPtr wallet) {
  if (!wallet) {
    BLOG(0, "Huhi wallet is null");
    return false;
  }
  ...
  BLOG_IF(0, !success, "Can't encrypt huhi wallet");

  return success;
}

void Wallet::LinkHuhiWallet(
    const std::string& destination_payment_id,
    ledger::ResultCallback callback) {
  promotion_server_->post_claim_huhi()->Request(
