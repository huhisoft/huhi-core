...
NSString * const BATHuhiLedgerErrorDomain = @"BATHuhiLedgerErrorDomain";
NSNotificationName const BATHuhiLedgerNotificationAdded = @"BATHuhiLedgerNotificationAdded";

BATHuhiGeneralLedgerNotificationID const BATHuhiGeneralLedgerNotificationIDWalletNowVerified = @"wallet_new_verified";
BATHuhiGeneralLedgerNotificationID const BATHuhiGeneralLedgerNotificationIDWalletDisconnected = @"wallet_disconnected";
...
@interface BATHuhiLedger () <NativeLedgerClientBridge> {
...
@property (nonatomic) NSHashTable<BATHuhiLedgerObserver *> *observers;
...
- (instancetype)initWithStateStoragePath:(NSString *)path
{
  if ((self = [super init])) {
    ...
    const auto pathToICUDTL = [[NSBundle bundleForClass:[BATHuhiLedger class]] pathForResource:@"icudtl" ofType:@"dat"];
...
- (void)addObserver:(BATHuhiLedgerObserver *)observer
{
  [self.observers addObject:observer];
}

- (void)removeObserver:(BATHuhiLedgerObserver *)observer
{
...
- (void)createWallet:(void (^)(NSError * _Nullable))completion
{
  ...
  ledger->CreateWallet(^(ledger::type::Result result) {
    ...
    if (result != ledger::type::Result::WALLET_CREATED) {
      ...
      error = [NSError errorWithDomain:BATHuhiLedgerErrorDomain code:static_cast<NSInteger>(result) userInfo:userInfo];
    }

    strongSelf.ads.enabled = [BATHuhiAds isCurrentLocaleSupported];
                    ...
                    completion:(nullable void (^)(BATResult result))completion
{
  ledger->DisconnectWallet(walletType.UTF8String, ^(ledger::type::Result result){
    ...
    for (BATHuhiLedgerObserver *observer in self.observers) {
                     ...
                     onlyOnce:(BOOL)onlyOnce
{
  ...
  [NSNotificationCenter.defaultCenter postNotificationName:BATHuhiLedgerNotificationAdded object:nil];
...
- (void)resetState:(const std::string &)name callback:(ledger::ResultCallback)callback
{
  ...
  // In huhi-core, failed callback returns `LEDGER_ERROR`
