...
@interface _MockNotificationHandler : NSObject <BATHuhiAdsNotificationHandler>
...
- (void)setUp
{
  [BATHuhiAds setDebug:YES];
  const auto path = [self stateStoragePath];
  [[NSFileManager defaultManager] removeItemAtPath:path error:nil];
  self.ads = [[BATHuhiAds alloc] initWithStateStoragePath:path];
}
...
- (void)testPreferencePersistance
{
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
    BATHuhiAds *secondAds = [[BATHuhiAds alloc] initWithStateStoragePath:[self stateStoragePath]];
