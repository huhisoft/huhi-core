...
/// Configuration around huhi rewards for ads & ledger
NS_SWIFT_NAME(HuhiRewardsConfiguration)
@interface BATHuhiRewardsConfiguration : NSObject <NSCopying>
...
@property (nonatomic, nullable) BATHuhiAdsBuildChannel *buildChannel;
...
@property (nonatomic, class, readonly) BATHuhiRewardsConfiguration *defaultConfiguration NS_SWIFT_NAME(default);
/// The staging configuration. Environment is staging, no changes to ads or ledger configuration
///
/// State is stored in Application Support
@property (nonatomic, class, readonly) BATHuhiRewardsConfiguration *stagingConfiguration NS_SWIFT_NAME(staging);
/// The production configuration. Environment is production, no changes to ads or ledger configuration
///
/// State is stored in Application Support
@property (nonatomic, class, readonly) BATHuhiRewardsConfiguration *productionConfiguration NS_SWIFT_NAME(production);
...
@property (nonatomic, class, readonly) BATHuhiRewardsConfiguration *testingConfiguration NS_SWIFT_NAME(testing);

@end

NS_SWIFT_NAME(HuhiRewardsDelegate)
@protocol BATHuhiRewardsDelegate <NSObject>
@required
...
/// A container for handling Huhi Rewards. Use `ads` to handle how many ads the users see,
/// when to display them. Use `ledger` to manage interactions between the users wallet & publishers
NS_SWIFT_NAME(HuhiRewards)
@interface BATHuhiRewards : NSObject

@property (nonatomic, readonly) BATHuhiAds *ads;
@property (nonatomic, readonly) BATHuhiLedger *ledger;
@property (nonatomic, weak) id<BATHuhiRewardsDelegate> delegate;

/// Resets the ads & ledger (by purging its data). This should likely never be used in production.
- (void)reset;

/// Create a HuhiRewards instance with a given configuration
- (instancetype)initWithConfiguration:(BATHuhiRewardsConfiguration *)configuration;
/// Create a HuhiRewards instance with a given configuration and custom ledger classes for mocking
- (instancetype)initWithConfiguration:(BATHuhiRewardsConfiguration *)configuration
...
@interface BATHuhiRewards (Reporting)
