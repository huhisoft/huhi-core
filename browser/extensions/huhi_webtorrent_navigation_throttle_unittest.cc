...
class MockBrowserClient : public content::ContentBrowserClient {
 public:
  ...
  // Only construct an HuhiWebTorrentNavigationThrottle so that we can test it
  // in isolation.
  std::vector<std::unique_ptr<NavigationThrottle>> CreateThrottlesForNavigation(
      content::NavigationHandle* handle) override {
    std::vector<std::unique_ptr<NavigationThrottle>> throttles;
    throttles.push_back(
        std::make_unique<HuhiWebTorrentNavigationThrottle>(handle));
    return throttles;
  }
};
...
class HuhiWebTorrentNavigationThrottleUnitTest
    : public content::RenderViewHostTestHarness {
 public:
  HuhiWebTorrentNavigationThrottleUnitTest()
  ...
  void AddExtension() {
    ...
    extension_ = ExtensionBuilder()
                     ...
                     .SetID(huhi_webtorrent_extension_id)
                     .Build();
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(HuhiWebTorrentNavigationThrottleUnitTest);
};

// Tests the basic case of loading a URL, it should proceed.
TEST_F(HuhiWebTorrentNavigationThrottleUnitTest, ExternalWebPage) {
  ...
  auto throttle =
      std::make_unique<HuhiWebTorrentNavigationThrottle>(&test_handle);
...
TEST_F(HuhiWebTorrentNavigationThrottleUnitTest,
...
TEST_F(HuhiWebTorrentNavigationThrottleUnitTest, WebTorrentUrlInstalled) {
  AddExtension();
  web_contents_tester()->NavigateAndCommit(GURL("http://example.com"));
  content::RenderFrameHost* host =
      render_frame_host_tester(main_rfh())->AppendChild("child");
...
// disabled.
TEST_F(HuhiWebTorrentNavigationThrottleUnitTest, WebTorrentDisabledByPref) {
