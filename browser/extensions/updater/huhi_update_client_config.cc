...
std::string HuhiUpdateClientConfig::GetChannel() const {
  return std::string("stable");
}

scoped_refptr<update_client::NetworkFetcherFactory>
HuhiUpdateClientConfig::GetNetworkFetcherFactory() {
...
bool HuhiUpdateClientConfig::EnabledCupSigning() const {
  return false;
}

HuhiUpdateClientConfig::~HuhiUpdateClientConfig() = default;

// static
scoped_refptr<ChromeUpdateClientConfig> HuhiUpdateClientConfig::Create(
    content::BrowserContext* context,
    base::Optional<GURL> update_url_override) {
  FactoryCallback& factory = GetFactoryCallback();
  return factory.is_null() ? base::MakeRefCounted<HuhiUpdateClientConfig>(
