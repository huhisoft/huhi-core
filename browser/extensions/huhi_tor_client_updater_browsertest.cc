...
class HuhiTorClientUpdaterTest : public ExtensionBrowserTest {
 public:
  HuhiTorClientUpdaterTest() {}
  ...
  void SetComponentIdAndBase64PublicKeyForTest(
    ...
    extensions::HuhiTorClientUpdater::SetComponentIdAndBase64PublicKeyForTest(
  ...
  bool InstallTorClientUpdater(const std::string& extension_dir) {
    ...
    g_huhi_browser_process->tor_client_updater()->OnComponentReady(
  ...
  void WaitForTorClientUpdaterThread() {
    scoped_refptr<base::ThreadTestHelper> io_helper(
        new base::ThreadTestHelper(
            g_huhi_browser_process->tor_client_updater()->GetTaskRunner()));
...
// installs the client.
IN_PROC_BROWSER_TEST_F(HuhiTorClientUpdaterTest, TorClientInstalls) {
...
// the client.
IN_PROC_BROWSER_TEST_F(HuhiTorClientUpdaterTest, TorClientLaunches) {
  SetComponentIdAndBase64PublicKeyForTest(
      kTorClientUpdaterComponentTestId,
      kTorClientUpdaterComponentTestBase64PublicKey);
  ASSERT_TRUE(InstallTorClientUpdater());
