...
HuhiExtensionService::HuhiExtensionService(Profile* profile,
...
HuhiExtensionService::~HuhiExtensionService() {
}

void HuhiExtensionService::AddComponentExtension(const Extension* extension) {
  ExtensionService::AddComponentExtension(extension);

  // Disable Huhi Rewards extension action for Guest and Tor profiles on all
  ...
  // the extension when HuhiActionViewController is queried about the
  // visibility of the action.
  if ((extension->id() == huhi_rewards_extension_id) &&
      (profile_->IsGuestSession() || huhi::IsTorProfile(profile_))) {
