...
class HuhiIpfsClientUpdaterTest : public ExtensionBrowserTest {
 public:
  HuhiIpfsClientUpdaterTest() {}
  ...
  void SetComponentIdAndBase64PublicKeyForTest(
    ...
    ipfs::HuhiIpfsClientUpdater::SetComponentIdAndBase64PublicKeyForTest(
  ...
  bool InstallIpfsClientUpdater(const std::string& extension_dir) {
    ...
    g_huhi_browser_process->ipfs_client_updater()->OnComponentReady(
  ...
  void WaitForIpfsClientUpdaterThread() {
    scoped_refptr<base::ThreadTestHelper> io_helper(
        new base::ThreadTestHelper(
            g_huhi_browser_process->ipfs_client_updater()->GetTaskRunner()));
...
// installs the client.
IN_PROC_BROWSER_TEST_F(HuhiIpfsClientUpdaterTest, IpfsClientInstalls) {
  ...
  base::FilePath executable_path =
      g_huhi_browser_process->ipfs_client_updater()->GetExecutablePath();
