...
class HuhiWalletAPIBrowserTest : public InProcessBrowserTest,
    public InfoBarManager::Observer {
 public:
  HuhiWalletAPIBrowserTest() : infobar_observer_(this),
      infobar_added_(false) {
  }

  void WaitForHuhiExtensionAdded() {
    // Huhi extension must be loaded, otherwise dapp detection events
    // could be missed from a race condition.
    ExtensionTestMessageListener extension_listener("huhi-extension-enabled",
        false);
...
IN_PROC_BROWSER_TEST_F(HuhiWalletAPIBrowserTest, DappDetectionTestAccept) {
