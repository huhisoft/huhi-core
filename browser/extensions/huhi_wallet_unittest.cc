...
class HuhiWalletUnitTest : public testing::Test {
 public:
  HuhiWalletUnitTest()
      : testing_profile_manager_(TestingBrowserProcess::GetGlobal()) {
  }
  ~HuhiWalletUnitTest() override {}
...
TEST_F(HuhiWalletUnitTest, TestGetRandomNonce) {
  std::string nonce = HuhiWalletService::GetRandomNonce();
  ASSERT_EQ(nonce.size(), HuhiWalletService::kNonceByteLength);
}

TEST_F(HuhiWalletUnitTest, TestGetRandomSeed) {
  std::string seed = HuhiWalletService::GetRandomSeed();
  ASSERT_EQ(seed.size(), HuhiWalletService::kSeedByteLength);
}

TEST_F(HuhiWalletUnitTest, TestGetEthereumRemoteClientSeedFromRootSeed) {
  const char seed[32] = {
...
TEST_F(HuhiWalletUnitTest, TestBitGoSeedFromRootSeed) {
  ...
  std::string derived =
    HuhiWalletService::GetBitGoSeedFromRootSeed(
        std::string(seed, base::size(seed)));
  ASSERT_EQ(derived, std::string(expected_derived_seed,
      base::size(expected_derived_seed)));
}

TEST_F(HuhiWalletUnitTest, TestSealSeed) {
  const char seed[32] = {
    48, 196, 56, 174, 243, 75, 120, 235,
    37, 174, 254, 97, 37, 205, 101, 93,
    181, 23, 190, 82, 53, 180, 51, 198,
  ...
  ASSERT_TRUE(HuhiWalletService::SealSeed(
...
TEST_F(HuhiWalletUnitTest, TestOpenSeed) {
  ...
  std::string seed;
  ASSERT_TRUE(HuhiWalletService::OpenSeed(
...
TEST_F(HuhiWalletUnitTest, TestLoadFromPrefs) {
  ...
  std::string cipher_seed;
  std::string nonce;
  ASSERT_TRUE(HuhiWalletService::LoadFromPrefs(
...
TEST_F(HuhiWalletUnitTest, TestSaveToPrefs) {
  const char nonce[12] = {
    200, 153, 224, 40, 58, 249, 156, 33, 152, 207, 177, 12
  };
  const char cipher_seed[48] = {
  ...
  HuhiWalletService::SaveToPrefs(
      ProfileManager::GetActiveUserProfile()->GetPrefs(),
      std::string(cipher_seed, base::size(cipher_seed)),
      std::string(nonce, base::size(nonce)));

  ASSERT_EQ(GetPrefs()->GetString(kHuhiWalletAES256GCMSivNonce),
      "yJngKDr5nCGYz7EM");
  ASSERT_EQ(GetPrefs()->GetString(kHuhiWalletEncryptedSeed),
      "IQu5fUMbXG6E7v8ITwcIKL3TI3rst0LU1US7ZxCKpgAGgLNAN6DbCN7nMF2Eg7Kx");
}
