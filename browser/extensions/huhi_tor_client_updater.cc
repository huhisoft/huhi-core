...
#if defined(OS_WIN)
const char kTorClientComponentName[] = "Huhi Tor Client Updater (Windows)";
...
const char kTorClientComponentBase64PublicKey[] =
    ...
    "rQIDAQAB";
#elif defined(OS_MAC)
const char kTorClientComponentName[] = "Huhi Tor Client Updater (Mac)";
...
const char kTorClientComponentName[] = "Huhi Tor Client Updater (Linux)";
...
std::string HuhiTorClientUpdater::g_tor_client_component_id_(
    kTorClientComponentId);
std::string HuhiTorClientUpdater::g_tor_client_component_base64_public_key_(
    kTorClientComponentBase64PublicKey);

HuhiTorClientUpdater::HuhiTorClientUpdater(HuhiComponent::Delegate* delegate)
...
HuhiTorClientUpdater::~HuhiTorClientUpdater() {
}

void HuhiTorClientUpdater::Register() {
  ...
  HuhiComponent::Register(kTorClientComponentName,
                           g_tor_client_component_id_,
                           g_tor_client_component_base64_public_key_);
  registered_ = true;
}

void HuhiTorClientUpdater::Unregister() {
  // We don't call HuhiComponent::Unregister here in order to prevent tor
  // executable component from getting deleted when last tor window closed
  registered_ = false;
}

void HuhiTorClientUpdater::Cleanup() {
...
void HuhiTorClientUpdater::SetExecutablePath(const base::FilePath& path) {
  executable_path_ = path;
  for (Observer& observer : observers_)
    observer.OnExecutableReady(path);
}

base::FilePath HuhiTorClientUpdater::GetExecutablePath() const {
  return executable_path_;
}

void HuhiTorClientUpdater::OnComponentReady(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&HuhiTorClientUpdater::SetExecutablePath,
                     weak_ptr_factory_.GetWeakPtr()));
}

void HuhiTorClientUpdater::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void HuhiTorClientUpdater::RemoveObserver(Observer* observer) {
  observers_.RemoveObserver(observer);
}

// static
void HuhiTorClientUpdater::SetComponentIdAndBase64PublicKeyForTest(
    const std::string& component_id,
