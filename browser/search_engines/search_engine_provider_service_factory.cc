...
#include "huhi/browser/search_engines/private_window_search_engine_provider_service.h"
#include "huhi/browser/search_engines/search_engine_provider_util.h"
#include "huhi/browser/search_engines/tor_window_search_engine_provider_service.h"
...
KeyedService* InitializeSearchEngineProviderServiceIfNeeded(Profile* profile) {
  ...
  if (huhi::IsTorProfile(profile) && profile->IsOffTheRecord()) {
  ...
  if (huhi::IsGuestProfile(profile) && profile->IsOffTheRecord()) {
