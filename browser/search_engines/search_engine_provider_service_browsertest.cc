...
#if BUILDFLAG(ENABLE_TOR)
    g_huhi_browser_process->tor_client_updater()->SetExecutablePath(
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  EXPECT_FALSE(huhi::UseAlternativeSearchEngineProviderEnabled(profile));
  ...
  huhi::ToggleUseAlternativeSearchEngineProvider(profile);
  EXPECT_TRUE(huhi::UseAlternativeSearchEngineProviderEnabled(profile));
...
// https://github.com/huhisoft/huhi-browser/issues/1452
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  huhi::ToggleUseAlternativeSearchEngineProvider(private_window_2->profile());
}
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  EXPECT_TRUE(huhi::IsTorProfile(tor_profile));

  auto* service = TemplateURLServiceFactory::GetForProfile(tor_profile);

  int default_provider_id = huhi::IsRegionForQwant(tor_profile) ?
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  int default_provider_id =
      huhi::IsRegionForQwant(tor_profile)
          ? TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_QWANT
          : TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_DUCKDUCKGO;
  auto* service = TemplateURLServiceFactory::GetForProfile(tor_profile);
  EXPECT_EQ(service->GetDefaultSearchProvider()->data().prepopulate_id,
...
IN_PROC_BROWSER_TEST_F(SearchEngineProviderServiceTest,
  ...
  if (huhi::IsRegionForQwant(guest_profile))
    return;
  ...
  EXPECT_FALSE(huhi::UseAlternativeSearchEngineProviderEnabled(guest_profile));

  huhi::ToggleUseAlternativeSearchEngineProvider(guest_profile);
  EXPECT_TRUE(huhi::UseAlternativeSearchEngineProviderEnabled(guest_profile));
  int provider_id =
      TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_DUCKDUCKGO;
