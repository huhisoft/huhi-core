...
IN_PROC_BROWSER_TEST_F(HuhiBrowserCommandControllerTest,
                       HuhiCommandsEnableTest) {
  // Test normal browser's huhi commands status.
  auto* command_controller = browser()->command_controller();
#if BUILDFLAG(HUHI_REWARDS_ENABLED)
  EXPECT_TRUE(command_controller->IsCommandEnabled(IDC_SHOW_HUHI_REWARDS));
#else
  EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_HUHI_REWARDS));
#endif
  ...
  if (switches::IsSyncAllowedByFlag())
    EXPECT_TRUE(command_controller->IsCommandEnabled(IDC_SHOW_HUHI_SYNC));
  else
    EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_HUHI_SYNC));
#else
  EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_HUHI_SYNC));
#endif

#if BUILDFLAG(HUHI_WALLET_ENABLED)
  EXPECT_TRUE(command_controller->IsCommandEnabled(IDC_SHOW_HUHI_WALLET));
#else
  EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_HUHI_WALLET));
#endif
  ...
  // Create private browser and test its huhi commands status.
  ...
  // Create guest browser and test its huhi commands status.
