...
bool HuhiActionViewController::IsEnabled(
...
bool HuhiActionViewController::DisabledClickOpensMenu() const {
  // disabled is a per-tab state
  return false;
}

ui::MenuModel* HuhiActionViewController::GetContextMenu() {
  // no context menu for huhi actions button
  return nullptr;
}

bool HuhiActionViewController::ExecuteActionUI(
...
HuhiActionViewController::GetPreferredPopupViewController() {
  return this;
}

bool HuhiActionViewController::TriggerPopupWithUrl(
...
void HuhiActionViewController::OnPopupClosed() {
  popup_host_observer_.Remove(popup_host_);
  popup_host_ = nullptr;
  view_delegate_->OnPopupClosed();
}

gfx::Image HuhiActionViewController::GetIcon(
...
std::unique_ptr<HuhiActionIconWithBadgeImageSource>
HuhiActionViewController::GetIconImageSource(
  content::WebContents* web_contents, const gfx::Size& size) {
  int tab_id = sessions::SessionTabHelper::IdForTab(web_contents).id();
  // generate icon
  std::unique_ptr<HuhiActionIconWithBadgeImageSource> image_source(
      new HuhiActionIconWithBadgeImageSource(size));
