...
#include "huhi/components/huhi_rewards/resources/grit/huhi_rewards_tip_generated_map.h"
...
class RewardsTipDOMHandler : public WebUIMessageHandler,
 ...
 private:
  ...
  void OnRecurringTipSaved(huhi_rewards::RewardsService* rewards_service,
                           bool success) override;

  void OnRecurringTipRemoved(huhi_rewards::RewardsService* rewards_service,
                             bool success) override;
...
void RewardsTipDOMHandler::OnGetRewardsParameters(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "huhi_rewards_tip.rewardsParameters", data);
}
...
void RewardsTipDOMHandler::OnGetRecurringTips(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "huhi_rewards_tip.recurringTips",
      *publishers);
}
...
void RewardsTipDOMHandler::OnPublisherBanner(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "huhi_rewards_tip.publisherBanner", result);
}

}  // namespace

HuhiTipUI::HuhiTipUI(content::WebUI* web_ui, const std::string& name)
  ...
  content::WebUIDataSource* data_source = CreateBasicUIHTMLSource(profile,
                                              name,
                                              kHuhiRewardsTipGenerated,
                                              kHuhiRewardsTipGeneratedSize,
                                              IDR_HUHI_REWARDS_TIP_HTML);
...
void RewardsTipDOMHandler::OnReconcileStamp(uint64_t reconcile_stamp) {
  ...
  web_ui()->CallJavascriptFunctionUnsafe("huhi_rewards_tip.reconcileStamp",
      base::Value(stamp));
}
...
void RewardsTipDOMHandler::OnRecurringTipRemoved(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "huhi_rewards_tip.recurringTipRemoved", base::Value(success));
}
...
void RewardsTipDOMHandler::OnRecurringTipSaved(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "huhi_rewards_tip.recurringTipSaved", base::Value(success));
}
...
void RewardsTipDOMHandler::TweetTip(const base::ListValue *args) {
  ...
  std::string comment = l10n_util::GetStringFUTF8(
      IDS_HUHI_REWARDS_LOCAL_COMPLIMENT_TWEET, base::UTF8ToUTF16(name));
  std::string hashtag = l10n_util::GetStringUTF8(
      IDS_HUHI_REWARDS_LOCAL_COMPLIMENT_TWEET_HASHTAG);
...
void RewardsTipDOMHandler::OnFetchBalance(
  ...
  web_ui()->CallJavascriptFunctionUnsafe("huhi_rewards_tip.balance", data);
}
...
void RewardsTipDOMHandler::OnExternalWallet(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "huhi_rewards_tip.externalWallet", data);
}
...
void RewardsTipDOMHandler::OnReconcileComplete(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "huhi_rewards_tip.reconcileComplete",
      complete);
}
