...
#include "huhi/browser/ui/webui/settings/huhi_import_data_handler.h"
...
HuhiImportDataHandler::HuhiImportDataHandler() : weak_factory_(this) {}
HuhiImportDataHandler::~HuhiImportDataHandler() = default;

void HuhiImportDataHandler::StartImport(
  ...
  if (source_profile.importer_type == importer::TYPE_SAFARI) {
    // Start import if Huhi has full disk access permission.
    ...
    base::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&HuhiImportDataHandler::OnGetDiskAccessPermission,
...
void HuhiImportDataHandler::OnGetDiskAccessPermission(
    const importer::SourceProfile& source_profile,
    uint16_t imported_items,
    bool allowed) {
  if (!allowed) {
...
void HuhiImportDataHandler::DidStopLoading() {
  Observe(nullptr);
