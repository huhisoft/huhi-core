...
class HuhiNewTabUIBrowserTest : public extensions::ExtensionFunctionalTest {
 public:
...
IN_PROC_BROWSER_TEST_F(HuhiNewTabUIBrowserTest, StartupURLTest) {
...
// This test simply checks that by default the Huhi new tab page is used.
// It does this by loading the newtab page and then checking if
// window.huhi_new_tab exists.
IN_PROC_BROWSER_TEST_F(HuhiNewTabUIBrowserTest, HuhiNewTabIsDefault) {
  auto* contents = browser()->tab_strip_model()->GetActiveWebContents();
  GURL new_tab_url(chrome::kChromeUINewTabURL);
  ui_test_utils::NavigateToURL(browser(), new_tab_url);
  WaitForLoadStop(contents);
  bool is_huhi_new_tab = false;
  ASSERT_TRUE(content::ExecuteScriptAndExtractBool(
      contents,
      "window.domAutomationController.send(!!document.querySelector(`html[data-test-id='huhi-new-tab-page']`))",  // NOLINT
      &is_huhi_new_tab));
  ASSERT_TRUE(is_huhi_new_tab);
}
...
IN_PROC_BROWSER_TEST_F(HuhiNewTabUIBrowserTest, NewTabPageLocationOverride) {
