...
#include "huhi/browser/ui/views/download/huhi_download_item_view.h"
...
void HuhiDownloadItemView::Layout() {
...
gfx::Size HuhiDownloadItemView::CalculatePreferredSize() const {
  ...
  int child_height = file_name_label_->GetLineHeight() +
                     kHuhiVerticalTextPadding +
                     origin_url_font_list_.GetHeight() +
                     kHuhiVerticalTextPadding + status_label_->GetLineHeight();
  if (IsShowingWarningDialog()) {
...
void HuhiDownloadItemView::OnPaint(gfx::Canvas* canvas) {
...
void HuhiDownloadItemView::OnDownloadUpdated() {
  ...
  if (IsShowingWarningDialog() != model()->IsDangerous()) {
    ...
    base::string16 new_origin_url =
        huhi_model_.GetOriginURLText(&new_is_secure);
  ...
  // Update tooltip.
  base::string16 new_tip = huhi_model_.GetTooltipText();
...
int HuhiDownloadItemView::GetYForFilenameText() const {
  int text_height = file_name_label_->GetLineHeight();
  if (!origin_url_text_.empty())
    text_height +=
        kHuhiVerticalTextPadding + origin_url_font_list_.GetHeight();
  if (status_label_ && !status_label_->GetText().empty())
    text_height += kHuhiVerticalTextPadding + status_label_->GetLineHeight();
  return (height() - text_height) / 2;
}

int HuhiDownloadItemView::GetYForOriginURLText() const {
  int y = GetYForFilenameText();
  y += (file_name_label_->GetLineHeight() + kHuhiVerticalTextPadding);
  return y;
}

int HuhiDownloadItemView::GetYForStatusText() const {
  int y = GetYForOriginURLText();
  if (!origin_url_text_.empty())
    y += (origin_url_font_list_.GetHeight() + kHuhiVerticalTextPadding);
  return y;
}

// Drawing routines.

void HuhiDownloadItemView::DrawOriginURL(gfx::Canvas* canvas) {
...
void HuhiDownloadItemView::DrawLockIcon(gfx::Canvas* canvas) {
  if (origin_url_text_.empty() || IsShowingWarningDialog())
    return;
