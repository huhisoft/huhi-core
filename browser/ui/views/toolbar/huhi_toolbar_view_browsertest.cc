...
class HuhiToolbarViewTest : public InProcessBrowserTest {
 public:
  HuhiToolbarViewTest() = default;
  ~HuhiToolbarViewTest() override = default;
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(HuhiToolbarViewTest);
};

IN_PROC_BROWSER_TEST_F(HuhiToolbarViewTest,
    AvatarButtonNotShownSingleProfile) {
  EXPECT_EQ(false, is_avatar_button_shown());
}

IN_PROC_BROWSER_TEST_F(HuhiToolbarViewTest, AvatarButtonIsShownGuestProfile) {
  // Open a Guest window.
