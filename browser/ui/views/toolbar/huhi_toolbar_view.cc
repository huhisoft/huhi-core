...
#include "huhi/browser/ui/views/toolbar/speedreader_button.h"
...
#include "huhi/components/speedreader/speedreader_pref_names.h"
#endif
...
bool IsAvatarButtonHideable(Profile* profile) {
  return !huhi::IsTorProfile(profile) && !profile->IsIncognitoProfile() &&
         !profile->IsGuestSession();
}

}  // namespace

HuhiToolbarView::HuhiToolbarView(Browser* browser, BrowserView* browser_view)
    : ToolbarView(browser, browser_view), profile_observer_(this) {}

HuhiToolbarView::~HuhiToolbarView() {}

void HuhiToolbarView::Init() {
  ToolbarView::Init();

  // For non-normal mode, we don't have to more.
  if (display_mode_ != DisplayMode::NORMAL) {
    huhi_initialized_ = true;
    return;
  }
  ...
  edit_bookmarks_enabled_.Init(
      ...
      base::Bind(&HuhiToolbarView::OnEditBookmarksEnabledChanged,
                 base::Unretained(this)));
  // track changes in wide locationbar setting
  location_bar_is_wide_.Init(
      kLocationBarIsWide, profile->GetPrefs(),
      base::Bind(&HuhiToolbarView::OnLocationBarIsWideChanged,
                 base::Unretained(this)));
...
void HuhiToolbarView::OnEditBookmarksEnabledChanged() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
  Update(nullptr);
}

void HuhiToolbarView::OnLocationBarIsWideChanged() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
...
void HuhiToolbarView::OnThemeChanged() {
  ToolbarView::OnThemeChanged();

  if (!huhi_initialized_)
    return;
...
void HuhiToolbarView::OnProfileAdded(const base::FilePath& profile_path) {
  Update(nullptr);
}

void HuhiToolbarView::OnProfileWasRemoved(const base::FilePath& profile_path,
                                           const base::string16& profile_name) {
  Update(nullptr);
}

void HuhiToolbarView::LoadImages() {
...
void HuhiToolbarView::Update(content::WebContents* tab) {
...
void HuhiToolbarView::ShowBookmarkBubble(
    const GURL& url,
    bool already_bookmarked,
    bookmarks::BookmarkBubbleObserver* observer) {
  // Show BookmarkBubble attached to Huhi's bookmark button
...
void HuhiToolbarView::ResetLocationBarBounds() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
...
void HuhiToolbarView::ResetButtonBounds() {
  DCHECK_EQ(DisplayMode::NORMAL, display_mode_);
