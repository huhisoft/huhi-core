...
HuhiWaybackMachineInfoBarContentsView::HuhiWaybackMachineInfoBarContentsView(
...
void HuhiWaybackMachineInfoBarContentsView::OnThemeChanged() {
  ...
  wayback_spot_graphic_->SetImage(
      rb.GetImageSkiaNamed(ui::NativeTheme::GetInstanceForNativeUi()->
          ShouldUseDarkColors() ? IDR_HUHI_WAYBACK_INFOBAR_DARK
                                : IDR_HUHI_WAYBACK_INFOBAR));
}

void HuhiWaybackMachineInfoBarContentsView::ButtonPressed(
    views::Button* sender,
...
void HuhiWaybackMachineInfoBarContentsView::OnWaybackURLFetched(
...
void HuhiWaybackMachineInfoBarContentsView::HideInfobar() {
  ...
  for (size_t i = 0; i < infobar_service->infobar_count(); ++i) {
    ...
    if (infobar->delegate()->GetIdentifier() ==
        HuhiWaybackMachineInfoBarDelegate::WAYBACK_MACHINE_INFOBAR_DELEGATE) {
...
void HuhiWaybackMachineInfoBarContentsView::InitializeChildren() {
  ...
  auto* label = CreateLabel(
      l10n_util::GetStringUTF16(
          IDS_HUHI_WAYBACK_MACHINE_INFOBAR_PAGE_MISSING_TEXT));
  label->SetFontList(
  ...
  label = CreateLabel(l10n_util::GetStringUTF16(
      IDS_HUHI_WAYBACK_MACHINE_INFOBAR_ASK_ABOUT_CHECK_TEXT));
  views_visible_before_checking_.push_back(label);
  label->SetProperty(
      views::kMarginsKey,
      gfx::Insets(ChromeLayoutProvider::Get()->GetDistanceMetric(
  ...
  label = CreateLabel(
      l10n_util::GetStringUTF16(
          IDS_HUHI_WAYBACK_MACHINE_INFOBAR_NOT_AVAILABLE_TEXT));
  ...
  button_ = new HuhiWaybackMachineInfoBarButtonContainer(this);
...
views::Label* HuhiWaybackMachineInfoBarContentsView::CreateLabel(
...
void HuhiWaybackMachineInfoBarContentsView::UpdateChildrenVisibility(
...
SkColor HuhiWaybackMachineInfoBarContentsView::GetColor(int id) const {
  const auto* theme_provider = GetThemeProvider();
  return theme_provider ? theme_provider->GetColor(id)
                        : gfx::kPlaceholderColor;
}

void HuhiWaybackMachineInfoBarContentsView::FetchWaybackURL() {
  button_->StartThrobber();
  wayback_machine_url_fetcher_.Fetch(contents_->GetVisibleURL());
  Layout();
}

void HuhiWaybackMachineInfoBarContentsView::LoadURL(const GURL& url) {
