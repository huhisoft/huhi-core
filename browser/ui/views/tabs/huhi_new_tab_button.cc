...
#include "huhi/browser/ui/views/tabs/huhi_new_tab_button.h"
...
// static
const gfx::Size HuhiNewTabButton::kButtonSize{20, 20};

gfx::Size HuhiNewTabButton::CalculatePreferredSize() const {
  // Overriden so that we use Huhi's custom button size
...
SkPath HuhiNewTabButton::GetBorderPath(const gfx::Point& origin,
                                      float scale,
                                      bool extend_to_top) const {
  // Overriden to use Huhi's non-circular shape
...
void HuhiNewTabButton::PaintIcon(gfx::Canvas* canvas) {
