...
#include "huhi/browser/ui/views/tabs/huhi_alert_indicator.h"
...
class HuhiAlertIndicator::HuhiAlertBackground : public views::Background {
 public:
  explicit HuhiAlertBackground(HuhiAlertIndicator* host_view)
 ...
 private:
  HuhiAlertIndicator* host_view_;

  DISALLOW_COPY_AND_ASSIGN(HuhiAlertBackground);
};

HuhiAlertIndicator::HuhiAlertIndicator(Tab* parent_tab)
    : AlertIndicator(parent_tab) {
  SetBackground(std::make_unique<HuhiAlertBackground>(this));
}

SkColor HuhiAlertIndicator::GetBackgroundColor() const {
...
bool HuhiAlertIndicator::OnMousePressed(const ui::MouseEvent& event) {
  mouse_pressed_ = true;
  SchedulePaint();
...
void HuhiAlertIndicator::OnMouseReleased(const ui::MouseEvent& event) {
...
void HuhiAlertIndicator::OnMouseEntered(const ui::MouseEvent& event) {
  if (IsTabAudioToggleable())
    SchedulePaint();
  AlertIndicator::OnMouseExited(event);
}

void HuhiAlertIndicator::OnMouseExited(const ui::MouseEvent& event) {
  if (IsTabAudioToggleable())
    SchedulePaint();
  AlertIndicator::OnMouseExited(event);
}

bool HuhiAlertIndicator::OnMouseDragged(const ui::MouseEvent& event) {
  if (IsTabAudioToggleable())
    SchedulePaint();
  return AlertIndicator::OnMouseDragged(event);
}

bool HuhiAlertIndicator::IsTabAudioToggleable() const {
