...
#include "huhi/browser/ui/views/bookmarks/bookmark_bar_instructions_view.h"
...
#include "huhi/browser/ui/huhi_view_ids.h"
...
BookmarkBarInstructionsView::BookmarkBarInstructionsView(Browser* browser)
    : instructions_(NULL),
      ...
      browser_(browser) {
  SetID(HUHI_VIEW_ID_BOOKMARK_IMPORT_INSTRUCTION_VIEW);
  instructions_ =
...
void BookmarkBarInstructionsView::UpdateColors() {
  ...
  SkColor text_color = theme_provider->GetColor(
      HuhiThemeProperties::COLOR_BOOKMARK_BAR_INSTRUCTIONS_TEXT);
