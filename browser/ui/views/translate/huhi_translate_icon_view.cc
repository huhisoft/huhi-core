...
HuhiTranslateIconView::HuhiTranslateIconView(
...
void HuhiTranslateIconView::InstallGoogleTranslate() {
  if (!GetWebContents())
    return;
  ...
  scoped_refptr<extensions::WebstoreInstallWithPrompt> installer =
        new extensions::WebstoreInstallWithPrompt(
            ...
            base::BindOnce(&HuhiTranslateIconView::OnInstallResult,
              weak_ptr_factory_.GetWeakPtr()));
  installer->BeginInstall();
}

void HuhiTranslateIconView::OnInstallResult(
    bool success,
...
void HuhiTranslateIconView::UpdateImpl() {
  if (!GetWebContents())
    return;
