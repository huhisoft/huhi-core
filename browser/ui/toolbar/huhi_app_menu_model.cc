...
HuhiAppMenuModel::~HuhiAppMenuModel() = default;

void HuhiAppMenuModel::Build() {
  // Insert huhi items after build chromium items.
  AppMenuModel::Build();
  InsertHuhiMenuItems();
}

void HuhiAppMenuModel::InsertHuhiMenuItems() {
  // Insert & reorder huhi menus based on corresponding commands enable status.
  // If we you want to add/remove from app menu, adjust commands enable status
  // at HuhiBrowserCommandController.
  ...
  // bookmark. Then, insert huhi items.
  ...
  if (IsCommandIdEnabled(IDC_SHOW_HUHI_REWARDS)) {
    InsertItemWithStringIdAt(GetIndexOfHuhiRewardsItem(),
                             IDC_SHOW_HUHI_REWARDS,
                             IDS_SHOW_HUHI_REWARDS);
  }

  // Insert wallet menu after download menu.
  if (IsCommandIdEnabled(IDC_SHOW_HUHI_WALLET)) {
    InsertItemWithStringIdAt(GetIndexOfCommandId(IDC_SHOW_DOWNLOADS) + 1,
                             IDC_SHOW_HUHI_WALLET,
                             IDS_SHOW_HUHI_WALLET);
  }

  // Insert sync menu
  if (IsCommandIdEnabled(IDC_SHOW_HUHI_SYNC)) {
    InsertItemWithStringIdAt(GetIndexOfHuhiSyncItem(),
                             IDC_SHOW_HUHI_SYNC,
                             IDS_SHOW_HUHI_SYNC);
  }

  // Insert adblock menu at last. Assumed this is always enabled.
  DCHECK(IsCommandIdEnabled(IDC_SHOW_HUHI_ADBLOCK));
  InsertItemWithStringIdAt(GetIndexOfHuhiAdBlockItem(),
                           IDC_SHOW_HUHI_ADBLOCK,
                           IDS_SHOW_HUHI_ADBLOCK);
  ...
  InsertItemWithStringIdAt(GetIndexOfCommandId(IDC_ABOUT),
                           ...
                           IDS_SHOW_HUHI_WEBCOMPAT_REPORTER);
}

int HuhiAppMenuModel::GetIndexOfHuhiAdBlockItem() const {
  // Insert as a last item in second section.
  int adblock_item_index = -1;
  adblock_item_index = GetIndexOfCommandId(IDC_SHOW_HUHI_SYNC);
  ...
  adblock_item_index = GetIndexOfCommandId(IDC_SHOW_HUHI_WALLET);
  if (adblock_item_index != -1)
    return adblock_item_index + 1;
...
int HuhiAppMenuModel::GetIndexOfHuhiRewardsItem() const {
...
int HuhiAppMenuModel::GetIndexOfHuhiSyncItem() const {
  ...
  sync_index = GetIndexOfCommandId(IDC_SHOW_HUHI_WALLET);
  if (sync_index != -1)
    return sync_index + 1;
