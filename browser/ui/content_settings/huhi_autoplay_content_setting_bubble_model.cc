...
#include "huhi/browser/ui/content_settings/huhi_autoplay_content_setting_bubble_model.h"
...
HuhiAutoplayContentSettingBubbleModel::HuhiAutoplayContentSettingBubbleModel(
    Delegate* delegate,
...
HuhiAutoplayContentSettingBubbleModel::
~HuhiAutoplayContentSettingBubbleModel() {}

void HuhiAutoplayContentSettingBubbleModel::CommitChanges() {
  if (settings_changed()) {
...
bool HuhiAutoplayContentSettingBubbleModel::settings_changed() const {
  return selected_item() != bubble_content().radio_group.default_item;
}

void HuhiAutoplayContentSettingBubbleModel::SetTitle() {
  set_title(l10n_util::GetStringUTF16(IDS_BLOCKED_AUTOPLAY_TITLE));
}

void HuhiAutoplayContentSettingBubbleModel::SetRadioGroup() {
...
void HuhiAutoplayContentSettingBubbleModel::SetNarrowestContentSetting(
    ContentSetting setting) {
  if (!GetProfile())
    return;
