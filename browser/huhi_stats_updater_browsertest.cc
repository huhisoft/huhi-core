...
class HuhiStatsUpdaterBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void SetBaseUpdateURLForTest() {
    ...
    huhi::HuhiStatsUpdater::SetBaseUpdateURLForTest(
        embedded_test_server()->GetURL("/1/usage/huhi-core").spec());
...
IN_PROC_BROWSER_TEST_F(HuhiStatsUpdaterBrowserTest,
  ...
  // code file
  auto referrals_service = huhi::HuhiReferralsServiceFactory::GetInstance()
    ->GetForPrefs(GetLocalState());
  referrals_service->SetReferralInitializedCallbackForTest(base::BindRepeating(
      &HuhiStatsUpdaterBrowserTest::OnReferralInitialized,
  ...
  huhi::HuhiStatsUpdater stats_updater(GetLocalState());
  stats_updater.SetStatsUpdatedCallback(base::BindRepeating(
      &HuhiStatsUpdaterBrowserTest::OnStatsUpdated, base::Unretained(this)));
  stats_updater.Start();
