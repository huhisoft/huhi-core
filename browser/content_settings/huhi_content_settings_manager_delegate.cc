...
#include "huhi/browser/content_settings/huhi_content_settings_manager_delegate.h"
...
bool ShouldStoreState(int render_process_id,
  ...
  return g_huhi_browser_process->tracking_protection_service()
...
HuhiContentSettingsManagerDelegate::~HuhiContentSettingsManagerDelegate() =
    default;

// content_settings::ContentSettingsManagerImpl::Delegate:
bool HuhiContentSettingsManagerDelegate::AllowStorageAccess(
...
HuhiContentSettingsManagerDelegate::Clone() {
  return std::make_unique<HuhiContentSettingsManagerDelegate>();
}
