...
// to huhi profile names (Profile X).
IN_PROC_BROWSER_TEST_F(HuhiProfileManagerTest, PRE_MigrateProfileNames) {
...
IN_PROC_BROWSER_TEST_F(HuhiProfileManagerTest, MigrateProfileNames) {
  ProfileManager* profile_manager = g_browser_process->profile_manager();
  ProfileAttributesStorage& storage =
      profile_manager->GetProfileAttributesStorage();
  auto profile_data = GetTestProfileData(profile_manager);
...
IN_PROC_BROWSER_TEST_F(HuhiProfileManagerTest,
  ...
  ASSERT_TRUE(huhi::IsTorProfile(tor_profile));
  EXPECT_TRUE(tor_profile->IsOffTheRecord());
  EXPECT_EQ(huhi::GetParentProfile(tor_profile), parent_profile);
...
IN_PROC_BROWSER_TEST_F(HuhiProfileManagerTest,
  ...
  ContentSetting setting = GetScriptSetting(tor_content_settings, huhi_url);
  ...
  setting = GetScriptSetting(tor_content_settings, huhi_url);
  ...
  SetScriptSetting(tor_content_settings,
                   ContentSettingsPattern::FromURL(huhi_url),
                   CONTENT_SETTING_BLOCK);
  setting = GetScriptSetting(parent_content_settings, huhi_url);
  EXPECT_EQ(setting, CONTENT_SETTING_ALLOW);
  setting = GetScriptSetting(tor_content_settings, huhi_url);
  EXPECT_EQ(setting, CONTENT_SETTING_BLOCK);
}
...
class HuhiProfileManagerExtensionTest
 ...
 public:
  void SetUpOnMainThread() override {
    ...
    base::PathService::Get(huhi::DIR_TEST_DATA, &test_data_dir_);
  }
...
IN_PROC_BROWSER_TEST_F(HuhiProfileManagerExtensionTest,
