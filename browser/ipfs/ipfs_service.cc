...
#include "huhi/components/ipfs/browser/ipfs_json_parser.h"
...
net::NetworkTrafficAnnotationTag GetNetworkTrafficAnnotationTag() {
  return net::DefineNetworkTrafficAnnotation("ipfs_service", R"(
      semantics {
        ...
        trigger:
          "Triggered by actions in huhi://ipfs."
        data:
      ...
      policy {
        cookies_allowed: NO
        setting:
          "You can enable or disable this feature in huhi://settings."
...
IpfsService::IpfsService(content::BrowserContext* context)
  ...
  // Return early since g_huhi_browser_process and ipfs_client_updater are not
  ...
  // Huhi's path is running (huhi-ipfs), which is leftover from browser
  // crash, send a shutdown request if so.

  if (g_huhi_browser_process)
    g_huhi_browser_process->ipfs_client_updater()->AddObserver(this);
...
base::FilePath IpfsService::GetIpfsExecutablePath() {
  return g_huhi_browser_process->ipfs_client_updater()->GetExecutablePath();
}
...
void IpfsService::OnExecutableReady(const base::FilePath& path) {
  ...
  if (g_huhi_browser_process)
    g_huhi_browser_process->ipfs_client_updater()->RemoveObserver(this);
...
void IpfsService::LaunchIfNotRunning(const base::FilePath& executable_path) {
  ...
  base::FilePath data_root_path =
    user_data_dir.Append(FILE_PATH_LITERAL("huhi_ipfs"));
