...
class HuhiStatsUpdaterTest: public testing::Test {
 public:
  ...
  void SetUp() override {
    huhi_stats::RegisterLocalStatePrefs(
  ...
  void SetCurrentTimeForTest(const base::Time& current_time) {
    huhi_stats::HuhiStatsUpdaterParams::SetCurrentTimeForTest(current_time);
  }
...
TEST_F(HuhiStatsUpdaterTest, IsDailyUpdateNeededLastCheckedYesterday) {
  ...
  huhi_stats::HuhiStatsUpdaterParams huhi_stats_updater_params(
...
TEST_F(HuhiStatsUpdaterTest, GetIsoWeekNumber) {
  ...
  EXPECT_EQ(huhi_stats::GetIsoWeekNumber(time), 31);

  exploded.day_of_month = 30;
  exploded.month = 9;

  ASSERT_TRUE(base::Time::FromLocalExploded(exploded, &time));
  EXPECT_EQ(huhi_stats::GetIsoWeekNumber(time), 40);
  ...
  ASSERT_TRUE(base::Time::FromLocalExploded(exploded, &time));
  EXPECT_EQ(huhi_stats::GetIsoWeekNumber(time), 35);
}
