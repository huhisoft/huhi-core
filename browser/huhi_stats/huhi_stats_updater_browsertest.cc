...
class HuhiStatsUpdaterBrowserTest : public InProcessBrowserTest {
 public:
  void SetUpOnMainThread() override {
    InProcessBrowserTest::SetUpOnMainThread();
    huhi_stats::RegisterLocalStatePrefs(testing_local_state_.registry());
    ...
    huhi_stats::HuhiStatsUpdaterParams::SetFirstRunForTest(true);
  }
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    command_line->AppendSwitchASCII(
        huhi_stats::switches::kHuhiStatsUpdaterServer,
        embedded_test_server()->base_url().spec());
  }

  void SetBaseUpdateURLForTest() {
    std::unique_ptr<base::Environment> env(base::Environment::Create());
    env->SetVar("HUHI_REFERRALS_SERVER",
                embedded_test_server()->host_port_pair().ToString());
    env->SetVar("HUHI_REFERRALS_LOCAL", "1");  // use http for local testing
  }
  ...
  void OnStandardStatsUpdated(const GURL& update_url) {
    ...
    // We get //1/usage/huhi-core here, so ignore the first slash.
    EXPECT_STREQ(update_url.path().c_str() + 1, "/1/usage/huhi-core");
  ...
  void OnThresholdStatsUpdated(const GURL& update_url) {
    ...
    // We get //1/usage/huhi-core-threshold here, so ignore the first slash.
    EXPECT_STREQ(update_url.path().c_str() + 1,
                 "/1/usage/huhi-core-threshold");
    update_url_ = update_url.spec();
    wait_for_callback_loop_->Quit();
  }
...
IN_PROC_BROWSER_TEST_F(HuhiStatsUpdaterBrowserTest,
  ...
  auto referrals_service =
      huhi::HuhiReferralsServiceFactory::GetInstance()->GetForPrefs(
          GetLocalState());
  referrals_service->SetReferralInitializedCallbackForTest(
      base::BindRepeating(&HuhiStatsUpdaterBrowserTest::OnReferralInitialized,
  ...
  huhi_stats::HuhiStatsUpdater stats_updater(GetLocalState());
  stats_updater.SetStatsUpdatedCallback(
      base::BindRepeating(&HuhiStatsUpdaterBrowserTest::OnStandardStatsUpdated,
                          base::Unretained(this)));
  stats_updater.Start();
...
IN_PROC_BROWSER_TEST_F(HuhiStatsUpdaterBrowserTest,
  ...
  stats_updater.SetStatsThresholdCallback(base::BindRepeating(
      &HuhiStatsUpdaterBrowserTest::OnThresholdStatsUpdated,
      base::Unretained(this)));
  stats_updater.Start();
...
IN_PROC_BROWSER_TEST_F(HuhiStatsUpdaterBrowserTest, StatsUpdaterMigration) {
