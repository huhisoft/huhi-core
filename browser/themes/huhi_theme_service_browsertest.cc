...
using HuhiThemeServiceTest = InProcessBrowserTest;
...
class HuhiThemeServiceTestWithoutSystemTheme : public InProcessBrowserTest {
 public:
  HuhiThemeServiceTestWithoutSystemTheme() {
...
IN_PROC_BROWSER_TEST_F(HuhiThemeServiceTestWithoutSystemTheme,
  ...
  auto test_theme_property = HuhiThemeProperties::COLOR_FOR_TEST;
  ...
  EXPECT_EQ(HuhiThemeProperties::kLightColorForTest,
  ...
  EXPECT_EQ(
      dark_mode::HuhiDarkModeType::HUHI_DARK_MODE_TYPE_DARK,
      dark_mode::GetActiveHuhiDarkModeType());
  EXPECT_EQ(HuhiThemeProperties::kDarkColorForTest,
            tp.GetColor(test_theme_property));

  // Test dark theme private
  EXPECT_EQ(HuhiThemeProperties::kPrivateColorForTest,
            tp_private.GetColor(test_theme_property));
}

// Test whether appropriate native/web theme observer is called when huhi theme
// is changed.
IN_PROC_BROWSER_TEST_F(HuhiThemeServiceTest, ThemeObserverTest) {
...
IN_PROC_BROWSER_TEST_F(HuhiThemeServiceTest, SystemThemeChangeTest) {
  ...
  if (dark_mode::SystemDarkModeEnabled()) {
    dark_mode::SetHuhiDarkModeType(
        dark_mode::HuhiDarkModeType::HUHI_DARK_MODE_TYPE_DEFAULT);
...
IN_PROC_BROWSER_TEST_F(HuhiThemeServiceTest, DarkModeChangeByRegTest) {
