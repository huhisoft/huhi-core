...
class HuhiContentBrowserClientTest : public InProcessBrowserTest {
 public:
  void SetUp() override {
  ...
  void SetUpOnMainThread() override {
    ...
    extension_url_ = GURL(
        ...
        "huhi_webtorrent.html?magnet%3A%3Fxt%3Durn%3Abtih%"
    ...
    torrent_extension_url_ = GURL(
        "chrome-extension://lgjmpdmojkpocjcopdikifhejkkjglho/extension/"
        "huhi_webtorrent.html?https://webtorrent.io/torrents/"
        "sintel.torrent#ix=5");
    torrent_invalid_query_extension_url_ = GURL(
        "chrome-extension://lgjmpdmojkpocjcopdikifhejkkjglho/extension/"
        "huhi_webtorrent.html?chrome://settings");
  }
...
IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientTest, CanLoadChromeURL) {
...
IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientTest, CanLoadCustomHuhiPages) {
  std::vector<std::string> pages {
    "adblock",
...
IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientTest, CanLoadAboutHost) {
  std::vector<std::string> schemes {
    "chrome://",
    "huhi://",
  };
...
IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientTest,
                           RewriteChromeSync) {
  std::vector<std::string> schemes {
    "huhi://",
    "chrome://",
  ...
  for (const std::string& scheme : schemes) {
    ...
    EXPECT_STREQ(contents->GetController().GetLastCommittedEntry()
                 ->GetURL().spec().c_str(),
                 "chrome://settings/huhiSync");
  }
}

IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientTest, RewriteMagnetURLURLBar) {
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
...
IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientTest, RewriteMagnetURLLink) {
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
...
IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientTest, TypedMagnetURL) {
...
IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientTest,
  ...
  ASSERT_FALSE(
      registry->enabled_extensions().Contains(huhi_webtorrent_extension_id));
...
class HuhiContentBrowserClientReferrerTest
    : public HuhiContentBrowserClientTest {
 public:
...
IN_PROC_BROWSER_TEST_F(HuhiContentBrowserClientReferrerTest,
                       DefaultBehaviour) {
  ...
  content_settings()->SetContentSettingCustomScope(
      ...
      huhi_shields::kReferrers, CONTENT_SETTING_ALLOW);
  referrer = kReferrer.Clone();
  client()->MaybeHideReferrer(browser()->profile(),
                              kRequestUrl, kDocumentUrl, true, "GET",
                              &referrer);
