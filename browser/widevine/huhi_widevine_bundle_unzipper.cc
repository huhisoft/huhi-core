...
// static
scoped_refptr<HuhiWidevineBundleUnzipper>
HuhiWidevineBundleUnzipper::Create(
  ...
  return base::WrapRefCounted(
      new HuhiWidevineBundleUnzipper(file_task_runner,
                                      std::move(done_callback)));
}

void HuhiWidevineBundleUnzipper::LoadFromZipFileInDir(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&HuhiWidevineBundleUnzipper::OnGetTempDirForUnzip, this));
}

void HuhiWidevineBundleUnzipper::OnGetTempDirForUnzip(
  ...
  unzip::UnzipWithFilter(
      ...
      base::BindOnce(&HuhiWidevineBundleUnzipper::OnUnzippedInTempDir, this));
}

void HuhiWidevineBundleUnzipper::OnUnzippedInTempDir(bool status) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  ...
  base::PostTaskAndReplyWithResult(
      ...
      FROM_HERE,
      base::BindOnce(
          &HuhiWidevineBundleUnzipper::MoveUnzippedLibFromTempToTargetDir,
          this),
      base::BindOnce(&HuhiWidevineBundleUnzipper::UnzipDone, this));
}

std::string HuhiWidevineBundleUnzipper::MoveUnzippedLibFromTempToTargetDir() {
...
void HuhiWidevineBundleUnzipper::UnzipDone(const std::string& error) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  std::move(done_callback_).Run(error);
}

HuhiWidevineBundleUnzipper::HuhiWidevineBundleUnzipper(
    scoped_refptr<base::SequencedTaskRunner> file_task_runner,
    DoneCallback done_callback)
