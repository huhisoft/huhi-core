...
class WidevinePermissionRequestBrowserTest
 ...
 public:
  ...
  HuhiDrmTabHelper* GetHuhiDrmTabHelper() {
    return HuhiDrmTabHelper::FromWebContents(GetActiveWebContents());
  }
...
IN_PROC_BROWSER_TEST_F(WidevinePermissionRequestBrowserTest, VisibilityTest) {
  ...
  auto* drm_tab_helper = GetHuhiDrmTabHelper();
...
IN_PROC_BROWSER_TEST_F(WidevinePermissionRequestBrowserTest, BubbleTest) {
  ...
  GetHuhiDrmTabHelper()->OnWidevineKeySystemAccessRequest();
  content::RunAllTasksUntilIdle();
...
// HuhiWidevineBundleManagerBrowserTest.
...
IN_PROC_BROWSER_TEST_F(WidevinePermissionRequestBrowserTest,
  ...
  auto* bundle_manager =
      g_huhi_browser_process->huhi_widevine_bundle_manager();
