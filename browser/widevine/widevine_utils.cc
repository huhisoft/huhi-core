...
bool IsActiveTabRequestedWidevine() {
  ...
  if (content::WebContents* active_web_contents = GetActiveWebContents()) {
    HuhiDrmTabHelper* drm_helper =
        HuhiDrmTabHelper::FromWebContents(active_web_contents);
...
void EnableWidevineCdmComponent(content::WebContents* web_contents) {
  ...
  RegisterWidevineCdmComponent(g_huhi_browser_process->component_updater());
}
#endif

#if BUILDFLAG(BUNDLE_WIDEVINE_CDM)
void InstallBundleOrRestartBrowser() {
  auto* manager = g_huhi_browser_process->huhi_widevine_bundle_manager();
