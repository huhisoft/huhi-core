...
#include "huhi/browser/widevine/huhi_widevine_bundle_unzipper.h"
...
// static
char HuhiWidevineBundleManager::kWidevineInvalidVersion[] = "";

HuhiWidevineBundleManager::HuhiWidevineBundleManager() : weak_factory_(this) {
}

HuhiWidevineBundleManager::~HuhiWidevineBundleManager() {
}

void HuhiWidevineBundleManager::InstallWidevineBundle(
  ...
  DownloadWidevineBundle(
      ...
      base::BindOnce(&HuhiWidevineBundleManager::OnBundleDownloaded,
                     base::Unretained(this)));
  DeleteDeprecatedWidevineCdmLib();
}

void HuhiWidevineBundleManager::DownloadWidevineBundle(
...
void HuhiWidevineBundleManager::OnBundleDownloaded(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&HuhiWidevineBundleManager::OnGetTargetWidevineBundleDir,
                     weak_factory_.GetWeakPtr(),
                     tmp_bundle_zip_file_path));
}

void HuhiWidevineBundleManager::OnGetTargetWidevineBundleDir(
...
void HuhiWidevineBundleManager::Unzip(
    const base::FilePath& tmp_bundle_zip_file_path,
    const base::FilePath& target_bundle_dir) {
  if (is_test_) return;

  HuhiWidevineBundleUnzipper::Create(
      file_task_runner(),
      base::BindOnce(&HuhiWidevineBundleManager::OnBundleUnzipped,
                     weak_factory_.GetWeakPtr()))
...
void HuhiWidevineBundleManager::OnBundleUnzipped(const std::string& error) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  DVLOG(1) << __func__;
  InstallDone(error);
}

bool HuhiWidevineBundleManager::in_progress() const {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  return in_progress_;
}

void HuhiWidevineBundleManager::set_in_progress(bool in_progress) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
...
bool HuhiWidevineBundleManager::needs_restart() const {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  return needs_restart_;
}

void HuhiWidevineBundleManager::set_needs_restart(bool needs_restart) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
...
void HuhiWidevineBundleManager::InstallDone(const std::string& error) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
...
void HuhiWidevineBundleManager::StartupCheck() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
...
void HuhiWidevineBundleManager::DeleteDeprecatedWidevineCdmLib() {
  if (is_test_) return;
...
void HuhiWidevineBundleManager::ScheduleBackgroundUpdate() {
  base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(&HuhiWidevineBundleManager::DoDelayedBackgroundUpdate,
                     weak_factory_.GetWeakPtr()),
      base::TimeDelta::FromMinutes(GetBackgroundUpdateDelayInMins()));
}

void HuhiWidevineBundleManager::OnBackgroundUpdateFinished(
...
void HuhiWidevineBundleManager::DoDelayedBackgroundUpdate() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  ...
  InstallWidevineBundle(
      base::BindOnce(&HuhiWidevineBundleManager::OnBackgroundUpdateFinished,
                     weak_factory_.GetWeakPtr()),
      false);
}

int
HuhiWidevineBundleManager::GetWidevinePermissionRequestTextFragment() const {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
...
void HuhiWidevineBundleManager::WillRestart() const {
  DCHECK(needs_restart());
  SetWidevinePrefsAsInstalledState();
  DVLOG(1) << __func__;
}
...
HuhiWidevineBundleManager::file_task_runner() {
