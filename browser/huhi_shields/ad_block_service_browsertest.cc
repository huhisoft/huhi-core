...
class AdBlockServiceTest : public ExtensionBrowserTest {
 public:
  ...
  void PreRunTestOnMainThread() override {
    ...
    ASSERT_TRUE(g_huhi_browser_process->ad_block_service()->IsInitialized());
  }
  ...
  void UpdateAdBlockInstanceWithRules(const std::string& rules,
    ...
    g_huhi_browser_process->ad_block_service()->ResetForTest(rules, resources);
  }

  void AssertTagExists(const std::string& tag, bool expected_exists) const {
    bool exists_default =
        g_huhi_browser_process->ad_block_service()->TagExists(tag);
  ...
  void SetDefaultComponentIdAndBase64PublicKeyForTest(
    ...
    huhi_shields::AdBlockService::SetComponentIdAndBase64PublicKeyForTest(
  ...
  void SetRegionalComponentIdAndBase64PublicKeyForTest(
      const std::string& component_id,
      const std::string& component_base64_public_key) {
    huhi_shields::AdBlockRegionalService::
  ...
  bool InstallDefaultAdBlockExtension(
    ...
    g_huhi_browser_process->ad_block_service()->OnComponentReady(
  ...
  bool InstallRegionalAdBlockExtension(const std::string& uuid) {
    ...
    EXPECT_EQ(g_huhi_browser_process->ad_block_regional_service_manager()
  ...
  bool InstallTrackingProtectionExtension() {
    ...
    g_huhi_browser_process->tracking_protection_service()->OnComponentReady(
  ...
  bool StartAdBlockRegionalServices() {
    g_huhi_browser_process->ad_block_regional_service_manager()->Start();
    if (!g_huhi_browser_process->ad_block_regional_service_manager()
             ->IsInitialized())
      return false;
    return true;
  }
  ...
  void WaitForHuhiExtensionShieldsDataReady() {
    ...
    ExtensionTestMessageListener extension_listener(
        "huhi-extension-shields-data-ready",
        false);
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
  ...
  ASSERT_TRUE(g_huhi_browser_process->ad_block_custom_filters_service()
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SocialButttonAdBlockTagTest) {
  UpdateAdBlockInstanceWithRules(
      base::StringPrintf("||example.com^$tag=%s",
                         huhi_shields::kFacebookEmbeds)
          .c_str());
  EXPECT_EQ(browser()->profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);
  GURL tab_url = embedded_test_server()->GetURL("b.com", kAdBlockTestPage);
  g_huhi_browser_process->ad_block_service()->EnableTag(
      huhi_shields::kFacebookEmbeds, true);
  WaitForAdBlockServiceThreads();
  GURL resource_url =
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, ResetPreservesTags) {
  ...
  WaitForAdBlockServiceThreads();
  UpdateAdBlockInstanceWithRules("");
  AssertTagExists(huhi_shields::kFacebookEmbeds, true);
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, TagPrefsControlTags) {
  ...
  AssertTagExists(huhi_shields::kFacebookEmbeds, true);
  AssertTagExists(huhi_shields::kTwitterEmbeds, true);
  AssertTagExists(huhi_shields::kLinkedInEmbeds, false);
  ...
  AssertTagExists(huhi_shields::kFacebookEmbeds, true);
  AssertTagExists(huhi_shields::kTwitterEmbeds, true);
  AssertTagExists(huhi_shields::kLinkedInEmbeds, true);

  prefs->SetBoolean(kFBEmbedControlType, false);
  WaitForAdBlockServiceThreads();
  AssertTagExists(huhi_shields::kFacebookEmbeds, false);
  AssertTagExists(huhi_shields::kTwitterEmbeds, true);
  AssertTagExists(huhi_shields::kLinkedInEmbeds, true);
  ...
  WaitForAdBlockServiceThreads();
  AssertTagExists(huhi_shields::kFacebookEmbeds, false);
  AssertTagExists(huhi_shields::kTwitterEmbeds, false);
  AssertTagExists(huhi_shields::kLinkedInEmbeds, true);
...
class CosmeticFilteringFlagDisabledTest : public AdBlockServiceTest {
 public:
  CosmeticFilteringFlagDisabledTest() {
    feature_list_.InitAndDisableFeature(kHuhiAdblockCosmeticFiltering);
  }
...
IN_PROC_BROWSER_TEST_F(CosmeticFilteringFlagDisabledTest,
  ...
  WaitForHuhiExtensionShieldsDataReady();
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringDisabled) {
  huhi_shields::SetCosmeticFilteringControlType(
      content_settings(), huhi_shields::ControlType::ALLOW, GURL());
  UpdateAdBlockInstanceWithRules(
      "b.com###ad-banner\n"
      "##.ad");
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringSimple) {
  ...
  ASSERT_EQ(true,
            EvalJs(contents, "checkSelector('#ad-banner', 'display', 'none')"));

  ASSERT_EQ(true, EvalJs(contents,
                         "checkSelector('.ad-banner', 'display', 'block')"));

  ASSERT_EQ(true, EvalJs(contents, "checkSelector('.ad', 'display', 'none')"));
}
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, CosmeticFilteringHide1pContent) {
  huhi_shields::SetCosmeticFilteringControlType(
      content_settings(), huhi_shields::ControlType::BLOCK, GURL());
  UpdateAdBlockInstanceWithRules("b.com##.fpsponsored\n");
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
  ...
  ASSERT_EQ(true,
            EvalJs(contents, "checkSelector('.ad', 'padding-bottom', '10px')"));
}
