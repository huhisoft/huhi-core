...
#if BUILDFLAG(ENABLE_HUHI_SYNC) && !defined(OS_ANDROID)
...
void HuhiBrowserMainParts::PostBrowserStart() {
  ...
  if (browser) {
    ...
    if (active_web_contents) {
      ...
      if (infobar_service) {
        HuhiConfirmP3AInfoBarDelegate::Create(
      ...
      SyncV2MigrateInfoBarDelegate::Create(infobar_service, is_v2_user,
          profile(), browser);
#endif  // BUILDFLAG(ENABLE_HUHI_SYNC)
      }
    }
  }
...
void HuhiBrowserMainParts::PreShutdown() {
  content::HuhiClearBrowsingData::ClearOnExit();
}

void HuhiBrowserMainParts::PreProfileInit() {
  ChromeBrowserMainParts::PreProfileInit();
#if !defined(OS_ANDROID)
  auto* command_line = base::CommandLine::ForCurrentProcess();
  if (!base::FeatureList::IsEnabled(huhi_sync::features::kHuhiSync)) {
...
void HuhiBrowserMainParts::PostProfileInit() {
