...
std::string HuhiStatsUpdater::g_base_update_url_(
    "https://laptop-updates.huhisoft.com/1/usage/huhi-core");
...
void HuhiStatsUpdater::OnServerPingTimerFired() {
  ...
  std::string today_ymd = huhi::GetDateAsYMD(base::Time::Now());
...
void HuhiStatsUpdater::SendServerPing() {
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("huhi_stats_updater", R"(
        semantics {
          sender:
            "Huhi Stats Updater"
          description:
  ...
  auto stats_updater_params =
      std::make_unique<huhi::HuhiStatsUpdaterParams>(pref_service_);
  ...
  resource_request->headers.SetHeader("X-Huhi-API-Key", huhi::GetAPIKey());
...
// static
void HuhiStatsUpdater::SetBaseUpdateURLForTest(
