...
constexpr char kTestOnionURL[] = "https://huhi.onion";
 ...
 public:
  ...
  void CheckOnionLocationLabel(Browser* browser) {
    ...
    views::LabelButton* onion_button =
        huhi_location_bar_view->GetOnionLocationView()->GetButton();
...
IN_PROC_BROWSER_TEST_F(OnionLocationNavigationThrottleBrowserTest,
  ...
  ui_test_utils::NavigateToURL(browser(), GURL("https://hnq.vn"));
  EXPECT_EQ(1U, browser_list->size());
  ASSERT_FALSE(huhi::IsTorProfile(browser_list->get(0)->profile()));
  ...
  ASSERT_TRUE(huhi::IsTorProfile(browser_list->get(1)->profile()));
  content::WebContents* web_contents =
...
IN_PROC_BROWSER_TEST_F(OnionLocationNavigationThrottleBrowserTest,
  ...
  ASSERT_TRUE(huhi::IsTorProfile(tor_browser->profile()));
  EXPECT_EQ(2U, browser_list->size());
