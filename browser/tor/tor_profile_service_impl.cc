...
#include "huhi/browser/tor/tor_profile_service_impl.h"
...
#include "huhi/net/proxy_resolution/proxy_config_service_tor.h"
...
TorProfileServiceImpl::TorProfileServiceImpl(Profile* profile)
  ...
  // Return early since g_huhi_browser_process and tor_client_updater are not
  ...
  if (g_huhi_browser_process) {
    g_huhi_browser_process->tor_client_updater()->AddObserver(this);
  }
}
...
TorProfileServiceImpl::~TorProfileServiceImpl() {
  ...
  if (g_huhi_browser_process) {
    g_huhi_browser_process->tor_client_updater()->RemoveObserver(this);
  }
}
