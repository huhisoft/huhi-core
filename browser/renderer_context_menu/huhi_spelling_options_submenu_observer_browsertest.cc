...
// accesses resources.
class HuhiSpellingOptionsSubMenuObserverTest : public InProcessBrowserTest {
 public:
  HuhiSpellingOptionsSubMenuObserverTest() {}
  ~HuhiSpellingOptionsSubMenuObserverTest() override {}
  ...
  void Reset(bool incognito = false,
             HuhiSpellingOptionsSubMenuObserver::GtestMode gtest_mode =
  ...
  void CheckExpected() {
    for (size_t i = 0; i < menu()->GetMenuSize(); ++i) {
      HuhiMockRenderViewContextMenu::MockMenuItem item;
  ...
  HuhiMockRenderViewContextMenu* menu() { return menu_.get(); }
  SpellingOptionsSubMenuObserver* observer() { return observer_.get(); }

 private:
  std::unique_ptr<HuhiMockRenderViewContextMenu> menu_;
  std::unique_ptr<SpellingOptionsSubMenuObserver> observer_;

  DISALLOW_COPY_AND_ASSIGN(HuhiSpellingOptionsSubMenuObserverTest);
};

// Tests that "Ask Huhi for suggestions" isn't shown in the menu and the menu
// doesn't end with a separator.

IN_PROC_BROWSER_TEST_F(HuhiSpellingOptionsSubMenuObserverTest,
                       CheckAskHuhiNotShown) {
  ...
  // Test empty submenu.
  Reset(false, HuhiSpellingOptionsSubMenuObserver::GTEST_MODE_EMPTY_SUBMENU);
  InitMenu(false, "en-US", std::vector<std::string>());
