...
base::string16 CryptoWalletsInfoBarDelegate::GetButtonLabel(
  ...
  if (subtype_ == InfobarSubType::LOAD_CRYPTO_WALLETS) {
    if (button == BUTTON_CANCEL) {
      return l10n_util::GetStringUTF16(IDS_HUHI_CRYPTO_WALLETS_SETTINGS);
    }
    return l10n_util::GetStringUTF16(
        IDS_HUHI_CRYPTO_WALLETS_START_AND_RELOAD);
  }
  ...
  if (subtype_ == InfobarSubType::LOAD_CRYPTO_WALLETS) {
    ...
    if (web_contents) {
      ...
      service->LoadCryptoWalletsExtension(base::BindOnce(
          &CryptoWalletsInfoBarDelegate::OnCryptoWalletsLoaded,
