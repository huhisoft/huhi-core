...
bool IsAdBlockEnabled(content::WebContents* contents, const GURL& url) {
  ...
  huhi_shields::ControlType control_type =
      huhi_shields::GetAdControlType(map, contents->GetLastCommittedURL());
  if (huhi_shields::GetHuhiShieldsEnabled(map, url) &&
      control_type != huhi_shields::ALLOW) {
    content::RenderFrameHost::AllowInjectingJavaScript();
