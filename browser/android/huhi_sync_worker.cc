...
#include "huhi/browser/android/huhi_sync_worker.h"
...
HuhiSyncWorker::HuhiSyncWorker(JNIEnv* env,
                                 const base::android::JavaRef<jobject>& obj)
    : weak_java_huhi_sync_worker_(env, obj) {
  Java_HuhiSyncWorker_setNativePtr(env, obj, reinterpret_cast<intptr_t>(this));
...
void HuhiSyncWorker::Destroy(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  delete this;
}

static void JNI_HuhiSyncWorker_DestroyV1LevelDb(
    JNIEnv* env,
  ...
  VLOG(3) << "[HuhiSync] " << __func__ << " destroy DB status is "
          << status.ToString();
}

static void JNI_HuhiSyncWorker_MarkSyncV1WasEnabledAndMigrated(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& obj) {
  ...
  huhi_sync_prefs.SetSyncV1WasEnabled();
  huhi_sync_prefs.SetSyncV1Migrated(true);
  VLOG(3) << "[HuhiSync] " << __func__ << " done";
}

base::android::ScopedJavaLocalRef<jstring> HuhiSyncWorker::GetSyncCodeWords(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
...
void HuhiSyncWorker::SaveCodeWords(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller,
...
syncer::HuhiProfileSyncService* HuhiSyncWorker::GetSyncService() const {
...
void HuhiSyncWorker::RequestSync(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
...
void HuhiSyncWorker::MarkFirstSetupComplete() {
...
void HuhiSyncWorker::FinalizeSyncSetup(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  MarkFirstSetupComplete();
}

bool HuhiSyncWorker::IsFirstSetupComplete(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
...
bool HuhiSyncWorker::GetSyncV1WasEnabled(
  ...
  bool sync_v1_was_enabled = huhi_sync_prefs.IsSyncV1Enabled();
  return sync_v1_was_enabled;
}

bool HuhiSyncWorker::GetSyncV2MigrateNoticeDismissed(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  huhi_sync::Prefs huhi_sync_prefs(profile_->GetPrefs());
  bool sync_v2_migration_notice_dismissed =
      huhi_sync_prefs.IsSyncMigrateNoticeDismissed();
  return sync_v2_migration_notice_dismissed;
}

void HuhiSyncWorker::SetSyncV2MigrateNoticeDismissed(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller,
    bool sync_v2_migration_notice_dismissed) {
  huhi_sync::Prefs huhi_sync_prefs(profile_->GetPrefs());
  huhi_sync_prefs.SetDismissSyncMigrateNotice(
      sync_v2_migration_notice_dismissed);
}

void HuhiSyncWorker::OnResetDone() {
  syncer::SyncService* sync_service = GetSyncService();
  if (sync_service) {
...
void HuhiSyncWorker::OnStateChanged(syncer::SyncService* sync) {
  // Fill SyncConfigInfo as it is done in
  // huhi_sync_subpage.js:handleSyncPrefsChanged_ and then configure encryption
  ...
  if (!service || !service->IsEngineInitialized()) {
    VLOG(3) << "[HuhiSync] " << __func__ << " sync engine is not initialized";
    return;
  }

  if (!FillSyncConfigInfo(service, &configuration, this->passphrase_)) {
    VLOG(3) << "[HuhiSync] " << __func__
...
static void JNI_HuhiSyncWorker_Init(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller) {
  new HuhiSyncWorker(env, jcaller);
}

base::android::ScopedJavaLocalRef<jstring>
JNI_HuhiSyncWorker_GetSeedHexFromWords(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller,
  ...
  if (huhi_sync::crypto::PassphraseToBytes32(str_seed_words, &bytes)) {
...
base::android::ScopedJavaLocalRef<jstring>
JNI_HuhiSyncWorker_GetWordsFromSeedHex(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& jcaller,
  ...
  if (base::HexStringToBytes(str_seed_hex, &bytes)) {
    ...
    if (bytes.size(), SEED_BYTES_COUNT) {
      sync_code_words = huhi_sync::crypto::PassphraseFromBytes32(bytes);
