...
TEST(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIHTTPScheme) {
  ...
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(huhi::ResponseCallback(),
                                                     huhi_request_info);
  EXPECT_EQ(rc, net::OK);
...
TEST(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIIPFSSchemeLocal) {
  ...
  auto huhi_request_info = std::make_shared<huhi::HuhiRequestInfo>(url);
  huhi_request_info->ipfs_local = true;
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(huhi::ResponseCallback(),
                                                     huhi_request_info);
  EXPECT_EQ(rc, net::OK);
  EXPECT_EQ(huhi_request_info->new_url_spec,
...
TEST(IPFSRedirectNetworkDelegateHelperTest, TranslateIPFSURIIPFSScheme) {
  GURL url("ipfs://QmfM2r8seH2GiRaC4esTjeraXEachRt8ZsSeGaWTPLyMoG");
  auto huhi_request_info = std::make_shared<huhi::HuhiRequestInfo>(url);
  huhi_request_info->ipfs_local = false;
  int rc = ipfs::OnBeforeURLRequest_IPFSRedirectWork(huhi::ResponseCallback(),
                                                     huhi_request_info);
  EXPECT_EQ(rc, net::OK);
  EXPECT_EQ(
      huhi_request_info->new_url_spec,
