...
#ifndef HUHI_BROWSER_NET_URL_CONTEXT_H_
#define HUHI_BROWSER_NET_URL_CONTEXT_H_
...
int OnBeforeURLRequest(const huhi::ResponseCallback& next_callback,
                       std::shared_ptr<huhi::HuhiRequestInfo> ctx);
}  // namespace huhi_rewards

namespace huhi {

enum HuhiNetworkDelegateEventType {
  kOnBeforeRequest,
...
struct HuhiRequestInfo {
 ...
 private:
  ...
  friend class ::HuhiRequestHandler;

  GURL* new_url = nullptr;

  DISALLOW_COPY_AND_ASSIGN(HuhiRequestInfo);
};
...
#endif  // HUHI_BROWSER_NET_URL_CONTEXT_H_
