...
#include "huhi/components/huhi_shields/browser/adblock_stub_response.h"
...
HuhiProxyingURLLoaderFactory::InProgressRequest::FollowRedirectParams::
    FollowRedirectParams() = default;
HuhiProxyingURLLoaderFactory::InProgressRequest::FollowRedirectParams::
    ~FollowRedirectParams() = default;

HuhiProxyingURLLoaderFactory::InProgressRequest::InProgressRequest(
    HuhiProxyingURLLoaderFactory* factory,
    uint64_t request_id,
  ...
  target_client_.set_disconnect_handler(base::BindOnce(
      &HuhiProxyingURLLoaderFactory::InProgressRequest::OnRequestError,
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::Restart() {
  UpdateRequestInfo();
  RestartInternal();
}

void HuhiProxyingURLLoaderFactory::InProgressRequest::UpdateRequestInfo() {
  // TODO(iefremov): Update |ctx_| here and get rid of multiple spots where
  // it is refilled.
}

void HuhiProxyingURLLoaderFactory::InProgressRequest::RestartInternal() {
  ...
  base::RepeatingCallback<void(int)> continuation =
      base::BindRepeating(&InProgressRequest::ContinueToBeforeSendHeaders,
                          weak_factory_.GetWeakPtr());
  redirect_url_ = GURL();
  ctx_ = huhi::HuhiRequestInfo::MakeCTX(request_, render_process_id_,
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::FollowRedirect(
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::SetPriority(
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::OnReceiveResponse(
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::OnReceiveRedirect(
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::OnUploadProgress(
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::OnReceiveCachedMetadata(
    mojo_base::BigBuffer data) {
  target_client_->OnReceiveCachedMetadata(std::move(data));
}

void HuhiProxyingURLLoaderFactory::InProgressRequest::OnTransferSizeUpdated(
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::OnComplete(
    const network::URLLoaderCompletionStatus& status) {
  UMA_HISTOGRAM_TIMES("Huhi.ProxyingURLLoader.TotalRequestTime",
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::
  ...
  if (ctx_->blocked_by != huhi::kNotBlocked) {
    ...
    huhi_shields::MakeStubResponse(ctx_->mock_data_url, request_, &response,
                                    &response_data);
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::ContinueToStartRequest(
    int error_code) {
  if (error_code != net::OK) {
    OnRequestError(network::URLLoaderCompletionStatus(error_code));
    return;
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::ContinueToSendHeaders(
    int error_code) {
  if (error_code != net::OK) {
    OnRequestError(network::URLLoaderCompletionStatus(error_code));
    return;
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::ContinueToBeforeRedirect(
    const net::RedirectInfo& redirect_info,
    int error_code) {
  if (error_code != net::OK) {
    OnRequestError(network::URLLoaderCompletionStatus(error_code));
...
void HuhiProxyingURLLoaderFactory::InProgressRequest::OnRequestError(
    const network::URLLoaderCompletionStatus& status) {
...
HuhiProxyingURLLoaderFactory::HuhiProxyingURLLoaderFactory(
    HuhiRequestHandler* request_handler,
    content::BrowserContext* browser_context,
    int render_process_id,
    int frame_tree_node_id,
  ...
  target_factory_.set_connection_error_handler(
      base::BindOnce(&HuhiProxyingURLLoaderFactory::OnTargetFactoryError,
                     base::Unretained(this)));

  proxy_receivers_.Add(this, std::move(receiver));
  proxy_receivers_.set_disconnect_handler(
      base::BindRepeating(&HuhiProxyingURLLoaderFactory::OnProxyBindingError,
                          base::Unretained(this)));
}

HuhiProxyingURLLoaderFactory::~HuhiProxyingURLLoaderFactory() = default;

// static
bool HuhiProxyingURLLoaderFactory::MaybeProxyRequest(
    content::BrowserContext* browser_context,
...
void HuhiProxyingURLLoaderFactory::CreateLoaderAndStart(
    mojo::PendingReceiver<network::mojom::URLLoader> loader_receiver,
    int32_t routing_id,
    int32_t request_id,
    uint32_t options,
  ...
  const uint64_t huhi_request_id = request_id_generator_->Generate();

  auto result = requests_.emplace(std::make_unique<InProgressRequest>(
      this, huhi_request_id, request_id, routing_id, render_process_id_,
...
void HuhiProxyingURLLoaderFactory::Clone(
    mojo::PendingReceiver<network::mojom::URLLoaderFactory> loader_receiver) {
  proxy_receivers_.Add(this, std::move(loader_receiver));
}

void HuhiProxyingURLLoaderFactory::OnTargetFactoryError() {
...
void HuhiProxyingURLLoaderFactory::OnProxyBindingError() {
...
void HuhiProxyingURLLoaderFactory::RemoveRequest(InProgressRequest* request) {
...
void HuhiProxyingURLLoaderFactory::MaybeRemoveProxy() {
