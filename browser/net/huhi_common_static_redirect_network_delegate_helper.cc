...
std::string GetUpdateURLHost() {
  ...
  if (!command_line.HasSwitch(huhi_component_updater::kUseGoUpdateDev) &&
      !base::FeatureList::IsEnabled(
          huhi_component_updater::kUseDevUpdaterUrl)) {
...
bool RewriteBugReportingURL(const GURL& request_url, GURL* new_url) {
  GURL url("https://github.com/huhisoft/huhi-browser/issues/new");
  ...
  for (const auto& pair : pairs) {
    if (pair.first == "comment") {
      ...
      base::ReplaceSubstringsAfterOffset(&query, 0, "Chrome", "Huhi");
...
int OnBeforeURLRequest_CommonStaticRedirectWorkForGURL(
  ...
  if (clients4_pattern.MatchesHost(request_url)) {
    replacements.SetSchemeStr("https");
    replacements.SetHostStr(kHuhiClients4Proxy);
    *new_url = request_url.ReplaceComponents(replacements);
    return net::OK;
  }
