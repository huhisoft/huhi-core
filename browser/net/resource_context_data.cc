...
void ResourceContextData::StartProxying(
  ...
  if (!self->request_handler_) {
    self->request_handler_.reset(new HuhiRequestHandler);
  }

  auto proxy = std::make_unique<HuhiProxyingURLLoaderFactory>(
...
// static
HuhiProxyingWebSocket* ResourceContextData::StartProxyingWebSocket(
  ...
  auto proxy = std::make_unique<HuhiProxyingWebSocket>(
...
void ResourceContextData::RemoveProxy(HuhiProxyingURLLoaderFactory* proxy) {
  auto it = proxies_.find(proxy);
  DCHECK(it != proxies_.end());
  proxies_.erase(it);
}

void ResourceContextData::RemoveProxyWebSocket(HuhiProxyingWebSocket* proxy) {
