...
HuhiProxyingWebSocket::HuhiProxyingWebSocket(
...
// static
HuhiProxyingWebSocket* HuhiProxyingWebSocket::ProxyWebSocket(
...
void HuhiProxyingWebSocket::Start() {
  ...
  if (proxy_has_extra_headers()) {
    continuation = base::BindRepeating(
        &HuhiProxyingWebSocket::ContinueToStartRequest,
        weak_factory_.GetWeakPtr());
  } else {
    continuation = base::BindRepeating(
        &HuhiProxyingWebSocket::OnBeforeRequestComplete,
        weak_factory_.GetWeakPtr());
  }
...
HuhiProxyingWebSocket::web_socket_factory() {
  return base::BindOnce(&HuhiProxyingWebSocket::WebSocketFactoryRun,
                        base::Unretained(this));
}

mojo::Remote<network::mojom::WebSocketHandshakeClient>
HuhiProxyingWebSocket::handshake_client() {
  return std::move(forwarding_handshake_client_);
}

bool HuhiProxyingWebSocket::proxy_has_extra_headers() {
  return proxy_trusted_header_client_.is_bound();
}

void HuhiProxyingWebSocket::WebSocketFactoryRun(
    const GURL& url,
...
void HuhiProxyingWebSocket::OnOpeningHandshakeStarted(
...
void HuhiProxyingWebSocket::ContinueToHeadersReceived() {
...
void HuhiProxyingWebSocket::OnConnectionEstablished(
...
void HuhiProxyingWebSocket::OnAuthRequired(
...
void HuhiProxyingWebSocket::OnBeforeSendHeaders(
...
void HuhiProxyingWebSocket::OnHeadersReceived(
...
void HuhiProxyingWebSocket::OnBeforeRequestComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnBeforeSendHeaders(
        request_.headers,
        base::BindOnce(
            &HuhiProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void HuhiProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy(
    int error_code,
...
void HuhiProxyingWebSocket::OnBeforeSendHeadersComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
         !receiver_as_handshake_client_.is_bound());
...
void HuhiProxyingWebSocket::ContinueToStartRequest(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  receiver_as_handshake_client_.set_disconnect_with_reason_handler(
      base::BindOnce(&HuhiProxyingWebSocket::OnMojoConnectionError,
                     base::Unretained(this)));
}

void HuhiProxyingWebSocket::OnHeadersReceivedCompleteFromProxy(
    int error_code,
...
void HuhiProxyingWebSocket::OnHeadersReceivedComplete(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnHeadersReceived(
        ...
        remote_endpoint_,
        base::BindOnce(
            &HuhiProxyingWebSocket::OnHeadersReceivedCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void HuhiProxyingWebSocket::PauseIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Pause();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Pause();
}

void HuhiProxyingWebSocket::ResumeIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Resume();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Resume();
}

void HuhiProxyingWebSocket::OnError(int error_code) {
  if (!is_done_) {
    is_done_ = true;
  }
...
// ResetWithReason
void HuhiProxyingWebSocket::OnMojoConnectionError(
