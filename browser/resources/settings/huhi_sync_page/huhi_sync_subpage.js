...
import './huhi_sync_configure.js';
import './huhi_sync_setup.js';
...
Polymer({
  ...
  */
  ...
  attached: function() {
    ...
    if (router.getCurrentRoute() == router.getRoutes().HUHI_SYNC_SETUP) {
  ...
  detached: function() {
    const router = Router.getInstance();
    if (router.getRoutes().HUHI_SYNC_SETUP.contains(router.getCurrentRoute())) {
  ...
  */
  ...
  onNavigateToPage_: function() {
    const router = Router.getInstance();
    assert(router.getCurrentRoute() == router.getRoutes().HUHI_SYNC_SETUP);
    if (this.beforeunloadCallback_) {
      return;
    }
