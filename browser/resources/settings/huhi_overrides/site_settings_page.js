  ...
  'settings-site-settings-page': [{
    registered: function() {
      if (!this.properties || !this.properties.lists_ || !this.properties.lists_.value) {
        console.error('[Huhi Settings Overrides] Could not find polymer lists_ property')
        return
      }
      ...
      this.properties.lists_.value = function () {
        ...
        if (!lists_) {
          console.error('[Huhi Settings Overrides] did not get lists_ data')
          return
        }
        if (!lists_.permissionsBasic) {
          console.error('[Huhi Settings Overrides] did not get lists_.permissionsBasic data')
        } else {
        ...
        if (!lists_.contentAdvanced) {
          console.error('[Huhi Settings Overrides] did not get lists_.contentAdvanced data')
        } else {
        ...
        if (!lists_.permissionsAdvanced) {
          console.error('[Huhi Settings Overrides] did not get lists_.permissionsAdvanced data')
        } else {
