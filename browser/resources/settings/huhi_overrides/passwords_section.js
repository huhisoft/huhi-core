...
RegisterPolymerTemplateModifications({
  'passwords-section': (templateContent) => {
    ...
    if (!checkPasswordsLinkRow) {
      console.error('[Huhi Settings Overrides] Could not find checkPasswordsLinkRow in passwords_section')
    } else {
      checkPasswordsLinkRow.remove()
    }
    const manageLink = templateContent.querySelector('#manageLink')
    if (!manageLink) {
      console.error('[Huhi Settings Overrides] Could not find manageLink in passwords_section')
    } else {
      manageLink.remove()
    }
  }
