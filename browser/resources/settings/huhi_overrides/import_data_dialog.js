...
import {RegisterPolymerTemplateModifications, RegisterPolymerComponentBehaviors} from 'chrome://huhi-resources/polymer_overriding.js'
...
RegisterPolymerComponentBehaviors({
  'settings-import-data-dialog': [{
    registered: function () {
      ...
      if (!oldPrefsChanged) {
        console.error('[Huhi Settings Overrides] cannot find prefsChanged_ on ImportDataDialog')
        return
      }
      this.prefsChanged_ = function () {
        if (typeof this.noImportDataTypeSelected_ !== 'boolean') {
          console.error('[Huhi Settings Overrides] cannot find noImportDataTypeSelected_ on ImportDataDialog')
          return
        }
