...
RegisterPolymerTemplateModifications({
  'settings-security-page': (templateContent) => {
    ...
    if (!safeBrowsingReportingToggle) {
      console.error('[Huhi Settings Overrides] Could not find safeBrowsingReportingToggle id on security page.')
    } else {
    ...
    if (!passwordsLeakToggle) {
      console.error('[Huhi Settings Overrides] Could not find passwordsLeakToggle id on security page.')
    } else {
      passwordsLeakToggle.setAttribute('hidden', 'true')
    }
    const link = templateContent.getElementById('advanced-protection-program-link')
    if (!link) {
      console.error('[Huhi Settings Overrides] Could not find advanced-protection-program-link id on security page.')
    } else {
